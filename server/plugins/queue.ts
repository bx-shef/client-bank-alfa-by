...
import { accountsForPolling, buildDemoFetchJobs, cronIntervalMs, demoTickMs, planFetches, pollSkipReason, pollWindow } from '../queue/cron'
      ...
      const poll = async () => {
        try {
          ...
          await withSpan('cron.real-poll', { 'job.queue': 'cron.real-poll' }, async () => {
            // ⚠ КАЖДЫЙ портал опрашивает СВОЁ подключение, даже если счёт у двух порталов один
            // и тот же (решение владельца 2026-09-10). Прежде здесь стояла свёртка «один счёт —
            // один опрос» (#615): она берегла лимит банка и снимала гонку за токеном, но требовала,
            // чтобы счёт назвал сам банк, а на боевом он его не называл — и свёртка не включалась
            // ни разу. Цена отказа от неё названа и принята: лишние переиздания пары, которые
            // Альфа переживает сама (переспрашивание токена на 401 + пауза перед повтором).
            const rows = await listAllBankAccountInfo(dbQuery)
            const byPortal = accountsForPolling(rows)
      ...
      const runSweep = async () => {
        try {
          ...
          await withSpan('cron.sweep', { 'job.queue': 'cron.sweep' }, async () => {
            ...
            // гоняется за ротацией refresh с другими порталами, у которых тот же счёт, а
