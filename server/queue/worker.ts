...
export function liveHandlerDeps(): HandlerDeps {
  return {
    ...
    deletePortal: async (memberId, eventTs, reason) => {
      ...
      // ⚠ След в журнале ДО удаления (#641). Это самый разрушительный из путей, стирающих связь
      // клиента с банком, и единственный, который до этого не оставлял в логе ничего вовсе. Живой
      // разбор 2026-08-26 упёрся ровно в это: строк `bank_tokens` не осталось, а кто их убрал —
      // неоткуда узнать.
      //
      // ⚠ ПОВОДА ДВА, и различает их только вызывающий (`reason`). Первая редакция писала
      // «приложение УДАЛЕНО» безусловно — а сюда же ходит уборщик мёртвых грантов (#574), у
      // которого клиент ничего не удалял, он просто исчез. Оператор, читающий журнал ровно затем,
      // чтобы понять, куда делось подключение, получил бы уверенный неверный ответ (находка ревью,
      // независимо у троих).
      //
      // ⚠ Формулировка — НАМЕРЕНИЕ, а не свершившийся факт: строка печатается ДО стирания (шагов
      // семь, транзакции нет — частичный отказ на середине иначе не оставил бы и её), поэтому
      // «стёрли» здесь было бы утверждением о том, чего ещё не произошло, а при отказе на первом
      // же шаге — навсегда ложным.
      //
      // ⚠ Портал ХЕШИРУЕТСЯ, в отличие от соседних строк того же канала из роутов. Довод тот же,
      // по которому хеширует уборщик (`portalReaperRun.ts`): строки этих таблиц прямо сейчас
      // уничтожаются, и лог остаётся ЕДИНСТВЕННЫМ пережившим упоминанием связи «этот портал ↔ мы
      // держали его банковские креды». У роутов портал остаётся жить, и там это не так.
      bankConnectLog.warning(
        `portal ${portalHash(memberId)}: стираем ВСЁ, включая подключения к банкам — ${reason === 'uninstall'
