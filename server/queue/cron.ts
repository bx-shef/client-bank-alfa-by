...
/**
 * Можно ли вообще опрашивать это подключение. ОДНА точка на все пути: крон и ручное «Опросить
 * сейчас».
 *
 */
export function isPollableAccount(ref: BankAccountRef): boolean {
  ...
  //
  // ⚠ Пауза — СВОЙСТВО ПОДКЛЮЧЕНИЯ, а не счёта: один и тот же номер, подключённый из двух Б24,
  // это две строки, и пауза одного портала не останавливает опрос второму.
