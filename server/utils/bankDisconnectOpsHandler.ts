...
// (`bankDisconnectReason` вернул причину) ЛИБО принадлежащее порталу с мёртвой подпиской на REST
// (#614). Рабочее подключение живого клиента из операторской не трогаем никогда — иначе это способ
// тихо оборвать импорт.
//
// ⚠ Вторая ветка добавлена потому, что там банк как раз ЖИВ, а нерабочее — всё остальное: клиент
// не может открыть приложение и потому не может отключиться сам. Метка подписки спрашивается
// ТОЛЬКО когда банковской причины нет — лишний запрос к базе на общем пути не нужен.
//
