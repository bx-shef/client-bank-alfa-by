  ...
  return {
    ...
    body: {
      accounts: accounts.map(a => ({
        ...
        // ⚠ Когда мы последний раз ПЫТАЛИСЬ обновить токен (#488). Без него экран не может
        // отличить «банк отказал» от «продление ни разу не бралось за эту строку», а это
        // противоположные действия: в первом случае помогает переподключение, во втором оно
        // вернёт зелёный бейдж ровно на один срок жизни токена. Не секрет — это метка времени
        // НАШЕЙ попытки, ни номера, ни токена в ней нет.
        lastAttemptAt: a.lastAttemptAt,
        // ⚠ Отказ БАНКА в гранте (#713). Без него карточка не могла сказать «истекло» там, где
        // срок жизни refresh у нас не измерен (Приор), и мёртвое подключение месяцами выглядело
        // здоровым. Не секрет: метка времени чужого ОТВЕТА, ни номера, ни токена в ней нет.
        refreshRejectedAt: a.refreshRejectedAt,
