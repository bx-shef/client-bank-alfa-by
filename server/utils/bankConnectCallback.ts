...
export async function handleBankConnectCallback(deps: CallbackDeps, input: CallbackInput): Promise<CallbackResult> {
  ...
  await deps.saveToken({
    ...
    // ГРАНТ — одно прохождение OAuth (#23). Согласие банк выдаёт на НАБОР счетов клиента, поэтому
    // второй и шестой счёт не требуют повторного входа владельца в интернет-банк: им нужна своя
    // строка под ТЕМ ЖЕ грантом, и обновление токена пишет ротированную пару сразу во все.
    //
    // ⚠ Значение — nonce из ПРОВЕРЕННОГО state: он криптослучаен и уникален на подключение, то есть
    // ровно то, чем грант и является. Заводить рядом второй идентификатор значило бы держать два
    // источника одной истины. Секретом он не является и никогда им не был — он ездит в authorize-URL
    // и служит только ключом группировки.
    grantId: state.nonce
  })
