...
// id comes from the payment purpose (`invoice-id`/`deal-id`/`smart-id`/`order-id`/
// `payment-id`) and is therefore UNTRUSTED — so the lookup filters by `companyId`
// in the query (IDOR: only the resolved counterparty's own entity comes back, never
// an arbitrary one whose id a payer guessed) and drops negative-stage entities.
//
 ...
 * `deal`/`smart-process` ignore them). A non-finite amount is normalized to 0.
 */
