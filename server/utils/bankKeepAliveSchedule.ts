...
export function liveBankKeepAliveDeps() {
  return {
    ...
    // ⚠ `force` обязателен, но по ДРУГОЙ причине, чем здесь было написано до 2026-09-06.
    // Сам по себе срок access-токена — сигнал НЕДОСТАТОЧНЫЙ (access бывает свежим, пока refresh
    // доживает последние часы), поэтому решение «идти ли в банк» принимает отбор, а не
    // `ensureBankToken`; `force` и нужен, чтобы отбор не переспрашивали.
    // ⚠ Здесь стояло «именно поэтому обновление по дороге никогда не спасало» — это ОПРОВЕРГНУТО
    // замером: обновление по дороге опроса, идущее по сроку access примерно раз в час, держало
    // подключение ШЕСТЕРО СУТОК. Умирало оно ровно тогда, когда опрос прекращался и оставалась
    // одна пятичасовая полоса refresh (#488). Срок access теперь учитывает и отбор.
