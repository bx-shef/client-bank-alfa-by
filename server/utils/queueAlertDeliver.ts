...
const RECOVERY_SENTENCE: Record<string, (subject: string) => string> = {
  ...
  // Подлежащее — воркеры, а не очередь: общий фолбэк вклеил бы английское «workers» в русскую
  // фразу. Тип `Record<string, …>` полноту не форсирует, поэтому стережёт тест.
  'no-workers': () => 'воркеры снова на связи — очереди разбираются.'
  // Подлежащее — воркеры, а не очередь: общий фолбэк вклеил бы английское «workers» в
  // русскую фразу. Тип `Record<string, …>` полноту не форсирует, поэтому стережёт тест.
