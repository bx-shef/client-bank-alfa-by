...
// Renders an HTML page. This URL must EXACTLY match the redirect URI registered with the bank
// (PRIOR_OAUTH_REDIRECT_URI).
//
// ⚠ ЗДЕСЬ ТОЛЬКО ПРИОР (#488): у Альфы authorize-поток убран, она подключается ключом API
// (`/api/bank/connect-key`). Провайдера по-прежнему берём из ПРОВЕРЕННОГО state, а не из запроса,
// и состояние Альфы, подписанное до выката, отвергается страницей отказа.
