...
describe('unmatchedNoticeKey', () => {
  ...
  it('срок переживает окно опроса — и не превращается в месяц мусора', () => {
    // ⚠ Границы с ОБЕИХ сторон, и обе содержательные. Снизу: окно перекрывает максимум выходные
    // плюс рабочий день (трое суток), отметка обязана пережить его с запасом — иначе операция
    // вернётся в ту же выписку уже без памяти. Сверху: значений вроде 10 или 30 суток не бывает,
    // они означают лишь месяц мусора в Redis (замечание владельца, 2026-09-15).
    const day = 24 * 60 * 60
    expect(UNMATCHED_NOTICE_TTL_SEC).toBeGreaterThanOrEqual(4 * day)
    expect(UNMATCHED_NOTICE_TTL_SEC).toBeLessThanOrEqual(14 * day)
  })
})
