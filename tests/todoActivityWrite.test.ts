...
describe('маркер проверяется на деле, а не на слово (#495)', () => {
  ...
  it('маркер НЕ находится — падаем громко И сносим за собой дело', async () => {
    // Без сноса каждая попытка BullMQ оставляла бы по одному ненаходимому делу: джоба падает,
    // ретрай не находит маркер, пишет заново, снова падает. Ограниченно числом попыток — но ноль
    // лучше, чем «немного», а если маркер на самом деле стоял и мы его просто не прочитали,
    // удаление тоже безвредно: повторный прогон создаст дело заново.
    ...
    expect(seen).toContain(ACTIVITY_DELETE_METHOD)
  })
