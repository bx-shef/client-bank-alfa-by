...
const OPERATOR = ['prod-up', 'prod-down', 'prod-pull', 'prod-redeploy', 'logs', 'ps',
  ...
  'bank-history',
  // #488: обновление токена по требованию и лестница пауз — ОДНИ на трёх поставщиков (P=alfa|
  // prior|b24). Общий способ нужен, чтобы ответы трёх банков потом можно было сравнить.
  'refresh-now', 'refresh-ladder', 'refresh-ladder-log', 'refresh-ladder-stop',
  // #488: почему банк не принял ключ API. Заведена по живому отказу 2026-09-09, когда браузер
  // сказал «банк не принял ключ», а в логе не было НИЧЕГО, кроме имени класса исключения.
  'bank-connect-log',
  // #19: почему в чат не дошли картинки. Та же болезнь, что у соседа выше, и заведена по тому же
  // поводу: снаружи «картинок нет», а причин три, и живут они в ДВУХ каналах лога backend'а —
  // `make logs` тайлит nginx и не показывает ни одной из них.
  'chat-log',
  // #488: форсировать обновление токена Альфы и показать, что шлём и что ответил банк — то, что
...
describe('операторские цели Makefile видны в `make help`', () => {
  ...
  it('цели, зовущие скрипты, тянут их из того же источника', () => {
    // Иначе `self-update` обновит Makefile из одной ветки (или репозитория), а скрипты приедут из
    // другой. Источник один — `$(SRC)`: копия клиентского репозитория на его сервере либо наш
    // репозиторий на `$(REF)`; `$(RAW)` — его подкаталог `scripts/`.
    ...
    for (const m of found) {
      ...
      expect(curlLine, `${m[1]!} качает не из $(SRC)`).toMatch(/"\$\((RAW|SRC)\)\//)
    }
  })
})
