...
describe('серверные команды: рантбук ⇄ Makefile ⇄ scripts (#487)', () => {
   ...
   *
   * ⚠ `is` — из англоязычных правил для агентов (`docs/AGENT_RULES.md`, 2026-08-27): «an edit you
   * did not make is a neighbour working». Здесь `make` — обычный глагол. Исключение узкое и
   * безопасное: цели `make is` не существует и существовать не может, а файл остаётся под гардом
   * для настоящих упоминаний целей. Альтернатива — вывести весь файл из-под проверки — молча
   * сняла бы её и с будущих строк, где цель действительно назовут.
   */
  const PROSE_AFTER_MAKE = new Set(['target', 'is'])
