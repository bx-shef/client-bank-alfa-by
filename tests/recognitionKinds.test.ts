...
describe('MATRIX_PRESETS / missingPresets', () => {
  ...
  it('в пресетах НЕТ ни одной маски без литерального якоря (#490)', () => {
    // Инвариант, а не пример. Пресет применяют ВСЛЕПУЮ, одной кнопкой, на портале, назначений
    // которого никто ещё не видел — значит он обязан быть безопасным без настройки. Маска без
    // якоря такой быть не может: она выхватывает фрагмент из чего угодно.
    for (const p of MATRIX_PRESETS) {
      expect(p.mask.replace(/d\+?/g, ''), p.mask).toMatch(/[^\s]/)
      expect(/^[dD]/.test(p.mask.trim()), `${p.mask} начинается с цифрового плейсхолдера`).toBe(false)
    }
  })

  it('составной номер договора пресетами НЕ распознаётся (#490)', () => {
    // Замерено на боевых назначениях: `d+/d+` дала ровно одно совпадение, и оно было ложным —
    // из составного номера договора вырезался хвост. Дефис в алфавитно-цифровой класс не входит,
    // поэтому граница слева честно проходит, а белорусская нумерация договоров сплошь составная.
    // Форма примера сохранена, значения синтетические: репозиторий публичный.
    const ids = recognizeByMatrices('СОГЛАСНО ДОГОВОРУ 30-06/765 ОТ 29.12.2015', MATRIX_PRESETS, 'cyrillic')
    expect(ids).toEqual([])
  })
