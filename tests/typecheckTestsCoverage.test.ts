...
describe('юнит-тесты покрыты typecheck (#527)', () => {
  ...
  it('охват включает scripts/** — они ПИШУТ в боевые порталы (#542)', () => {
    // ⚠ Риск-профиль здесь выше, чем у тестов: `mutate-payment-live`, `todo-activity-test`,
    // `trigger-register-test` с `--apply` мутируют CRM живого клиента, а запускаются они через
    // `node --experimental-strip-types` — это ВЫРЕЗАНИЕ типов, без всякой проверки. Замер при
    // включении дал пять ошибок, включая нарушение контракта в самом мутирующем скрипте.
