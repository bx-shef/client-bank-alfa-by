...
describe('согласие банка перекрывает оценку по refresh (#503)', () => {
  // ⚠ Два РАЗНЫХ срока: refresh-токен и согласие. У Приора согласие живёт три года, и когда оно
...
describe('ПОЧЕМУ истекло: банк отказал или мы не пытались (#488)', () => {
  ...
  it('⚠ дата успеха нечитаема, а попытка БЫЛА — это «банк отказал»', () => {
    // Ветка `!Number.isFinite(success)` (находка ревью): единственный тест, который её упоминал,
    // отдавал `lastAttemptAt: 0` и до неё не доходил — короткое замыкание срабатывало раньше.
    // Мутация «вернуть здесь never-tried» проходила зелёной.
    expect(expiredCause(dead({ connectedAt: Number.NaN, lastAttemptAt: NOW - 3_600_000 }), NOW))
      .toBe('bank-refused')
  })
  ...
  it('подсказка «не пытались» НЕ советует переподключать', () => {
    ...
    // посылала его в `make poll-check` — команду на нашем сервере, к которой у него нет доступа:
    // невыполнимый совет плюс лишняя подробность о нашей инфраструктуре.
    expect(hint, 'внутренняя команда оператора уехала клиенту').not.toContain('poll-check')
    expect(hint).not.toContain('make ')
    // Переподключение не выдаётся за починку, но и не запрещается: импорт бывает нужен сегодня.
    expect(hint).toContain('вернёт')
  })
