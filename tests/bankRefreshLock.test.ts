...
describe('ключ лока строится ТОЛЬКО хелпером', () => {
  it('каждый ПИСАТЕЛЬ в bank_tokens классифицирован явно', () => {
    ...
    const CLASSIFIED: Record<string, string> = {
      ...
      // ⚠ БЕЗ лока намеренно (#713), по тому же правилу, что два соседа выше: пишет ТОЛЬКО
      // `refresh_rejected_at` — колонку, за которую обновление токена не борется. Более того,
      // взять лок здесь НЕЛЬЗЯ ФИЗИЧЕСКИ: отметка ставится ВНУТРИ уже залоченной секции
      // `ensureBankToken`, в ветке отказа банка, — попытка взять тот же лок повторно была бы
      // взаимоблокировкой с самим собой.
      // ⚠ Гонки с успехом нет по построению: успешное обновление сбрасывает эту колонку в НОЛЬ тем
      // же оператором, которым пишет свежую пару, и оба идут под одним и тем же грантовым локом.
      // ⚠ Появится второе поле в этом UPDATE — классификация обязана пересматриваться.
      markBankRefreshRejected: 'unlocked-writes-only-the-rejection-stamp'
    }
...
describe('маршруты, ждущие этот лок, не могут выесть пул соединений', () => {
  ...
  // запроса счетов жила прямо в роуте сверки и была вынесена в `server/utils`; роут при этом берёт
  // лок ровно как прежде, а гард, смотрящий только на текст `server/api/**`, перестал его видеть —
  // то есть вынос функции молча снял проверку, ради которой гард и написан (#539). Ровно тот
  // способ, каким появился сам #539.
