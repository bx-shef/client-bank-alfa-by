...
const ADMIN_ONLY = [
  ...
  // ⚠ Маршрутов САМОСТОЯТЕЛЬНОГО подключения (`bank/connect.post.ts` и `bank/connect-key.post.ts`)
  // здесь больше НЕТ — они удалены вместе с кнопкой «Подключить» (решение владельца 2026-09-17).
  // Подключение делает только владелец счёта, получив приглашение; ключ API он вводит на своём
  // экране `bank/submit-key.post.ts`, гейт которого — подписанный грант и совпадение личности, а
  // не должность, поэтому в этом списке ему не место.
  // ⚠ Передача подключения владельцу счёта (#19) — админская по тому же доводу, по которому было
  // админским само подключение: сообщение приглашает привязать банковские креды ко ВСЕМУ порталу,
  // просто чужими руками. Послабление здесь означало бы, что любой сотрудник рассылает такие
  // приглашения.
...
const ANY_PORTAL_USER = [
  'app-rating.get.ts',
  // ⚠ ЭКРАН ВЛАДЕЛЬЦА СЧЁТА (#19) — НЕ админский НАМЕРЕННО: он для того и заведён, чтобы ключ API
  // вводил тот, кто его выпустил, а не администратор. Но «любой сотрудник» здесь неточно, и это
  // важно не потерять: сверх фрейм-токена оба маршрута требуют ПОДПИСАННЫЙ ГРАНТ, выданный
  // администратором конкретному человеку, и сверяют `userId` гранта с тем, кто открыл экран.
  // Разбор — шапка `server/utils/bankKeySubmit.ts`; отказ обоих проверен тестами на 403.
  'bank/key-request.get.ts',
  'bank/submit-key.post.ts',
  'app-rating.post.ts',
  'chat-search.get.ts',
