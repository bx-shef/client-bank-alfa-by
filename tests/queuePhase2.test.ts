...
import {
  handleCrmSyncJob, handleEventJob, handleFetchJob, handleParseJob, type HandlerDeps, MAX_BACKFILLS_PER_RUN, MAX_DEFERRED_WRITES_PER_RUN, MAX_UNRESOLVED_NOTICES } from '../server/queue/handlers'
import {
...
interface FakeOpts {
  ...
  /** #45: какой исход возвращает дозаливка (по умолчанию 'filled'). */
  backfillOutcome?: 'filled' | 'already' | 'created'
...
function fakeDeps(opts: FakeOpts | StatementItem[] = {}): { deps: HandlerDeps, calls: Record<string, unknown[]> } {
  ...
  const deps: HandlerDeps = {
    ...
    backfillRegistry: async (it, companyId, memberId, provider, paymentSp) => {
      calls.backfill.push([it.docId, companyId, memberId, provider, paymentSp.entityTypeId])
      if (o.backfillThrows) throw new Error('portal refused the backfill update')
      return o.backfillOutcome ?? 'filled'
    },
...
describe('handleFetchJob / handleParseJob → crm-sync', () => {
  ...
  it('пачка уходит ТОЛЬКО опрашивавшему порталу', async () => {
    // ⚠ Раздача соседям по совпадению НОМЕРА счёта (#615) снята 2026-09-10: она требовала
    // подтверждения счёта САМИМ банком (введённый руками номер доказательством не является —
    // иначе админ чужого портала вписал бы ваш IBAN и получил бы вашу выписку в свою CRM), а
    // боевая Альфа счёт среди своих не называла, и раздача не включалась ни разу.
    const { deps, calls } = fakeDeps({ batch: [item('d1')] })
    await handleFetchJob(fetchJob, deps)
    expect((calls.crm as CrmSyncJob[]).map(j => j.memberId)).toEqual(['M'])
  })
...
describe('handleCrmSyncJob', () => {
  ...
  // ⚠ #45: дедуп-гейт стоит РАНЬШЕ записи реестра, поэтому портал, обновивший смарт-процесс уже
  // после импорта, не мог заполнить историю НИКОГДА — колонки появлялись только у операций,
  // которых ещё нет в CRM. На живом портале это стоило владельцу ручного удаления дел и элементов.
  // Чинит повторная ЗАГРУЗКА ФАЙЛА — ровно тот путь, что назван человеку в сообщениях (#43).
  describe('#45 дозаливка колонок реестра у уже записанной операции', () => {
    const spCfg: RecognitionSettings = {
      alphabet: 'cyrillic', matrices: [],
      configFields: { 'payment-sp': '1044', 'payment-sp-id': '44', 'distribution-sp': '1046', 'distribution-sp-id': '46' }
    }
    const parseJob = (items: StatementItem[]): CrmSyncJob => ({
      memberId: 'M', providerId: 'alfa-by', source: 'parse', batchId: 'b', items
    })

    it('ручная загрузка дозаполняет операцию, отсеянную дедупом', async () => {
      const { deps, calls } = fakeDeps({ company: '77', recognition: spCfg, alreadyWritten: new Set([dedupKey(item('old'))]) })
      const r = await handleCrmSyncJob(parseJob([item('old')]), deps)
      expect(r.skipped, 'операция всё так же отсеяна — второго дела быть не должно').toBe(1)
      expect(calls.activity, 'дело НЕ создаётся повторно').toHaveLength(0)
      expect(calls.backfill).toEqual([['old', '77', 'M', 'alfa-by', 1044]])
      expect(r.registryBackfilled).toBe(1)
    })

    // ⚠ Несущее ограничение: окно автоопроса намеренно нахлёстывается, и сквозь дедуп на каждом
    // тике проходят все вчерашние операции. Безусловная дозапись стоила бы лишнего вызова на
    // каждую из них каждые несколько минут — навсегда, ради состояния, случающегося однажды.
    it('АВТООПРОС не платит за дозаливку — деп не зовут вовсе', async () => {
      const { deps, calls } = fakeDeps({ recognition: spCfg, alreadyWritten: new Set([dedupKey(item('old'))]) })
      const r = await handleCrmSyncJob(job([item('old')]), deps) // source: 'fetch'
      expect(r.skipped).toBe(1)
      expect(calls.backfill, 'опрос не должен трогать историю реестра').toHaveLength(0)
      expect(r.registryBackfilled).toBe(0)
    })

    it('без смарт-процесса дозаливать нечего — деп не зовут', async () => {
      const { deps, calls } = fakeDeps({ alreadyWritten: new Set([dedupKey(item('old'))]) }) // без configFields
      await handleCrmSyncJob(parseJob([item('old')]), deps)
      expect(calls.backfill).toHaveLength(0)
    })

    // ⚠ КАП: файл несёт сотни операций, а очередь `crm-sync` идёт по одной задаче — без потолка
    // один портал занимал бы её десятки минут. Остаток обязан быть НАЗВАН, иначе оборванная на
    // середине дозаливка выглядит законченной и человек не догадается повторить загрузку.
    it('дозаливка ограничена капом за прогон, остаток посчитан', async () => {
      const many = Array.from({ length: MAX_BACKFILLS_PER_RUN + 2 }, (_, i) => item(`old-${i}`))
      const { deps, calls } = fakeDeps({
        company: '77',
        recognition: spCfg,
        alreadyWritten: new Set(many.map(dedupKey))
      })
      const r = await handleCrmSyncJob(parseJob(many), deps)
      expect(calls.backfill, 'сверх капа в портал не ходим').toHaveLength(MAX_BACKFILLS_PER_RUN)
      expect(r.registryBackfilled).toBe(MAX_BACKFILLS_PER_RUN)
      expect(r.registryBackfillSkipped, 'остаток назван — нужна ещё одна загрузка').toBe(2)
    })

    it('отказ дозаливки не роняет пачку — считается отдельно от registryFailed', async () => {
      const { deps } = fakeDeps({
        recognition: spCfg,
        alreadyWritten: new Set([dedupKey(item('old'))]),
        backfillThrows: true
      })
      const r = await handleCrmSyncJob(parseJob([item('old')]), deps)
      expect(r.registryBackfillFailed).toBe(1)
      expect(r.registryFailed, 'это ДРУГОЙ отказ: там элемент не создан вовсе').toBe(0)
  ...
  it('is idempotent across job redelivery (round-trip through the B24 marker)', async () => {
    ...
    expect(first).toMatchObject({ created: 2, notified: 2, skipped: 0, excluded: 0, directionSkipped: 0, nonPayment: 0, unreadableAmount: 0, registryFailed: 0, registryBackfilled: 0, registryBackfillFailed: 0, registryBackfillSkipped: 0, bindingsFailed: 0, unmatched: 0 })
  ...
  it('excluded counterparty → op skipped ENTIRELY: no company lookup, no activity, no chat (PROCESSING §2 A2)', async () => {
    ...
    expect(r).toEqual({ processed: 1, landed: 0, created: 0, notified: 0, skipped: 0, excluded: 1, directionSkipped: 0, nonPayment: 0, unreadableAmount: 0, registryFailed: 0, registryBackfilled: 0, registryBackfillFailed: 0, registryBackfillSkipped: 0, bindingsFailed: 0, unmatched: 0, unresolved: 0, misconfigured: 0, recognized: 0, resolved: 0, allocatable: 0, ambiguous: 0, manual: 0, allocated: 0, distributed: 0, ledgerWritten: 0, credits: 1, debits: 0 })
  ...
  // #572 — «карта распознавания настроена неверно». Отдельный статус от `unsupported` («не искали»)
  // и от `resolved` («искали, не нашли»): портал прямо сказал, что такого поля нет.
  it('портал отверг фильтр → счётчик misconfigured и ОДНО сообщение на прогон', async () => {
    // ⚠ Три операции, а сообщение обязано быть одно: причина у них общая (админ указал поле,
    // которого нет), и по сообщению на операцию чат ошибок залило бы одинаковыми строками.
    const { deps, calls } = fakeDeps({
      recognition: invoiceMatrix,
      company: 'CO',
      errorChat: { dialogId: 'err' },
    ...
    const r = await handleCrmSyncJob(job([
      ...
      item('d2', 'credit', 'Оплата по счету СЧ-1234'),
      item('d3', 'credit', 'Оплата по счету СЧ-1234')
    ]), deps)
    expect(r.misconfigured, 'считаем ОПЕРАЦИИ, а не резолюции').toBe(3)
    expect(calls.settingsChat).toHaveLength(1)
    expect((calls.settingsChat[0] as unknown[])[1], 'ушло в чат ОШИБОК').toBe('err')
  })

  it('неверная настройка НЕ считается «искали и не нашли»', async () => {
    // ⚠ Иначе бухгалтер получил бы совет «проверьте номер и статус документа» — то есть пошёл бы
    // искать несуществующую проблему в платеже, тогда как чинится это на экране настроек.
    const { deps, calls } = fakeDeps({
      recognition: invoiceMatrix,
      company: 'CO',
      errorChat: { dialogId: 'err' },
  ...
  it('без чата ошибок неверная настройка всё равно СЧИТАЕТСЯ', async () => {
    ...
    expect(calls.settingsChat).toEqual([])
  })
  ...
  it('операция без движения денег НЕ становится делом и считается отдельно (#735)', async () => {
    // ⚠ Тест ПЕРЕПИСАН: прежде он закреплял обратное — «дело всё равно пишется, человек увидит
    // артефакт». Замер боевой валютной выписки Альфы это опроверг: таких строк там 22 из 23 за
    // месяц по одному счёту («Переоценка входящего остатка»), то есть «увидит» означало 22 дела
    // «Приход 0,00 USD», которые бухгалтеру нужно закрыть руками.
    const { deps, calls } = fakeDeps({ recognition: dealMatrix, resolve: [dealAt('77')], autoDistribute: true, allocation: { triggerCode: 'cbatest_pay' } })
    const zero = { ...item('d1', 'credit', 'оплата Д-55'), amount: 0 }
    const r = await handleCrmSyncJob(job([zero]), deps)
    expect(r.created).toBe(0)
    expect(calls.crm).toEqual([]) // ни одного REST: гейт стоит до похода в портал
    expect(calls.resolve).toEqual([])
    expect(calls.trigApply).toEqual([])
    expect(r.nonPayment).toBe(1)
    expect(r.unreadableAmount).toBe(0) // штатная запись банка, а не наш дефект разбора
  })

  it('отрицательная сумма считается ОТДЕЛЬНО от нулевой — это наш дефект разбора (#735)', async () => {
    // Направление у нас отдельным полем, поэтому минус в сумме не «расход», а признак того, что мы
    // неверно прочли файл или ответ банка. Слить его с «без движения денег» значило бы записать
    // собственную ошибку в штатные записи банка и никогда её не увидеть.
    const { deps } = fakeDeps({})
    const negative = { ...item('d1', 'credit', 'оплата'), amount: -5 }
    const r = await handleCrmSyncJob(job([negative]), deps)
    expect(r.created).toBe(0)
    expect(r.unreadableAmount).toBe(1)
    expect(r.nonPayment).toBe(0)
  })

  it('копейки ниже цента отбрасываются как нулевые, а не пишутся делом «0,00» (#735)', async () => {
    // Округление ДО сравнения с нулём: 0,004 проходит «> 0» и даёт дело «Приход 0,00».
    const { deps } = fakeDeps({})
    const dust = { ...item('d1', 'credit', 'оплата'), amount: 0.004 }
    const r = await handleCrmSyncJob(job([dust]), deps)
    expect(r.created).toBe(0)
    expect(r.nonPayment).toBe(1)
  })
...
describe('#579 привязки дела к сущностям CRM', () => {
  ...
  it('сущность списания и вторая компания привязаны; элемент реестра и владелец — нет', async () => {
    const exact: IntentResolution[] = [{
      kind: 'invoice-number', value: 'СЧ-0001', status: 'resolved',
      candidates: [{ kind: 'invoice', id: '7', amount: 10, currency: 'BYN' }]
    }]
    ...
    // 4:9 — моя компания, 31:7 — смарт-счёт (сущность списания).
    ...
    // отвечает ошибкой.
    // ⚠ Элемента реестра (1044:…) быть НЕ должно: привязка ДИНАМИЧЕСКОГО типа перехватывает
    // владельца дела — замерено на живом портале, дело становится «принадлежащим» смарт-процессу,
    // и в списке дел колонка «Клиент» показывает «Импорт выписки: платежи #45» вместо плательщика.
    // Вернуть владельца нечем: `crm.activity.update` отвечает «Fields is not specified».
    expect(refs).toEqual(['4:9', '31:7'])
  })
