...
import { abandonedPending, connectionHealth } from '../app/utils/bankTokenLifetime'
...
describe('угаданный срок жизни не хоронит подключение (ревью #489)', () => {
  ...
  const row = (provider: 'alfa-by' | 'prior-by', ageMs: number) => ({
    ...
    id: 1, lastAttemptAt: 0, refreshRejectedAt: 0, consentExpiresAt: 0, grantId: ''
  })
...
describe('истёкшее согласие не тратит запросы банка (#503)', () => {
  ...
  const row = (over: Record<string, unknown> = {}) => ({
    ...
    id: 1, lastAttemptAt: 0, refreshRejectedAt: 0, pollPaused: false, grantId: '', ...over
  })
...
describe('подключение не хоронится без вопроса к банку (#489, живой прогон 2026-08-20)', () => {
  const row = (over: Partial<BankAccountInfo> = {}): BankAccountInfo => ({
    ...
    connectedAt: 0, lastAttemptAt: 0, refreshRejectedAt: 0, expiresAt: 0, hasRefresh: true, consentExpiresAt: 0,
...
describe('счёт, подключённый с НЕСКОЛЬКИХ порталов (#488)', () => {
  ...
  // ⚠ У АЛЬФЫ действия нет, и звать отключать портал нельзя (живой прогон 2026-09-10): пара там
  // правда обесценивается соседом, но выпускается заново ключом API — человек не нужен, забор
  // проходит. Прежняя редакция советовала оборвать РАБОТАЮЩЕЕ подключение.
  it('у Альфы общий счёт НЕ зовёт отключать портал', () => {
    const alfa = sharedAccountNotices([row('A', 'BY09'), row('B', 'BY09')])
    expect(alfa).toHaveLength(1)
    expect(alfa[0]?.text, 'совет отключить портал вернулся').not.toMatch(/disconnect|ONE portal/i)
  ...
  // ⚠ У ПРИОРА ровно наоборот, и это замер ночи 2026-09-11: портал, авторизовавшийся первым, к
  // утру мёртв (`invalid_grant: Persisted access token data not found`, заборов ноль), сосед
  // минутой позже — работает. Значит выживает ПОСЛЕДНИЙ авторизовавшийся, а действие есть и оно
  // единственное — оставить счёт на одном портале.
  // ⚠ Промежуточная редакция говорила «у каждого портала свой грант, жечь нечего» и была `info`.
  // Она не следовала из замера: продление за эти строки тогда не бралось ни разу (`selected=0`).
  it('⚠ у Приора сказано, что выживает ПОСЛЕДНИЙ, и это предупреждение', () => {
    ...
    expect(notices[0]?.text, 'вернулось «жечь нечего»').not.toMatch(/OWN grant|nothing to burn/i)
    expect(notices[0]?.text, 'не сказано, что выживает последний').toMatch(/LAST AUTHORISED/)
    expect(notices[0]?.text, 'не названо действие').toMatch(/ONE portal/)
    expect(notices[0]?.text, 'Приору приписали ключ API').not.toMatch(/API key/)
    // Действие есть ⇒ это предупреждение, а не справка.
    expect(notices[0]?.level, 'причину ночной смерти понизили до info').toBe('warn')
  })
...
// Ожидающее подключение, чей грант банк уже отверг, больше не долбит банк каждый час (#715).
//
// Замерено на живом прогоне: строка `~pending:` Приора получала `invalid_grant` на КАЖДОМ тике
// продления и оставалась — свип #485 сносит её только по потолку возраста (двое суток), потому
// что путь в `expired` открыт лишь ИЗМЕРЕННОМУ сроку, а у Приора срок — догадка.
describe('#715 ожидающее подключение, отвергнутое банком', () => {
  const rejected = { accountKey: '~pending:abc', provider: 'prior-by' as const, connectedAt: NOW - 9 * HOUR, refreshRejectedAt: NOW - HOUR }

  it('в банк не ходим — обновлять нечего', () => {
    const r = selectBankAccountsNearExpiry([acc(rejected)], NOW)
    expect(r.due, 'отвергнутая ожидающая строка попала в план продления').toEqual([])
    expect(r.expired).toHaveLength(1)
  })

  it('ГАРД: правило узкое — РАБОЧУЮ строку с тем же отказом продлевать продолжаем', () => {
    // Цена поспешного вывода у рабочей строки — поход владельца счёта в интернет-банк, а у Альфы
    // отказ вдобавок лечится переизданием пары ключом API. Мутация «снять проверку `~pending:`»
    // обязана ронять этот тест.
    const r = selectBankAccountsNearExpiry([acc({ ...rejected, accountKey: 'BY00BANK00000000000000000009' })], NOW)
    expect(r.due).toHaveLength(1)
  })

  it('ГАРД: ожидающая строка БЕЗ отказа банка продлевается как прежде', () => {
    // Иначе починка молча похоронила бы подключение, которое админ вот-вот доведёт до рабочего:
    // «банк отверг» и «счёт ещё не выбран» — разные вещи.
    const r = selectBankAccountsNearExpiry([acc({ ...rejected, refreshRejectedAt: 0 })], NOW)
    expect(r.due).toHaveLength(1)
  })

  it('устаревшая метка (отказ ДО последней удачной пары) продление не блокирует', () => {
    const r = selectBankAccountsNearExpiry([acc({ ...rejected, refreshRejectedAt: NOW - 20 * HOUR })], NOW)
    expect(r.due).toHaveLength(1)
  })

  it('свип сносит такую строку сразу, не дожидаясь потолка возраста', () => {
    // Вторая половина #715: продление её больше не трогает, а свип обязан её убрать — иначе она
    // просто висела бы в базе. Работает через `connectionHealth` → `expired` (#713).
    expect(abandonedPending(acc(rejected), NOW)).toBe(true)
  })
})
