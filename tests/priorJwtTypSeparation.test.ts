...
//
// ⚠ ДВЕ СТОРОНЫ ЗАЩИЩЕНЫ ПО-РАЗНОМУ, и это стоит знать, выбирая, что чинить при красном.
// `PriorAuthDeps.signJwt` (assertion) типизирован СТРОГО тремя аргументами, поэтому четвёртый там
// ловит компилятор (`TS2554`) — для этой стороны гард дублирует typecheck. А `PriorConnectDeps.signJwt`
// (authorize) несёт `typ?: string`, и заставить звать функцию с ОПЦИОНАЛЬНЫМ параметром типы не могут
// в принципе: тихое удаление `config.requestTyp` из вызова проходит typecheck начисто и молча
// откатывает поведение к дефолту навсегда. На authorize-стороне этот гард — ЕДИНСТВЕННАЯ защита.
//
