...
describe('проба хоста Приорбанка не разошлась с боевым кодом (#522)', () => {
  ...
  it('401 НЕ трактуется как «эндпоинта нет» — это «дай токен»', () => {
    ...
    // ⚠ И 400 с ДОМЕННОЙ ошибкой банка — тоже доказательство наличия API: сервис разобрал
    // запрос и назвал недостающий заголовок. Живой прогон дал именно это
    // (`BY.PRIORBANK.Request.Invalid: Header x-accountConsentId is null`), а проба назвала код
    // «неожиданным» — то есть самый информативный ответ выглядел как сбой.
    expect(SCRIPT).toMatch(/BY\\?\.PRIORBANK/)
  })
