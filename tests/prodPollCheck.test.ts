...
describe('тишина в продлении токенов читается как АВАРИЯ, а не как норма (#504)', () => {
  ...
  it('и на тике С работой — тоже (сводка одна на все исходы)', async () => {
    ...
    const acc = {
      ...
      hasRefresh: true, lastAttemptAt: 0, refreshRejectedAt: 0, consentExpiresAt: 0, pollPaused: false, grantId: ''
    }
    await runBankKeepAlive({
      now: () => NOW,
...
describe('вердикт «КТО продлевает» не имеет права соврать (живой прогон 2026-08-26)', () => {
  ...
  // ⚠ ЖИВОЙ ЗАМЕР 2026-09-09. Вердикт грозил «выключите опрос — и подключение умрёт за срок
  // refresh (~10 ч у Альфы)». С переходом Альфы на ключ API это перестало быть правдой ИМЕННО
  // для неё: продление, получив отказ, выпускает пару заново само — строку
  // «пара выпущена заново ключом API» владелец увидел в том же прогоне, где вердикт грозил
  // смертью. У Приора цена прежняя (ключа нет, лечится входом владельца счёта в банк).
  // ⚠ Диагностика, обещающая смерть тому, кто самозаживает, хуже её отсутствия: она толкает
  // держать опрос включённым «чтобы не умерло», то есть принимать решение по неверной причине.
  it('вердикт РАЗЛИЧАЕТ два банка: Приор умрёт, Альфа переиздаст ключом', () => {
    const start = SCRIPT.indexOf('ОТВЕТ: продлевает ОПРОС')
    expect(start, 'ветка вердикта пропала').toBeGreaterThan(0)
    const branch = SCRIPT.slice(start, SCRIPT.indexOf('elif', start))
    // Оба банка названы поимённо — иначе читатель отнесёт сказанное к своему.
    expect(branch, 'Приор в вердикте не назван').toMatch(/ПРИОР/)
    expect(branch, 'Альфа в вердикте не названа').toMatch(/АЛЬФА/)
    // И названа сама починка: без неё «переживает» звучит как обещание без механизма.
    expect(branch, 'не сказано, ЧЕМ Альфа переживает остановку опроса').toMatch(/ключ/i)
    // ⚠ Оговорка про старые подключения обязательна: переиздание работает только при СОХРАНЁННОМ
    // ключе, а подключение, созданное до перехода, его не имеет и умрёт ровно как раньше.
    expect(branch, 'не оговорены подключения без ключа').toMatch(/старше|переподключ/i)
  })
...
describe('КТО ОТКЛЮЧАЛ БАНК — ШЕСТЬ путей, и все обязаны быть видны (#641)', () => {
  ...
  it('⚠ СПИСОК ПУТЕЙ ЗАКРЫТ: каждое место, стирающее `bank_tokens`, обязано быть учтено', () => {
    ...
    const KNOWN: Record<string, string> = {
      ...
      // ⚠ #654: `worker.ts` ушёл отсюда не потому, что перестал стирать портал, а потому что
      // перестал стирать его САМ — список хранилищ теперь один, в `portalPurge.ts`, и оба
      // вызывающих (`worker.ts` через очередь и `events.post.ts` аварийным путём без Redis)
      // ходят через него. До #654 аварийный путь стирал ОДНУ таблицу из семи, оставляя
      // банковские креды удалённого приложения, и не писал в журнал ничего. Строку пишут
      // по-прежнему вызывающие: повод знает только тот, кто зовёт, — текст даёт
      // `portalPurgeReasonText`. Секция «КТО ОТКЛЮЧАЛ БАНК» ищет их строки, а не этот файл.
      'server/utils/portalPurge.ts': 'ONAPPUNINSTALL и уборщик грантов #574 → «стираем ВСЁ, включая подключения»'
...
describe('ВОРОНКА: вычитаем только гейты ДО распознавания (#646)', () => {
  // ⚠ ИСТОРИЯ ОШИБКИ, и она важнее самих тестов. Первая редакция (#647) вычитала из знаменателя
  // ДЕДУП — и это глушило ВЕРНУЮ тревогу: распознавание в `handleCrmSyncJob` стоит РАНЬШЕ
  // дедуп-гейта, намеренно («run for every unique op (even ones skipped below) so recognition
  // COVERAGE is observable»). На портале со скользящим окном дедуп срабатывает почти всегда, то
  // есть тревога о сломанной карте распознавания гасилась бы каждый тик и вечно, а на экране
  // стояло бы зелёное «карту трогать НЕ надо».
  //
  // Вторая редакция добавила направление и исключения — эти два вычитать НАДО, у них `continue`
  // раньше счётчика `recognized`.

  /** Прогнать блок воронки с подставленной строкой итога. */
  function funnel(summary: string): string {
    const from = SCRIPT.indexOf('section "ВОРОНКА платежа')
    const to = SCRIPT.indexOf('section "Ошибки, упоминающие банк')
    expect(from, 'секция воронки не найдена — её переписали').toBeGreaterThan(0)
    expect(to).toBeGreaterThan(from)
    const block = SCRIPT.slice(from, to)
      .replace('section "ВОРОНКА платежа            (#501)"', '')
      .replace(/sum_line="\$\([^\n]*\)"/, `sum_line=${JSON.stringify(summary)}`)
    return execFileSync('bash', ['-c', 'SINCE=w\n' + block], { encoding: 'utf8' })
  }

  it('⚠ ДЕДУП НЕ ГАСИТ ТРЕВОГУ: распознавание отрабатывает и на уже записанных', () => {
    // Живая строка 2026-08-26. Вчера я объявил эту тревогу ложной и погасил её — ошибочно:
    // операция дошла до распознавания, и ноль там означает «искали и не нашли».
    const out = funnel('[crm-sync] INFO: portal ab12cd34: 1 обработано, 0 создано, 0 приземлилось, '
      + '0 без клиента, 0 без цели, 0 с распознанным номером, 1 уже было записано')
    expect(out, 'верная тревога о карте распознавания снова погашена').toContain('НОМЕР не распознан')
    expect(out).toContain('дошли до разбора     1')
  })

  it('⚠ ЖИВАЯ строка 2026-08-27: направление отсеивает ДО разбора, дедуп — после', () => {
    // `10 обработано, 2 уже было записано, 8 не перенесено по настройке направлений`:
    // 8 не дошли до распознавания, 2 дошли. Знаменатель — 2, а не 0 и не 10.
    const out = funnel('[crm-sync] INFO: portal ab12cd34: 10 обработано, 0 создано, 0 приземлилось, '
      + '0 без клиента, 0 без цели, 0 с распознанным номером, 2 уже было записано, '
      + '8 не перенесено по настройке направлений')
    expect(out).toContain('дошли до разбора     2')
    expect(out).toContain('отсеяно направлением 8')
    // Разбор состоялся на двух операциях и ничего не нашёл — тревога верна.
    expect(out).toContain('НОМЕР не распознан')
  })

  it('всё отсеяно ДО разбора — тревоги нет, и причина названа поимённо', () => {
    const out = funnel('[crm-sync] INFO: portal ab12cd34: 8 обработано, 0 создано, 0 приземлилось, '
      + '0 без клиента, 0 без цели, 0 с распознанным номером, 8 не перенесено по настройке направлений')
    expect(out, 'исправная карта распознавания объявлена ненастроенной').not.toContain('НОМЕР не распознан')
    expect(out).toContain('дошли до разбора     0')
    expect(out).toContain('НАСТРОЙКОЙ НАПРАВЛЕНИЙ')
    expect(out, 'не сказано, что это выбор администратора, а не поломка').toMatch(/ваш выбор/)
  })

  it('правила исключений — тоже гейт до разбора', () => {
    const out = funnel('[crm-sync] INFO: portal ab12cd34: 6 обработано, 0 создано, 0 приземлилось, '
      + '0 без клиента, 0 без цели, 0 с распознанным номером, 6 исключено правилами')
    expect(out).not.toContain('НОМЕР не распознан')
    expect(out).toContain('ПРАВИЛАМИ ИСКЛЮЧЕНИЙ')
  })

  // ⚠ ЖИВОЙ ПРОГОН 2026-09-10: «122 обработано, 0 создано, 0 приземлилось, 122 без клиента».
  // Секция уверенно печатала «они ушли в «мою компанию» (фолбэк)» — утверждение, которого строка
  // итога не доказывает: фолбэк (#91) срабатывает, только если «моя компания» НАЙДЕНА, иначе не
  // пишется ничего. Диагностика отправляла искать в CRM записи, которых там нет.
  it('⚠ «ушли в мою компанию» НЕ утверждается, когда создано ноль', () => {
    const out = funnel('[crm-sync] INFO: portal ab12cd34: 122 обработано, 0 создано, 0 приземлилось, '
      + '122 без клиента, 0 без цели, 0 с распознанным номером')
    expect(out, 'обещан фолбэк, которого не было').not.toContain('ушли в «мою компанию»')
    expect(out).toContain('создано НОЛЬ дел')
    expect(out, 'не сказано, что не записано вообще ничего').toContain('не записано НИЧЕГО')
  })

  it('фолбэк НАЗЫВАЕТСЯ, когда дела действительно создались', () => {
    // Обратная сторона: при живом фолбэке прежний текст был верен, и терять его нельзя —
    // «клиент не опознан» без указания, что записи всё-таки есть, отправляет искать поломку.
    const out = funnel('[crm-sync] INFO: portal ab12cd34: 10 обработано, 10 создано, 0 приземлилось, '
      + '10 без клиента, 0 без цели, 0 с распознанным номером')
    expect(out).toContain('ушли в «мою компанию»')
    expect(out, 'не названо, сколько дел создано').toContain('создано дел: 10')
  })

  it('⚠ ЧИСЛА причин не перепутаны местами', () => {
    // Мутация «поменять местами `$n_dir` и `$n_excl` в printf» проходила зелёной: тест смотрел
    // только на итог. Оператор увидел бы неверную причину и пошёл бы не в тот раздел настроек.
    const out = funnel('[crm-sync] INFO: portal ab12cd34: 20 обработано, 5 создано, 5 приземлилось, '
      + '0 без клиента, 0 без цели, 5 с распознанным номером, 6 уже было записано, '
      + '4 исключено правилами, 5 не перенесено по настройке направлений')
    expect(out).toContain('отсеяно направлением 5')
    expect(out).toContain('отсеяно исключениями 4')
    expect(out, 'дедуп попал в знаменатель — тревога о карте снова гасится').toContain('дошли до разбора     11')
    expect(out).toContain('из них уже записаны  6')
  })

  it('приписок в строке нет вовсе — считаем за ноль, а не за пустоту', () => {
    const out = funnel('[crm-sync] INFO: portal ab12cd34: 5 обработано, 5 создано, 5 приземлилось, '
      + '0 без клиента, 0 без цели, 5 с распознанным номером')
    expect(out).toContain('дошли до разбора     5')
    expect(out).not.toContain('отсеяно направлением')
  })

  it('⚠ РАССОГЛАСОВАННАЯ сводка не уводит в ложную тревогу', () => {
    // Пол `n_new`. Если счётчики когда-нибудь перестанут биться (баг подсчёта, обрезанная докером
    // строка), отрицательный знаменатель уводит в ветку тревоги — то есть скрипт советует чинить
    // исправную настройку ровно тогда, когда сломан он сам.
    const out = funnel('[crm-sync] INFO: portal ab12cd34: 5 обработано, 0 создано, 0 приземлилось, '
      + '0 без клиента, 0 без цели, 0 с распознанным номером, '
      + '4 исключено правилами, 4 не перенесено по настройке направлений')
    expect(out).toContain('дошли до разбора     0')
    expect(out, 'отрицательный знаменатель увёл в тревогу').not.toContain('НОМЕР не распознан')
  })

  it('НОВЫЕ операции без распознанного номера — тревога остаётся, она тут верная', () => {
    const out = funnel('[crm-sync] INFO: portal ab12cd34: 40 обработано, 40 создано, 0 приземлилось, '
      + '0 без клиента, 0 без цели, 0 с распознанным номером')
    expect(out, 'настоящий обрыв распознавания замолчали').toContain('НОМЕР не распознан')
  })

  it('⚠ ФОРМУЛИРОВКИ берутся из НАСТОЯЩЕЙ сводки, а не переписаны сюда', () => {
    // Гард на дрейф: переименуй поле в `opLogPolicy.ts` — и `fld` вернёт пустоту, гейт не
    // вычтется, знаменатель завысится, тревога сработает на исправном портале. Молча.
    const line = runSummaryLine('ab12cd34', {
      processed: 10, created: 0, landed: 0, unmatched: 0, unresolved: 0, recognized: 0,
      skipped: 2, excluded: 3, directionSkipped: 5
    } as never, 'notable')
    for (const needle of ['уже было записано', 'исключено правилами', 'не перенесено по настройке направлений']) {
      expect(line, `формулировка «${needle}» изменилась — скрипт её больше не найдёт`).toContain(needle)
    }
    const out = funnel(`[crm-sync] INFO: ${line}`)
    expect(out, 'гейты не вычлись из НАСТОЯЩЕЙ строки').toContain('дошли до разбора     2')
  })

  it('⚠ СПИСОК ГЕЙТОВ ЗАКРЫТ: каждый `continue` до распознавания обязан вычитаться', () => {
    // ⚠ За два дня я дважды ошибся в этом списке — сначала вычел один гейт из трёх, потом вычел
    // лишний. Скрипт проверить полноту не может: он читает текст лога, а список гейтов живёт в
    // коде. Проверяет её ГРЕП, а не перечисление.
    const HANDLERS = readFileSync(join(ROOT, 'server/queue/handlers.ts'), 'utf8')
    const recAt = HANDLERS.indexOf('recognized++')
    const loopAt = HANDLERS.lastIndexOf('for (const item of', recAt)
    expect(recAt, 'счётчик распознавания исчез').toBeGreaterThan(0)
    expect(loopAt, 'цикл обработки операций не найден').toBeGreaterThan(0)
    // Счётчики, инкрементируемые ДО распознавания, — ровно те, что скрипт обязан вычитать.
    const before = HANDLERS.slice(loopAt, recAt)
    const gates = [...before.matchAll(/^\s*(\w+)\+\+$/gm)].map(m => m[1]!)
    const SUBTRACTED: Record<string, string> = {
      directionSkipped: 'n_dir',
      excluded: 'n_excl'
    }
    for (const g of gates) {
      expect(SUBTRACTED[g], `гейт «${g}» стоит ДО распознавания и не вычитается из знаменателя`).toBeTruthy()
    }
    for (const [gate, v] of Object.entries(SUBTRACTED)) {
      expect(gates, `гейт «${gate}» исчез из кода — уберите ${v} из вычитания`).toContain(gate)
    }
    // ⚠ И обратное: дедуп стоит ПОСЛЕ распознавания, вычитать его НЕЛЬЗЯ. Это и была ошибка #647.
    expect(HANDLERS.indexOf('skipped++'), 'дедуп переехал до распознавания — пересмотрите знаменатель')
      .toBeGreaterThan(recAt)
    expect(SCRIPT, 'дедуп снова вычитается из знаменателя — тревога о карте будет гаснуть')
      .not.toMatch(/n_new=\$\(\([^)]*n_dedup/)
  })
})
