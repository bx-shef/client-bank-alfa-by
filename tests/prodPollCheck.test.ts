...
describe('ВОРОНКА: дедуп — не отказ, и тревожить по нему нельзя (#646)', () => {
  // ⚠ Исправление ЛОЖНОЙ ТРЕВОГИ по живому прогону 2026-08-26. Владелец переподключил банк, опрос
  // пошёл, и первая же строка итога была `1 обработано, 0 создано, …, 0 с распознанным номером,
  // 1 уже было записано`. Воронка ответила красным «НОМЕР не распознан ни в одном назначении» и
  // советом лезть в карту распознавания — при том что операция туда даже не заходила: дедуп-гейт
  // стоит РАНЬШЕ распознавания. Диагностика уверенно отправляла крутить исправную настройку.
  ...
  it('⚠ ЖИВАЯ строка с прода: всё уже записано — это НОРМА, а не «карта не настроена»', () => {
    // Строка приведена дословно (портал заменён на синтетический хеш).
    ...
    expect(out, 'исправная карта распознавания снова объявлена ненастроенной')
      .not.toContain('НОМЕР не распознан')
    expect(out).toContain('уже были записаны прошлыми прогонами')
    expect(out).toContain('до разбора не дошла ни одна')
  })

  it('⚠ ЖИВАЯ строка 2026-08-27: гейт НАПРАВЛЕНИЙ тоже стоит до распознавания', () => {
    // Вчерашняя правка была сделана НАПОЛОВИНУ: дедуп я вычел, а гейт направлений (#44) — нет.
    // Строка с прода `10 обработано, 2 уже было записано, 8 не перенесено по настройке
    // направлений` снова дала красное «НОМЕР не распознан», хотя до распознавания не дошла НИ ОДНА
    // операция. Тот же дефект, вычтенный на один слой.
    ...
    // ⚠ Причина названа ПОИМЕННО: «отсеяно» без указания гейта отправляет искать по всем настройкам.
  ...
  it('⚠ гейты СКЛАДЫВАЮТСЯ, а не заменяют друг друга', () => {
    // Прогон, где часть отсеял дедуп, часть — направление, часть — исключения, и до разбора дошли
    // ровно оставшиеся. Вычти мы только один гейт — знаменатель был бы завышен, и порог сработал
    // бы на исправном портале.
    ...
    expect(out).toMatch(/до разбора дошли 5/)
    expect(out).not.toContain('НОМЕР не распознан')
  })
  ...
  it('⚠ СМЕШАННЫЙ прогон: выводы строятся на НОВЫХ, и это сказано вслух', () => {
    // «40 обработано, 3 создано» без оговорки читается как провал, хотя 37 уже лежат в CRM.
    const out = funnel('[crm-sync] INFO: portal ab12cd34: 40 обработано, 3 создано, 3 приземлилось, '
      + '0 без клиента, 0 без цели, 3 с распознанным номером, 37 уже было записано')
    expect(out).toContain('уже были записаны    37')
    expect(out).toContain('дошли до разбора     3')
    expect(out, 'читатель не предупреждён, что выводы только про дошедшие операции')
      .toMatch(/до разбора дошли 3/)
    expect(out).not.toContain('НОМЕР не распознан')
  })

  it('приписки дедупа в строке нет вовсе — считаем за ноль, а не за пустоту', () => {
    // Приписка условная: при нуле её не печатают. Без явного дефолта арифметика дала бы пустую
    // строку в вычитании и `n_new` сломался бы молча.
    ...
    expect(out).toContain('уже были записаны    0')
    expect(out).toContain('дошли до разбора     5')
  })

  it('⚠ поле дедупа берётся из НАСТОЯЩЕЙ сводки, а не из выдуманной строки', () => {
    // Тот же приём, что у соседей: формулировку задаёт `runSummaryLine`, и разойдись она со
    // скриптом — воронка молча вернулась бы к ложной тревоге.
    const line = runSummaryLine('ab12cd34', {
      processed: 1, created: 0, landed: 0, unmatched: 0, unresolved: 0, recognized: 0, skipped: 1
    } as never, 'notable')
    expect(line, 'формулировка дедупа изменилась — скрипт её больше не найдёт').toContain('уже было записано')
    expect(funnel(`[crm-sync] INFO: ${line}`), 'ложная тревога вернулась на настоящей строке')
      .not.toContain('НОМЕР не распознан')
  })
})
