...
describe('findCandidateByField', () => {
  it('maps a found deal to a candidate and queries by the configured field + companyId', async () => {
    const call = vi.fn(async (_m: string, _p: Record<string, unknown>) => resp([item({ id: 77, opportunity: 0, currencyId: 'BYN', stageId: 'C5:NEW' })]))
  ...
  it('returns null for a malformed field name (fail-safe — no filter-key injection)', async () => {
    const call = vi.fn(async (_m: string, _p: Record<string, unknown>) => resp([item()]))
  ...
  it('returns null for a negative-stage match (same drop as by-id)', async () => {
    const call = vi.fn(async (_m: string, _p: Record<string, unknown>) => resp([item({ stageId: 'C5:LOSE' })]))
...
describe('findCandidateById', () => {
  it('maps a found deal to an AllocationCandidate', async () => {
    const call = vi.fn(async (_m: string, _p: Record<string, unknown>) => resp([item({ id: 33, opportunity: 1200, currencyId: 'BYN' })]))
  ...
  it('populates dealId from parentId2 for an invoice-id target (#229)', async () => {
    const call = vi.fn(async (_m: string, _p: Record<string, unknown>) => resp([item({ id: 9, opportunity: 1200, currencyId: 'BYN', parentId2: 15, stageId: 'DT31_11:N' })]))
  ...
  it('does NOT set dealId for a non-invoice kind (a deal is not its own parent)', async () => {
    const call = vi.fn(async (_m: string, _p: Record<string, unknown>) => resp([item({ id: 33, parentId2: 99 })]))
  ...
  it('returns null when the id belongs to another company (empty items — IDOR)', async () => {
    const call = vi.fn(async (_m: string, _p: Record<string, unknown>) => resp([])) // companyId filter excluded it
  ...
  it('normalizes a non-finite amount to 0 for a trigger kind (deal/smart-process ignore amount)', async () => {
    const call = vi.fn(async (_m: string, _p: Record<string, unknown>) => resp([item({ opportunity: undefined, currencyId: 'BYN' })]))
  ...
  it('threads entityTypeId onto a smart-process candidate (needed as OWNER_TYPE_ID to fire its trigger, #79)', async () => {
    const call = vi.fn(async (_m: string, _p: Record<string, unknown>) => resp([item({ id: 1, opportunity: 0, currencyId: 'BYN' })]))
  ...
  it('does NOT set entityTypeId for a deal (fixed OWNER_TYPE_ID=2) or amount kinds (#79)', async () => {
    const call = vi.fn(async (_m: string, _p: Record<string, unknown>) => resp([item({ id: 33, opportunity: 5, currencyId: 'BYN' })]))
  ...
  it('returns null on a non-finite amount for an amount-gated kind (fail-closed, like invoiceLookup)', async () => {
    const call = vi.fn(async (_m: string, _p: Record<string, unknown>) => resp([item({ opportunity: 'n/a' })]))
  ...
  it('parses a string opportunity (the real crm.item.list shape)', async () => {
    const call = vi.fn(async (_m: string, _p: Record<string, unknown>) => resp([item({ opportunity: '250.0000' })]))
  ...
  it('returns null when the found item has an empty id', async () => {
    const call = vi.fn(async (_m: string, _p: Record<string, unknown>) => resp([item({ id: undefined })]))
  ...
  it('feeds an empty string to the stage predicate when stageId is missing', async () => {
    const seen: string[] = []
    const call = vi.fn(async (_m: string, _p: Record<string, unknown>) => resp([item({ stageId: undefined })]))
