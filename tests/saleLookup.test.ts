...
describe('findOrderPaymentIds', () => {
  it('returns the payment record ids of the order (as strings)', async () => {
    const call = vi.fn(async (_m: string, _p: Record<string, unknown>) => resp([{ id: 5, orderId: 1 }, { id: 8, orderId: 1 }]))
  ...
  it('blank orderId → [] WITHOUT a REST call (an empty filter would list all payments)', async () => {
    const call = vi.fn(async (_m: string, _p: Record<string, unknown>) => resp([{ id: 5, orderId: 1 }]))
  ...
  it('guards against a portal that ignores the orderId filter (echo mismatch dropped)', async () => {
    ...
    const call = vi.fn(async (_m: string, _p: Record<string, unknown>) => resp([{ id: 5, orderId: 1 }, { id: 9, orderId: 2 }]))
    expect(await findOrderPaymentIds('1', call)).toEqual(['5'])
  })

  it('skips rows with a blank/absent id', async () => {
    const call = vi.fn(async (_m: string, _p: Record<string, unknown>) => resp([{ id: '', orderId: 1 }, { orderId: 1 }, { id: 7, orderId: 1 }]))
