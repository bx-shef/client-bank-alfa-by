...
describe('разделы настроек (#530)', () => {
  ...
  it('каждая строка готовности ведёт в раздел, где её и чинят', () => {
    ...
    // ⚠ #46: строка смарт-процессов краснеет ещё и когда СП создан, но полей реестра на нём нет —
    // а чинится это кнопкой «Настроить смарт-процессы» в разделе распределения. Без этой ссылки
    // подсказка «нажмите кнопку» оставляла бы админа искать её по настройкам самому.
    expect(sectionForReadiness('smart-process'), 'смарт-процессы чинит кнопка в разделе распределения').toBe('distribution')
