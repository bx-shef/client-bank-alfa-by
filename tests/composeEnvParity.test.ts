...
//
// ⚠ Проверяются ДВА файла — крон-плагин и воркер, — а не весь `server/`. В них сходятся настройки
// периодических задач и подробность логирования, то есть места, где цена молчаливого дефолта это
// ретенция ПДн, частота обращений к банку и глубина истории логов. Расширять охват на весь сервер
// нельзя: там есть переменные, заведомо задаваемые не через compose (например, ключи дев-скриптов).

const ROOT = join(import.meta.dirname, '..')
// ⚠ Оба файла, а не только крон-плагин: настройки периодических задач живут в одном, а
// подробность логирования — в другом, и ловушка «объявили в документации, забыли в compose» у них
// общая. Ровно на ней споткнулись дважды (#485 и #498).
const PLUGIN = [
  readFileSync(join(ROOT, 'server/plugins/queue.ts'), 'utf8'),
  readFileSync(join(ROOT, 'server/queue/worker.ts'), 'utf8')
].join('\n')
...
describe('переменные крон-плагина и воркера объявлены в docker-compose.prod.yml (#485)', () => {
  ...
  const BAKED_AT_BUILD: Record<string, string> = {
    ...
    NUXT_PUBLIC_COMMIT_SHA: 'baked into the image by Dockerfile ARG/ENV (CI passes github.sha)'
  }
