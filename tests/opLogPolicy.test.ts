...
describe('runSummaryLine', () => {
  ...
  // ⚠ #44: единственный операторский сигнал о том, что половина выписки не поехала по настройке.
  // Считается ОТДЕЛЬНО от «исключено правилами»: там причина «этот плательщик не нужен», здесь
  // «этот вид операций не переносим», и чинятся они в разных разделах настроек — по одному числу
  // нельзя было бы понять, куда идти.
  it('не перенесённые по направлению названы СВОИМ числом, отдельно от исключений — #44', () => {
    const line = runSummaryLine('M1', { ...broken, processed: 7, excluded: 2, directionSkipped: 5, unmatched: 0 }, 'notable')
    expect(line).toContain('5 не перенесено по настройке направлений')
    expect(line).toContain('2 исключено правилами')
  })
  ...
  it('нулевые пояснения НЕ печатаются — постоянный шум перестают читать', () => {
    ...
    expect(line).not.toContain('по настройке направлений')
  })
