...
describe('смарт-процессы приложения в вердикте установки', () => {
  // ⚠ Заведено вместе с автопровижинингом на установке (2026-08-23, решение владельца: режим
  // приложения всегда «включено»). Прежде смарт-процессы создавал ЧЕЛОВЕК кнопкой в настройках за
  // feature-флагом — то есть портал штатно оставался наполовину настроенным, и узнавали об этом по
  // пустому смарт-процессу при живом импорте (#575).
  const base = { finished: true, missingScopes: [] as string[], trigger: 'ok', backend: 'ok' as const }

  it('созданы — вердикт чистый', () => {
    expect(installVerdict({ ...base, smartProcess: 'ok' }).level).toBe('ok')
  })

  it('не создались — degraded с действием, а не тихая диагностика', () => {
    const v = installVerdict({ ...base, smartProcess: 'ошибка: crm.type.add failed' })
    expect(v.level).toBe('degraded')
    expect(v.issues.some(i => i.title.includes('смарт-процессы'))).toBe(true)
    // Действие обязано быть выполнимым из настроек: переустановка здесь ничего не чинит.
    expect(v.issues.find(i => i.title.includes('смарт-процессы'))!.action).toContain('Настроить смарт-процессы')
  })

  it('не пытались — молчим', () => {
    // Так бывает, когда серверная часть ещё не подтвердила установку: звать провижининг нечем, и
