...
describe('#15 пусковая страница главного экрана', () => {
  ...
  it('лаунчер: шапка с заголовком и кнопками «Настройки»/«Загрузить выписку» скрыта (#38)', async () => {
    // Заголовок и кнопки относятся к РАБОЧЕМУ экрану (он открыт слайдером поверх). На пусковой
    // странице они вели бы во второй фрейм и путали бы — экран лаунчера это только путь обратно.
    const wrapper = await mount()
    await flushPromises()
    expect(wrapper.find('[data-testid="app-launcher"]').exists()).toBe(true)
    expect(wrapper.find('button[aria-label="Настройки"]').exists(), 'кнопка настроек не скрыта').toBe(false)
    expect(wrapper.find('button[aria-label="Загрузить выписку"]').exists(), 'кнопка импорта не скрыта').toBe(false)
    expect(wrapper.text(), 'заголовок рабочего экрана виден в лаунчере').not.toContain('Банковские выписки')
    // Сообщение оформлено заголовком экрана (#38).
    expect(wrapper.text()).toContain('Выписки открываются в отдельном окне')
  })
