    ...
    test.describe(`${theme} · ${vp.name}`, () => {
      ...
      for (const route of VISUAL_ROUTES.filter(r => r.themes.includes(theme))) {
        test(route.slug, async ({ page }) => {
          ...
          // ⚠ ЖДЁМ `networkidle`, а не только `load` (#630). Замерено: на `load` гидратация Nuxt
          // могла ещё не отработать, страница была короче вьюпорта, раскатывать было нечего — и
          // проверка «скроллеров не осталось» проходила ВПУСТУЮ, а снимок совпадал со старым
          // обрезанным эталоном. Один прогон из четырёх зеленел именно так. Внешние запросы здесь
          // и так обрываются (`LOCAL_ORIGIN`), поэтому ожидание дешёвое.
          //
          // ⚠ Но держится готовность НЕ на нём, и путать это нельзя: сам Playwright помечает
          // `networkidle` как DISCOURAGED («rely on web assertions instead»), и работает он здесь
          // лишь потому, что монтирование страницы делает запросы, которых он и ждёт. Настоящая
          // гарантия — сверка `scrollers` с закрытым списком `unrolls` ниже: она смотрит на
          // СОДЕРЖИМОЕ и падает на неотрисованной странице. Уберут ожидание — тест не ослабнет
          // молча, а покраснеет.
          ...
          // ⚠ РАСКАТКА ВНУТРЕННИХ СКРОЛЛЕРОВ (#630). `fullPage` снимает ДОКУМЕНТ, а на
          // in-portal-страницах документ не растёт: b24ui-оболочка `B24DashboardGroup` — это
          // `fixed inset-0 flex overflow-hidden`, экран пришпилен к вьюпорту, прокручивается
          // внутренний `flex-1 overflow-y-auto`. Замерено на `/settings?preview=1`: документ 900
          // при содержимом 2118, то есть эталон равнялся первому экрану, а всё ниже — сверка
          // счетов, исключения, авто-проведение, карта распознавания, очистка и сами кнопки
          // Save/Cancel — не проверялось НИ РАЗУ. Джоба при этом была зелёной: ровно тот отказ,
          // который проект держит на прицеле, — проверка выглядит рабочей и не работает.
          //
          // ⚠ Замерено, что правка ХИРУРГИЧЕСКАЯ: из десяти маршрутов раскатка задевает ОДИН
          // (`/settings`), у остальных девяти прокручивается сам документ и раскатывать нечего.
          // Числа — в `docs/VISUAL_VERIFICATION.md`; здесь их не дублируем.
          const unrolled = await page.evaluate(unrollScrollContainers)

          // ⚠ Проверять исход ОБЯЗАТЕЛЬНО, и это не перестраховка. Раскатка ищет скроллеры по
          // вычисленному стилю, а не по классам темы, — но следующая перестройка оболочки всё
          // равно может увести её мимо, и тогда покрытие съёжится обратно до первого экрана
          // МОЛЧА, тем же способом, каким дефект и появился. Пусть падает.
          expect(
            unrolled.left,
            `${route.slug}: остался непрокрученный контейнер — fullPage снимет не всю страницу`
          ).toEqual([])
          // ⚠ И сверяем ФАКТ с ожиданием из `routes.ts`. «Раскатывать было нечего» само по себе
          // ничего не доказывает: ровно так выглядит и страница, которая не успела отрисоваться, —
          // замерено, один прогон из четырёх так и зеленел. Ожидание закрытое: заведут страницу под
          // той же оболочкой и забудут пометить — здесь станет красным.
          expect(
            unrolled.scrollers > 0,
            route.unrolls
              ? `${route.slug}: ожидали внутренний скроллер, его нет — страница не отрисовалась?`
              : `${route.slug}: появился внутренний скроллер — пометьте маршрут \`unrolls: true\``
          ).toBe(route.unrolls)
          // Скроллер можно не только «раскатать», но и потерять: контейнер с `overflow-y: hidden`
          // переполнение не покажет и прокрутить себя не даст. Сегодня таких нет ни на одном
          // маршруте (замерено), поэтому утверждение строгое.
          expect(
            unrolled.clipped,
            `${route.slug}: содержимое обрезано и недостижимо для снимка`
          ).toEqual([])
