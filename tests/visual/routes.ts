...
export const VISUAL_ROUTES: readonly VisualRoute[] = [
  ...
  // ⚠ Экран владельца счёта (#19) — единственный экран приложения не для администратора. Под
  // `?preview=1` фрейм-токена нет, поэтому снимок фиксирует ветку «этот экран открывается только
  // внутри портала»: снаружи её и увидит всякий, кто откроет адрес мимо ссылки из чата, а вёрстку
  // ей больше не проверяет никто. ⚠ Проверено по снимку, а не по коду: первая редакция этого
  // комментария уверяла, что здесь ветка «ссылка неполная» — она наступает ВНУТРИ портала, когда
  // гранта нет, и в эталон не попадает вовсе.
  { slug: 'bank-key', path: '/bank-key?preview=1', themes: ['light', 'dark'], unrolls: false }
]
