...
// ⚠ #43: справка — ЕДИНСТВЕННОЕ место, куда сообщения (карточка дела, чат ошибок, сводка загрузки)
// отсылают за полным порядком действий. Если она разойдётся с ними, человек получит две разные
// инструкции, и одна из них будет невыполнимой — ровно то, с чего задача и началась. Гард держит
// обе несущие вещи: невыполнимого совета нет, а обязательный шаг с удалением дела назван.
describe('справка про «клиент не определён» согласована с сообщениями (#43)', () => {
  const section = FAQ.find(s => s.id === 'my-company')!
  const text = section.answer.join(' ')

  it('не советует перепривязать дело вручную — Bitrix24 этого не даёт (#579)', () => {
    expect(text).not.toMatch(/привяжите (его |её )?вручную/i)
    expect(text).toMatch(/перепривязать .*нельзя|нельзя.*сменить владельца/i)
  })

  it('называет обязательный шаг: удалить дело, иначе дедуп отсеет операцию', () => {
    expect(text).toMatch(/удалит[ьеи] это дело/i)
    expect(text).toMatch(/обязательно/i)
  })

  it('оговаривает, что тип компании на поиск не влияет — ищем по счёту', () => {
    expect(text).toMatch(/на поиск не влияет/i)
    expect(text).toMatch(/только по расчётному счёту/i)
  })
})
