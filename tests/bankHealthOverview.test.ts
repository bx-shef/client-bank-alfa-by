...
describe('раздел «подписка не отвечает» (#614)', () => {
  ...
  it('ожидающее подключение при мёртвой подписке ВИДНО оператору', () => {
    // ⚠ Автоотключение стирает ВСЕ строки портала, включая `~pending:`. Пока отсев ожидающих стоял
    // раньше чтения метки, раздел прятал ровно то состояние, ради которого написан: админ прошёл
    // банк, не успел выбрать счёт, подписка кончилась — и через четверо суток недонастроенное
    // подключение исчезало молча, без обратного отсчёта и без кнопки.
    const p = subRow({ accountKey: provisionalAccountKey('n1') })
    const o = summarizeBankHealth([p], NOW_S, hash, new Map([['M1', NOW_S - 2 * DAY_S]]))
    expect(o.subscriptionDead?.map(c => c.id)).toEqual([1])
    // ⚠ `unknown`, а не `ok`: счёта ещё нет, судить о банковской паре не по чему.
    expect(o.subscriptionDead?.[0]?.health).toBe('unknown')
    // И оно по-прежнему считается ожидающим — это не подмена одного раздела другим.
    expect(o.pending.connections).toBe(1)
  })
