...
describe('контекстная справка через слайдер (#576 п.2)', () => {
  ...
  it('якоря контекстных ссылок в UI существуют в справке', () => {
    ...
    // ⚠ Гард видит только СТАТИЧЕСКИЕ якоря. Динамическая привязка (`:anchor="x"`) молча вывела бы
    // ссылку из-под проверки — то есть ровно тот тихий дефект, ради которого гард и написан.
    // Поэтому она запрещена: понадобится — сначала придумать, чем её проверять.
    const dynamic = files.filter(f => /:anchor=/.test(readFileSync(join(process.cwd(), 'app', f), 'utf8')))
    expect(dynamic, 'динамический якорь справки — гард перестаёт его проверять').toEqual([])
