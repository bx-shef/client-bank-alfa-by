...
describe('совпадение тел на пустом дне (#561)', () => {
  const src = readFileSync(new URL('../scripts/prod-alfa-page-probe.sh', import.meta.url), 'utf8')

  it('вывод «банк проигнорировал pageNo» стоит ПОД условием непустой страницы', () => {
    ...
    const block = /if \[ -n "\$FLAT0" \] && \[ "\$FLAT1" = "\$FLAT0" \]; then([\s\S]*?)\n {4}fi/.exec(src)?.[1] ?? ''
    expect(block, 'блок сравнения тел не найден — проверьте, не переписан ли он').not.toBe('')
    expect(block, 'вывод про игнор pageNo снова безусловен').toContain('if [ "$P0" = "0" ]')
    // И на пустом дне человек всё равно получает объяснение, а не молчание.
    expect(block).toContain('сравнивать нечего')
  })
})
