...
describe('#488 проба использует токен после обновления — только Альфа', () => {
  // ⚠ Зачем вызов вообще. Замер 8 сентября: 19 обменов по полчаса проходят, 20-й (10 ч 03 мин от
  // подключения) отвергается — потолок сессии в 10 часов настоящий. Но та лестница НИ РАЗУ не
  // обращалась к API. В августе, когда подключение прожило шестеро суток, опрос ходил за выпиской
  // каждые пять минут. Различие ровно одно, и оно и есть новый вопрос: продлевает ли сессию
  // ИСПОЛЬЗОВАНИЕ токена.
  const src = readFileSync(PROBE, 'utf8')

  it('вызов идёт ПОСЛЕ успешного обмена и только у Альфы', () => {
    const ok = src.indexOf('VERDICT|ok')
    const call = src.indexOf('ИСПОЛЬЗУЕМ ТОКЕН (')
    expect(call).toBeGreaterThan(ok)
    expect(src.slice(ok, call)).toMatch(/if \(P === 'alfa'\)/)
  })

  it('НОМЕРА СЧЕТОВ не печатаются — только код ответа и количество', () => {
    // Вывод пробы копируют в переписку, а номер счёта — персональные данные. Печатать разрешено
    // ровно две вещи, и тело ответа среди них не значится.
    const block = src.slice(src.indexOf('ИСПОЛЬЗУЕМ ТОКЕН ('), src.indexOf('const keep ='))
    expect(block).toMatch(/счетов в ответе/)
    expect(block).toMatch(/body\.accounts\.length/)
    expect(block, 'в вывод уходит тело ответа банка').not.toMatch(/console\.log\([^)]*\bbody\b(?!\.accounts)/)
    expect(block, 'номера счетов попадают в вывод').not.toMatch(/accounts\[|\.number/)
  })

  it('исход вызова не меняет вердикт ступени', () => {
    // Лестница отвечает на вопрос «принимает ли банк обмен». Свяжи мы с ним отказ API — получили
    // бы «банк не терпит паузу» там, где обмен как раз прошёл.
    const block = src.slice(src.indexOf('ИСПОЛЬЗУЕМ ТОКЕН ('), src.indexOf('const keep ='))
