...
import { splitCsvLine } from '~/utils/csvLine'
...
describe('фикстуры CSV — синтетика, а не выписки клиента', () => {
   ...
   * названия контрагентов. Гард смотрит на сами файлы, а не на намерение.
   *
   * ⚠ Первая редакция этого гарда НЕ РАБОТАЛА, и это замерено ревью мутацией: файл с чужим
   * названием, УНП, телефоном и счётом проходил зелёным. Проверялась только форма счёта — маской
   * по СЫРОМУ тексту, которая не переживает пробелы, а Приорбанк пишет счёт контрагента именно с
   * ними (1 найденный счёт из 3, и невидимы были ровно счета контрагентов), — плюс чёрный список
   * из семи слов, то есть ровно тех данных, которые уже утекали. Проверка теперь ОДНА на оба
   * гарда (`assertSyntheticStatement`), потому что две копии разошлись немедленно.
   */
  it('ни одна фикстура не похожа на настоящую выписку', () => {
    const files = readdirSync(DIR)
    expect(files.length).toBeGreaterThan(0)
    for (const name of files) assertSyntheticStatement(name, text(name), expect)
  })
})
...
describe('CSV Приорбанка', () => {
  ...
  it('расхождение оборотов — отказ, а не частичный импорт', () => {
    ...
    expect(() => parsePriorCsv(body)).toThrow(/не сошлась с оборотами/i)
  })
  ...
  /**
   * ⚠ Третье число в строке оборотов (эквивалент в BYN у валютного счёта, добавленная банком
   * колонка) — это ДРУГАЯ РАСКЛАДКА, а не обрезанный файл. Прежний код молча не присваивал итог и
   * отвечал «в выписке нет строки Обороты… выгрузите заново» — совет, который не помог бы никогда,
   * потому что файл выгрузится точно так же. Найдено ревью.
   */
  it('неожиданная форма строки оборотов называет СВОЮ причину', () => {
    const body = text('prior-byn.csv').replace('Обороты ;;;;;325,78;2 330,00;', 'Обороты ;;;;;325,78;2 330,00;1 000,00;')
    expect(() => parsePriorCsv(body)).toThrow(/ожидались две суммы/i)
  })

  /**
   * ⚠ Признак операции — дата в первой ячейке, и «у сальдо там всегда текст» верно ровно для
   * замеренного файла. Страховка: строка со словом «сальдо» операцией не считается НИКОГДА. Иначе
   * её суммы ушли бы в сверку, та не сошлась бы, и исправный файл был бы отвергнут с ложным
   * «выгрузился не до конца».
   */
  it('строка сальдо не становится операцией, даже если начинается с даты', () => {
    // ⚠ Суммы сальдо ставим РОВНО в колонки дебета и кредита — иначе мутация «снять страховку» не
    // проявляется, и тест зелен при любой реализации (первая редакция была именно такой,
    // поймано собственной мутацией).
    const body = text('prior-byn.csv')
      .replace('Входящее сальдо  на  04.08.2026;;;;;;;0,00;185,98;;',
        '04.08.2026;;;;;;Входящее сальдо;100,00;0,00;;')
    const { items } = normalizePriorCsvRows(parsePriorCsv(body), CTX)
    expect(items).toHaveLength(4)
  })

  it('«сальдо» в назначении НЕ выбрасывает операцию', () => {
    // Обратная сторона страховки: слово встречается в тексте плательщика, и проверка по всей
    // строке съедала бы настоящие платежи.
    const body = text('prior-byn.csv').replace('ПОПОЛНЕНИЕ КОРПОРАТИВНОЙ КАРТЫ;;', 'ОПЛАТА САЛЬДО ПО ДОГОВОРУ;;')
    const { items } = normalizePriorCsvRows(parsePriorCsv(body), CTX)
    expect(items).toHaveLength(4)
    expect(items[0]!.purpose).toBe('ОПЛАТА САЛЬДО ПО ДОГОВОРУ')
  })
...
describe('точка с запятой внутри поля (#707, находка ревью)', () => {
  /**
   * ⚠ Назначение платежа пишет ПЛАТЕЛЬЩИК, а разделитель — `;`. Замерено на голом `split`:
   * «ОПЛАТА; СЧЕТ 5» превращалось в «ОПЛАТА», хвост исчезал МОЛЧА — сверка оборотов такой случай
   * не ловит по построению (суммы стоят левее назначения). Цена — не косметика: в хвосте живёт
   * номер счёта, по которому работает распознавание, и его пропажа всплывает в другом месте
   * («цель не найдена»), где связать её с разделителем некому.
   */
  it('назначение с точкой с запятой доезжает целиком — Приорбанк', () => {
    const body = text('prior-byn.csv').replace('ПОПОЛНЕНИЕ КОРПОРАТИВНОЙ КАРТЫ;;', 'ОПЛАТА; СЧЕТ 5 ОТ 01.08.2026;;')
    const { items } = normalizePriorCsvRows(parsePriorCsv(body), CTX)
    expect(items[0]!.purpose).toBe('ОПЛАТА; СЧЕТ 5 ОТ 01.08.2026')
  })

  it('назначение с точкой с запятой доезжает целиком — Альфа', () => {
    const body = text('alfa-byn.csv').replace('OTHR 121101 ОПЛАТА ПО СЧЕТУ 38/52 ОТ 01.09.2026;', 'ОПЛАТА; СЧЕТУ 38/52;')
    const { items } = normalizeAlfaCsvRows(parseAlfaCsv(body), CTX)
    expect(items.find(i => i.direction === 'credit')!.purpose).toBe('ОПЛАТА; СЧЕТУ 38/52')
  })

  it('поле в кавычках может содержать разделитель', () => {
    expect(splitCsvLine('a;"б;в";г')).toEqual(['a', 'б;в', 'г'])
    // `""` внутри поля — экранированная кавычка.
    expect(splitCsvLine('a;"ООО ""ДЕМО"";x";b')).toEqual(['a', 'ООО "ДЕМО";x', 'b'])
    // ⚠ Кавычка ПОСЕРЕДИНЕ поля — обычный символ имени, а не экранирование: иначе строка
    // «ООО "ДЕМО" СЕРВИС;100,00» схлопнулась бы в одну ячейку и сумма исчезла бы.
    expect(splitCsvLine('ООО "ДЕМО" СЕРВИС;100,00')).toEqual(['ООО "ДЕМО" СЕРВИС', '100,00'])
  })
})
...
describe('CSV Альфа-Банка', () => {
  ...
  it('расхождение оборотов — отказ', () => {
    ...
    expect(() => parseAlfaCsv(body)).toThrow(/не сошлась с оборотами/i)
  })
