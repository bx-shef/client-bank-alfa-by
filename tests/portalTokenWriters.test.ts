...
describe('писатели portal_tokens классифицированы', () => {
  it('список писателей ЗАКРЫТ — новый обязан получить причину', () => {
    ...
    const CLASSIFIED: Record<string, string> = {
      ...
      // ⚠ Подписка на REST истекла (#614). Пишет РОВНО ОДНУ колонку `subscription_ended_at` и
      // только когда она пуста: отсчёт от ПЕРВОГО отказа, иначе срок отодвигался бы каждым
      // отказом и отключение не наступило бы никогда. `updated_at` не трогает — по нему
      // выбираются порталы для продления (#175).
      markSubscriptionEnded: 'update-only-one-column',
      // ⚠ Обратная ей, и в отличие от снятия метки мёртвого гранта она ОТДЕЛЬНАЯ, а не вшита в
      // запись токена. Причина в том, что сигналы разные: грант оживает вместе с новой парой
      // токенов (одна запись строки), а подписка — просто удачным вызовом REST, при котором
      // токен не меняется и писать его незачем. Условие `<> 0` защищает от UPDATE на каждый
      // прогон каждого живого портала.
      clearSubscriptionEnded: 'update-only-one-column'
