...
say "Канал оповещений оператору"
# ⚠ Канал молчит ОДИНАКОВО в трёх случаях: всё хорошо, он выключен, он включён и не доходит.
# Здесь различаем первые два — по НАЛИЧИЮ переменных, без значений. Третий по переменным не
# определить в принципе, на него есть `make alert-test`: отвечает сам Telegram.
#
# ⚠ Половина настройки — ОТДЕЛЬНАЯ строка, а не «одна из двух не задана»: приложение канал в таком
# виде не включает вовсе, то есть выглядит настроенным и роняет каждую тревогу молча. Ровно этим
# и опасен канал, который зовут ради того, чтобы узнать о поломке без человека.
tg_tok=$($DC exec -T backend sh -c 'printf %s "${TELEGRAM_ALERT_BOT_TOKEN:-}" | wc -c' 2>/dev/null | tr -d '[:space:]')
tg_chat=$($DC exec -T backend sh -c 'printf %s "${TELEGRAM_ALERT_CHAT_ID:-}" | wc -c' 2>/dev/null | tr -d '[:space:]')
if [ "${tg_tok:-0}" -gt 0 ] 2>/dev/null && [ "${tg_chat:-0}" -gt 0 ] 2>/dev/null; then
  ok "оповещения настроены (токен $tg_tok симв., чат $tg_chat симв.) — доставку проверяет 'make alert-test'"
elif [ "${tg_tok:-0}" -gt 0 ] 2>/dev/null || [ "${tg_chat:-0}" -gt 0 ] 2>/dev/null; then
  bad "оповещения настроены НАПОЛОВИНУ — канал выключен, каждая тревога теряется молча"
else
  warn "оповещения выключены: о падении очередей и смерти подключений банка никто не сообщит"
fi
