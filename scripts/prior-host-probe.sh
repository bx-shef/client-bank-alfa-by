...
if [ -z "$ACCESS" ]; then
  ...
  case "$ACC_CODE" in
    ...
    400)
      # ⚠ 400 здесь — ХОРОШИЙ ответ, и это не натяжка. Живой прогон дал
      # `BY.PRIORBANK.Request.Invalid: Header x-accountConsentId is null` — то есть API не просто
      # существует, он РАЗОБРАЛ наш запрос, проверил заголовки и назвал недостающий. Так отвечает
      # работающий сервис, а не отсутствующий маршрут. Отличаем доменную ошибку банка от общей
      # ошибки шлюза: первая доказывает наличие API, вторая ничего не доказывает.
      if grep -q 'BY\.PRIORBANK\|accountConsentId\|openbanking_intent_id' "$ACC_FILE"; then
        ok "ресурсный API НА ЭТОМ ХОСТЕ ЕСТЬ (разобрал запрос и назвал недостающий заголовок согласия)"
      else
        warn "400, но без доменной ошибки банка — смотрим тело"
      fi
      ;;
...
if [ -z "$ACCESS" ]; then
  ...
  # ⚠ ФОРМАТ — `yyyy-MM-dd`, и это ЗАМЕР, а не вкус: на полной метке времени банк отвечает
  # `BY.NBRB.Field.Invalid: must match yyyy-MM-dd` (2026-09-10). Проба слала метку с самого начала
  # и расходилась с боевым кодом, который всегда слал дату, — то есть первый же осмысленный вопрос
  # («сколько банк даёт?») она отбила собственной ошибкой, а выглядело это как отказ банка.
  EXP="$(date -u -d "+$CONSENT_DAYS day" +%Y-%m-%d 2>/dev/null || date -u -v+"$CONSENT_DAYS"d +%Y-%m-%d)"
  ...
  if [ "$CONS_CODE" = "201" ] || [ "$CONS_CODE" = "200" ]; then
    ...
    if [ -n "$GOT_EXP" ]; then
      ...
      elif [ "${GOT_EXP%%T*}" = "$EXP" ]; then
        ok "срок принят как есть — банк терпит запрошенную длительность"
      else
        echo "  ⚠ банк УРЕЗАЛ срок: просили $EXP, дал ${GOT_EXP%%T*} — это и есть его потолок."
      fi
    else
