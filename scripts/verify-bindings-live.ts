...
/** id из конверта `{result: {item?: {id}} | id}` — формы у разных методов разные. */
...
async function main() {
  ...
  const planned = planActivityBindings({
    ...
    refs: [
      ...
      companyRef(clientId) // владелец — обязан быть отсеян
    ]
  })
  ...
  // ⚠ Портал отвечает на повторную привязку ОШИБКОЙ (`ACTIVITY_IS_ALREADY_BOUND`), поэтому «не
  // отказал» здесь доказывает не идемпотентность метода, а работу чтения `binding.list` в
  // транспорте: он обязан увидеть уже стоящие пары и не звать `binding.add` второй раз.
  ...
  if (!keep) {
    ...
    await drop('crm.activity.delete', { id: Number(activityId) })
    await drop('crm.item.delete', { entityTypeId: sp.entityTypeId, id: Number(elementId) })
