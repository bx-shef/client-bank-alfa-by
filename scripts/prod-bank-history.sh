...
while IFS='|' read -r portal prov acct ok_at ok_h try_at try_h cause paused consent health selfheal shared; do
  ...
  # ⚠ ПРИОР + СЧЁТ ЕЩЁ ГДЕ-ТО = НАЗЫВАЕМ ПРИЧИНУ ПРЯМО (замер ночи 2026-09-11). Портал,
  # авторизовавшийся первым, к утру мёртв: последняя удачная пара 11:23, попытка 03:04 —
  # `invalid_grant: Persisted access token data not found`, заборов за 12 часов ноль. Сосед,
  # авторизовавшийся минутой позже, работает. Значит у Приора выживает ПОСЛЕДНИЙ авторизовавшийся.
  # ⚠ Совет здесь ПРОТИВОПОЛОЖЕН обычному: переподключение вылечит эту строку и убьёт соседа, то
  # есть превратится в пинг-понг. Лечится выбором одного портала, а не походом в интернет-банк.
  # ⚠ Версия «у Приора свой потолок refresh» отвергнута: до второго портала подключение
  # продлевалось кроном месяц.
  if [ "$prov" = "prior-by" ] && [ "$shared" = "shared" ] && [ "$cause" = "bank-refused" ]; then
    echo "   ⇒ ⚠ ЭТОТ СЧЁТ ПОДКЛЮЧЁН ЕЩЁ С ДРУГОГО ПОРТАЛА, и это причина отказа."
    echo "     У Приора выживает подключение, авторизованное ПОСЛЕДНИМ: банк роняет прежний грант,"
    echo "     и тот умирает на первом же продлении — через часы, поэтому выглядит как «само»."
    echo "     Лечится ОДНИМ портала на счёт. Переподключать эту строку НЕ НАДО: она оживёт, а"
    echo "     сосед умрёт — и так по кругу."
    echo
    continue
  fi
