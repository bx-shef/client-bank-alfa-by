...
echo
# ⚠ Два инструмента печатают портал ПО-РАЗНОМУ, и на живом разборе 2026-09-10 это стоило времени:
# здесь md5-отпечаток, а `[fetch]` в логе печатает сам `member_id`. Связать их можно, но догадаться
# — нет, поэтому говорим как.
echo "Сопоставить строку с логом: в '[fetch] … portal <member_id>' портал напечатан целиком,"
echo "  а здесь — его отпечаток. Перевести:  printf '%s' <member_id> | md5sum | cut -c1-6"
echo
