...
# ⚠ Метка времени — В UTC, и это сказано вслух. База рендерит `updated_at` в своём поясе (UTC), а
# лестница пишет свои строки локальным временем сервера (UTC+3). В одном журнале это давало разрыв
# в три часа, который читается как сбой: «последняя удачная 17:09» под строкой «обновляю в 20:39».
# Переводить в локальное нельзя — пояс базы и пояс шелла не обязаны совпадать и на другом стенде
# разъедутся молча. Поэтому не гадаем, а подписываем.
echo "последняя удачная   : $ok_at UTC  ($ok_min мин назад)"
