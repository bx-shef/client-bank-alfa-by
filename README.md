...
> и настройки чата (`app.option`). Транспорт **онлайн-опроса** Альфы собран, покрыт юнит-тестами (A5,
> `server/utils/bankFetch.ts`), **подключён к воркеру** (A9), есть **реестр счетов (A6) и крон-таймер
> опроса (A10)** — вся машинерия собрана (таймер default-OFF до rate-limiter A8; нужны банк-креды +
> `ALFA_OAUTH_API_BASE`); Приор (async) — A5b. Дальше — **connect-поток OAuth банков** (подключит реальные
> счета), rate-limiter, чат-уведомления в бою и MCP.
