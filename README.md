...
> `server/utils/bankFetch.ts`) и **подключён к воркеру** (A9) — реальный счёт с банк-токеном опрашивается
> вживую (нужны реальные банк-креды + `ALFA_OAUTH_API_BASE`); Приор (async create+poll) — A5b. Дальше —
> реестр счетов + connect-поток OAuth банков, живой крон-таймер, чат-уведомления в бою и MCP.
