...
> и настройки чата (`app.option`). Транспорт **онлайн-опроса** Альфы собран и покрыт юнит-тестами (A5,
> `server/utils/bankFetch.ts`), ещё **не подключён к воркеру** (A9) — нужны реальные банк-креды +
> `ALFA_OAUTH_API_BASE`; Приор (async create+poll) — A5b. Дальше — свап заглушки воркера, живые
