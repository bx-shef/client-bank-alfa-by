 ...
 * ссылке `/marketplace/view/<код приложения>/` (#19, второй, ПАРАЛЛЕЛЬНЫЙ вход в экраны).
 *
 * ⚠ Это часть РЕГИСТРАЦИИ на портале клиента, а не просто путь: обработчик у точки один, и сменить
 * его повторным `placement.bind` нельзя — сначала `placement.unbind` (порядок в
 * `docs/APP_LINKS.md`). Пока этого не сделали, портал продолжает открывать ПРЕЖНИЙ адрес, и
 * снаружи свежий выкат выглядит как «ссылка ведёт не туда». Менять только вместе с процедурой.
 */
export const APP_URI_HANDLER_PATH = '/open'

/** Имя параметра ссылки, которым выбирается экран: `?params[place]=app-import`.
 *
 *  ⚠ То же самое `place`, что у кнопок внутри приложения, и это НАМЕРЕННО: таблица
 *  `sliderRouteForPlace` одна на оба входа, поэтому новый экран становится доступен ссылкой сам,
 *  без второй таблицы, которая однажды разошлась бы с первой. ⚠ Значение приходит ИЗВНЕ (ссылку
 *  может составить любой, кто пишет в чат портала), поэтому оно сверяется с закрытым списком —
 *  неизвестное открывает главный экран, а не подставляется в адрес. */
export const APP_URI_PLACE_PARAM = 'place'
