...
<template>
  ...
  >
    ...
    <!-- ⚠ «НИЧЕГО НЕ ПОДКЛЮЧЕНО» — УТВЕРЖДЕНИЕ О ФАКТЕ, и произносить его после ОТКАЗА чтения
         нельзя. Живой экран 2026-09-09: красная плашка «слишком много запросов подряд» и прямо под
         ней «Пока ничего не подключено» — при двух рабочих подключениях Альфы. Список пуст не
         потому, что подключений нет, а потому что мы не смогли спросить; `loaded` истинно и на
         неудачном чтении (он значит «проверка завершилась», а не «данные настоящие»).
         Человек, поверивший этой строке, пойдёт подключать банк заново — то есть тратить поход
         владельца счёта в интернет-банк на подключение, которое уже есть. -->
    <p
      v-if="loaded && !error && !accounts.length"
      class="text-sm text-(--ui-color-base-3)"
