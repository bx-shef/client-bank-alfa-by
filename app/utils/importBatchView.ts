...
export function summaryMessage(results: ImportBatchResult[]): string {
  ...
  if (t.unmatched) {
    ...
    text += ` По ${t.unmatched} ${w} не нашли компанию по счёту плательщика — дела записаны в вашу компанию. Чтобы платежи встали к контрагентам, заведите их в CRM со счётом в реквизитах, удалите эти дела и элементы смарт-процесса «Платежи», затем повторите импорт (порядок — в справке).`
  }
  return text
}
