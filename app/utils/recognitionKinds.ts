 ...
 * ни первую тысячу счетов, ни пятизначные. И наоборот — номер БЕЗ префикса в пресеты не входит
 * вовсе: такая маска цепляет из назначения год (`2026`) и сумму (`1500`), а каждый ложный номер
 * теперь ещё и порождает сообщение «цель не найдена». Кому нужен номер без префикса — добавит
 * строку руками, осознанно.
 */
export const MATRIX_PRESETS: ReadonlyArray<MatchMatrix> = [
  ...
  { mask: 'd+/d+', kind: 'order-number', note: 'Номер оплаты заказа вида «заказ/порядковый» (1/1, 12/3).' },
