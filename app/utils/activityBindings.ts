...
/**
 * Порог, с которого начинаются ДИНАМИЧЕСКИЕ типы (смарт-процессы). Системные сущности живут ниже:
 * лид 1, сделка 2, контакт 3, компания 4, предложение 7, счёт 31.
 */
export const DYNAMIC_ENTITY_TYPE_MIN = 128

/** Динамический ли тип — то есть смарт-процесс, а не системная сущность CRM. */
export function isDynamicEntityType(entityTypeId: number): boolean {
  return entityTypeId >= DYNAMIC_ENTITY_TYPE_MIN
}
 ...
 */
/**
 * ⚠ ПРИВЯЗКА СМАРТ-ПРОЦЕССА ПЕРЕХВАТЫВАЕТ ВЛАДЕЛЬЦА ДЕЛА — замерено на живом портале 2026-08-23.
 *
 * Мы создаём дело владельцем-КОМПАНИЕЙ (`ownerTypeId: 4`), и до появления привязок так и было:
 * дела с ID 1303+ читаются как `OWNER_TYPE_ID: '4'`. У дел, записанных ПОСЛЕ того, как к ним стал
 * привязываться элемент реестра платежей, `crm.activity.get` отдаёт `OWNER_TYPE_ID: '1038'` и
 * `OWNER_ID` элемента — при том, что компания в привязках тоже есть. Наружу это выглядит так:
 * в списке дел колонки «Клиент» и «Сделка/Лид» показывают «Импорт выписки: платежи #45» вместо
 * плательщика, то есть основной рабочий экран бухгалтера врёт о том, чей это платёж.
 *
 * ⚠ Обратно владельца НЕ вернуть: `crm.activity.update` с `OWNER_TYPE_ID`/`OWNER_ID` отвечает
 * «Fields is not specified» (проверено и JSON-, и form-кодировкой). То есть чинить постфактум
 * нечем — можно только не допускать.
 *
 * Поэтому динамические типы в привязки НЕ ПОПАДАЮТ. Цена названа честно: из дела больше нельзя
 * одним кликом перейти в элемент реестра или в смарт-процесс списания. Связь при этом не теряется —
 * элемент реестра несёт поля платежа и компанию, а дело остаётся в таймлайне клиента, потому что
 * привязка компании никуда не делась.
 */
export function planActivityBindings(opts: {
  ...
  for (const ref of opts.refs) {
    if (!ref) continue
    // ⚠ Отбор стоит ЗДЕСЬ, а не у вызывающего: правило про перехват владельца должно быть записано
    // один раз и действовать на любой источник ссылок — и на реестр, и на цели списания, среди
    // которых смарт-процессы тоже бывают.
    if (isDynamicEntityType(ref.entityTypeId)) continue
