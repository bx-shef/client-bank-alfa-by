...
/**
 * Срок жизни ACCESS-токена, секунды. Документирован банком И подтверждён живым ответом
 * (`expires_in: 3600` в каждом обмене).
 *
 * ⚠ Именно он ограничивает продление, а не срок refresh (#488). Замер 2026-09-06: обмен
 * `grant_type=refresh_token` проходит, пока access жив, и отвергается через три секунды после его
 * истечения — `400 invalid_grant: User session not alive`. Отсюда каденция продления считается от
 * ЭТОГО числа.
 *
 * ⚠ Значение per-строку берётся из хранимого `expires_at`, а не отсюда: банк волен сменить
 * `expires_in`. Константа нужна только для вывода ПОТОЛКА каденции, где нужно число на этапе
 * сборки, а не значение конкретной строки.
 */
export const ALFA_ACCESS_TOKEN_TTL_SEC = 3600

/**
 * Срок жизни REFRESH-токена по документации Альфы (~10 ч). В ответе не возвращается.
 *
 * ⚠ НА ПРАКТИКЕ НЕДОСТИЖИМ. Документация обещает 36000 с и предлагает обменивать refresh ПОСЛЕ
 * истечения access — измерено, что именно тогда обмен и отвергается (#488). Число оставлено как
 * документированное значение банка и как грубый потолок «мертво наверняка», но каденцию продления
 * по нему считать НЕЛЬЗЯ: пять часов, выведенные из него, и убивали подключения.
 */
