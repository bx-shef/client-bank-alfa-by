...
/**
 * Сообщение «настройка распознавания неверна» (#572).
 *
 * ⚠ До этого случай был НЕОТЛИЧИМ от сбоя портала. Имя поля для поиска сделки и номер
 * смарт-процесса задаёт администратор в «карте распознавания»; ошибся — портал отвечает отказом на
 * КАЖДУЮ операцию с распознанным номером. Ошибка летела наверх и роняла джобу, BullMQ повторял её
 * трижды с тем же исходом, а в логе это выглядело как отказ транспорта.
 *
 * ⚠ Сырой текст портала СЮДА НЕ ПОПАДАЕТ, и это правка по ревью. Первая редакция вставляла его
 * целиком — вместе с внутренним ключом вида (`deal-field`) и английским «Invalid filter: field
 * 'UF_CRM_NOPE' is not allowed in filter». Читает это сообщение БУХГАЛТЕР клиента, а не
 * разработчик: соседние билдеры в этом файле говорят по-русски и просят о действии, которое человек
 * может выполнить сам. Внутренние коды остаются в логе сервера, где их и ищут.
 *
 * ⚠ По той же причине адресат действия — АДМИНИСТРАТОР: настройки admin-only, и просить бухгалтера
 * «проверьте карту распознавания» значит отправить его туда, куда его не пустят.
 *
 * ⚠ Причина названа осторожно. `INVALID_ARG_VALUE` — общий код «неверный аргумент», а не
 * специальный «нет такого поля» (`portalError.ts`), поэтому «портал не принял» честнее, чем
 * «поля не существует»: второе иногда было бы уверенной неправдой.
 */
export function buildSettingsErrorMessage(reason: string): string | null {
  const parsed = parseSettingsReason(reason)
  if (!parsed) return null
  return [
    '[b]⚠️ Платежи не привязываются: настройка приложения не подходит порталу[/b]',
    `Приложение искало, к чему привязать платёж, и портал не принял запрос — ${parsed.what}.`,
    `Что делать: администратору портала открыть настройки приложения, раздел «Карта распознавания», и проверить ${parsed.fix}.`,
    'Пока это не исправлено, платежи записываются, но без привязки к счетам и сделкам.'
  ].join('\n')
}

/** Что именно портал не принял и что смотреть — по виду настройки, а не по тексту портала. */
...
/**
 * Разобрать структурную причину `<вид>|<field|entity>|<текст портала>` из `resolveConfigured`.
 *
 * ⚠ Разбираем СВОЮ строку, а не портальную: формат задаём мы, и он не зависит от языка портала.
 * Неизвестный/битый вход ⇒ `null` ⇒ сообщения нет: сказать «что-то не так с настройками» без
 * указания раздела значит потратить внимание человека впустую.
 */
function parseSettingsReason(reason: string): SettingsProblem | null {
  const [, param] = reason.split('|')
  if (param === 'field') {
    return {
      what: 'в CRM нет поля, указанного для поиска',
      fix: 'имя поля сделки или смарт-процесса — оно должно совпадать с заведённым в вашей CRM'
    }
  }
  if (param === 'entity') {
    return {
      what: 'указанный смарт-процесс не найден или не подходит',
      fix: 'номер смарт-процесса (поле «smart-entity») — возможно, он удалён или указан неверно'
    }
  }
  return null
}
