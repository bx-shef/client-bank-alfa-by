...
export function buildUnmatchedSummaryMessage(s: UnmatchedSummary): string | null {
  ...
  if (accounts.length > 0) {
    ...
    parts.push(rest > 0
      ? `${head}, первые ${shown.length}: ${shown.join(', ')} (остальные ${rest} не показаны).`
      : `${head}: ${shown.join(', ')}.`)
    parts.push('Заведите их в CRM компаниями с этим счётом в реквизитах — порядок в справке приложения.')
  }
  return parts.join(' ')
}
