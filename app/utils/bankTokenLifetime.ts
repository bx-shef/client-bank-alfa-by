 ...
 *
 * ⚠ Различение держится на ПОРЯДКЕ двух записей, и это не совпадение, а инвариант ОБОИХ путей
 * продления: отметка попытки ставится ДО похода в банк, а `updated_at` (он же `connectedAt`) —
 * только после успеха. Значит после удачи `lastAttemptAt < connectedAt`, а любая попытка после неё
 * двигает отметку вперёд. Отсюда правило ниже.
 *
 * ⚠ ПУТЕЙ ИМЕННО ДВА, и второй пришлось чинить ради этой функции (находка ревью). Отметку ставил
 * только крон продления (`runBankKeepAlive`), а опрос обновлял токен по дороге МОЛЧА
 * (`ensureBankToken`) — то есть отказ банка, полученный по пути опроса, выглядел как «никто не
 * пробовал», и карточка уверенно советовала НЕ переподключать ровно там, где переподключение и
 * было лекарством. Зеркальная копия дефекта, ради которого всё писалось. Теперь отметку ставят оба
 * (`BankRefreshDeps.markAttempt`), и «ни разу не пытались» означает то, что говорит.
 *
 * ⚠ Ноль в `lastAttemptAt` — это «отметки нет», а не «попытка в эпоху». Строка живёт с
 * `DEFAULT 0`, поэтому подключение, которого продление ещё ни разу не касалось, честно читается
 * как `never-tried`.
 */
...
/**
 * ⚠ Текст НЕ выдаёт переподключение за починку: здесь оно вернёт зелёный бейдж на один срок жизни
 * токена, а потом всё повторится. Но и не запрещает — импорт бывает нужен сегодня.
 *
 * ⚠ ЧИТАЕТ ЭТО АДМИН ПОРТАЛА КЛИЕНТА, а не мы (замечание ревью безопасности). Прежняя редакция
 * посылала его в `make poll-check` — команду на НАШЕМ сервере, к которому у него нет и не будет
 * доступа: совет, невыполнимый по построению, плюс лишняя подробность о нашей инфраструктуре.
 * Поэтому подсказка называет, что происходит, и оставляет человеку то, что он может сделать сам.
 *
 * ⚠ «Похоже», а не «значит»: отметку теперь ставят оба пути продления, но остаётся узкое окно —
 * строка, до которой продление ещё не добралось ни разу. Утверждать в нём «дело не в банке» было
 * бы тем же перебором уверенности, от которого лечится вся эта задача.
 */
export const EXPIRED_NEVER_TRIED_HINT
  = 'Срок обновления вышел, но приложение НИ РАЗУ не пыталось продлить это подключение с момента '
    + 'последнего успеха. Похоже, дело не в банке, а в самом приложении: переподключение вернёт '
    + 'работу на несколько часов, и всё повторится. Если импорт нужен прямо сейчас — переподключите, '
    + 'но сообщите об этой строке тому, кто обслуживает приложение.'
