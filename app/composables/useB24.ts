...
export const useB24 = () => {
  ...
  /**
   * Открыть МЕССЕНДЖЕР портала штатным методом SDK (`parent.imOpenMessenger`).
   *
   ...
   *
   * ⚠ Диалог НЕ адресуем, хотя метод его принимает. Сообщение пишет БОТ приложения ПОЛУЧАТЕЛЮ,
   * то есть переписка идёт между ними; передать сюда id выбранного сотрудника значит открыть
   * переписку АДМИНА с ним, где нужного сообщения нет вовсе — кнопка, ведущая не туда, хуже
   * кнопки, ведущей в список чатов, где оно лежит сверху.
   *
   ...
   */
  async function openMessenger(): Promise<boolean> {
    await init()
    const f = get()
    if (!f) return false
    try {
      await f.parent.imOpenMessenger(undefined)
      return true
    } catch {
      return false
    }
