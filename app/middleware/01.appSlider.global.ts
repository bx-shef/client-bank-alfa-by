 ...
 *  приложения тот же код работает потому, что там страница рендерится НА ЗАПРОС и метки в payload
 *  нет.
 *
 *  ⚠ Ось различия — пререндер против рендера на запрос, а НЕ «nginx против Nitro»: Nitro есть и у
 *  нас (он отдаёт `/api/*`), просто фронтенд собран `nuxt generate` и HTML страниц он не рендерит
 *  никогда. Метка живёт в самом payload (`.output/public/app/_payload.json` → `prerenderedAt`),
 *  поэтому отдавай ту же пререндеренную страницу хоть наш Nitro — поведение было бы тем же.
 *
