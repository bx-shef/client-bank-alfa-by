 ...
 *
 *  ⚠ Ось различия — ПРЕРЕНДЕР, а не «nginx против Nitro». Проверено на трёх сборках одного и того
 *  же кода: SPA (`ssr: false`) — работает; SSR-роут БЕЗ пререндера — работает; пререндеренный роут —
 *  не работает, причём одинаково и под nginx, и под самим Nitro (`node .output/server/index.mjs`).
 *  Значит дело не в том, кто отдаёт файл, а в метке `prerenderedAt` внутри его payload. Перейти на `ssr: false`
