...
export default defineNuxtConfig({
  ...
  nitro: {
    prerender: {
      ...
      // `/404.html` — отдельной строкой, а не в `PRERENDER_ROUTES`: это не страница приложения
      // (её нет в `app/pages`, она не попадает ни в sitemap, ни в гарды служебных маршрутов), а
      // артефакт под `error_page` в nginx. Без неё Nitro кладёт туда голую SPA-оболочку: человек
      // с опечаткой в адресе видит пустой белый экран до гидрации, а краулер — всегда.
      routes: [...PRERENDER_ROUTES, '/404.html']
    }
  },
