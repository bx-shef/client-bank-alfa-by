...
| Опрос банков (stage 5) | 🧪 ~30% | OAuth-ядра + **рефреш банк-токена** (A4) готовы; **транспорт `bankFetch.ts` собран + юнит-тесты (A5)** — Альфа синхронный GET, Приор → A5b; **не подключён к воркеру** (свап заглушки `fetchStatement` — A9) |
