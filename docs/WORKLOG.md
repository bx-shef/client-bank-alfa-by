...
| Дедуп через origin-метку в B24 | ⬜ 0% | **оптимизация, не блокер**; уточнено #259 — B24-маркер есть **только** у `crm.item` (`originId`/`xmlId`), у `todo`-дела **нет** → его стор верен by design. Поэтапно: Фаза A (факт→состояние цели), Фаза B (носитель = элемент СП, `xmlId`); чужие поля не штампуем |
...
2. **Дедуп через origin-метку в B24** (#109/#259/PROCESSING §1) — Фаза A (факт разнесения → состояние цели, live-gated) + Фаза B (носитель = элемент смарт-процесса, `xmlId`); `todo`-стор остаётся (маркера нет). Делает crm-sync масштабируемым.
