...
| Дедуп через origin-метку в B24 | 🧪 ~55% | **Носитель = настраиваемое дело, дедуп в B24 — СДЕЛАНО (#259, без флага):** `configurable.add` с маркером `originatorId`+`originId` + дедуп поиском `crm.activity.list`; стор `activity_dedup` и todo-путь **удалены**. Ядра+тесты; **live-смоук пройден** (`pnpm activity:test`, `bel.bitrix24.by` — write+дедуп round-trip; live-находка: `icon` обязателен). **Фаза A (amount-путь) — сделана** (`isTargetApplied` читает состояние цели в B24, live-verified); осталось: элемент СП, усиление ключа |
| Опрос банков (stage 5) | 🧪 ~62% | OAuth-ядра + рефреш (A4) + транспорт `bankFetch.ts` (A5) + свап в воркер (A9) + реестр счетов (A6) + крон-таймер (A10) + **connect-поток A7 ЗАВЕРШЁН** (A7a state + A7b роуты + A7c UI) + **глобальный rate-limiter A8** (100/60с) — вся машинерия собрана. Осталось для боевого прогона: банк-креды владельца (`ALFA_OAUTH_*`), `CRON_REAL_POLL=1`, Приор async (A5b) |
...
4. ✅ **Опрос банков (stage 5)** — `bank-fetch` живой (A5+A9), реестр/таймер (A6/A10), connect-поток с UI
   (A7), глобальный rate-limiter (A8). Осталось: банк-креды владельца для боевого прогона, Приор async (A5b).
