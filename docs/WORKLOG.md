...
2. **Дедуп через origin-метку в B24** (#109/#259/PROCESSING §1) — ✅ **носитель = настраиваемое дело, дедуп в B24 (без флага, стор `activity_dedup` удалён)**; ждёт live-смоук на OAuth-портале. Осталось Фаза A (факт разнесения → состояние цели) + элемент СП. Делает crm-sync масштабируемым.
