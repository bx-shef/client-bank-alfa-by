...
| Опрос банков (stage 5) | 🧪 ~48% | OAuth-ядра + рефреш токена (A4) + транспорт `bankFetch.ts` (A5) + свап в воркер (A9) + **реестр счетов (A6)** + **живой крон-таймер (A10)** готовы — вся машинерия опроса собрана (таймер default-OFF `CRON_REAL_POLL` до rate-limiter A8). Осталось: connect-эндпоинты (A7 — подключат реальные счета), rate-limiter (A8), Приор async (A5b) |
