...
| Опрос банков (stage 5) | 🧪 ~58% | OAuth-ядра + рефреш (A4) + транспорт `bankFetch.ts` (A5) + свап в воркер (A9) + реестр счетов (A6) + крон-таймер (A10) + **connect-поток A7 ЗАВЕРШЁН** (state A7a + роуты A7b + UI на b24ui A7c) — админ может подключить счёт из настроек. Осталось для боевого прогона: банк-креды владельца (`ALFA_OAUTH_*`), `CRON_REAL_POLL=1`, rate-limiter (A8), Приор async (A5b) |
