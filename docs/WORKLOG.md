...
| Разнесение оплат #109 | 🧪 ~80% | recognize→route→resolve→отсев стадий→решение→**запись факта** (#184) + **триггеры** (#79) — **live-verified**; **мост `via-document` — ПОДКЛЮЧЁН + LIVE-VERIFIED** (`verify:109` #8, `documentgenerator` в скоупах) + попутно **починен `deal-id`-путь** (`parentId2` self-reference на сделке, живьём); **мутация портала** (`payment.pay` / стадия инвойса) — **код готов, за гейтом `autoDistribute`, но `payment.pay` живьём не проведён** (нужен баланс покупателя — на seed недоступен; стадия инвойса проверялась apply/revert). Осталось: order `payment.add`-путь, элемент СП, live-verify мутации `payment.pay` |
...
### 2026-07-18 — #78 телеметрия: полное покрытие конвейера деревом трасс (слайс 1 добит)
- **Аудит покрытия** (агент): спаны были только на `makeSdkRestCall` + `crm-sync` — 3 из 4 job-воркеров,
  батч, OAuth-refresh и крон-корни **без спанов**; file-parse вообще без телеметрии.
- **Закрыто:** все 4 job-воркера в `withSpan` (b24-events/bank-fetch/file-parse + уже был crm-sync);
  `makeSdkBatchCall` → `withDependencySpan` (`dep.op_count`); `sdkRefreshTransport` → `dep bitrix24 oauth.refresh`;
  крон-корни `cron.real-poll/keep-alive/sweep`.
- **🔑 Главный фикс (ревьюер полноты):** хелперы создавали спан через `startSpan`, но **не активировали** его в
  контексте → дочерние спаны (авто pg/redis/undici + вложенные dep) висели отдельными корнями, а не вложенными.
  Добавлен `context.with(trace.setSpan(...))` в оба хелпера → трасса теперь реально **древовидная** (иначе cron-корни
  и «дочерние под bank-fetch-root» были бы фикцией). Канонический OTel-паттерн; no-op когда телеметрия выключена.
- **PII:** новые allowlist-ключи `job.kind`/`dep.op_count` — форма/счётчик; member_id только `portal.hash`; params/тело
  батча/refresh-токен к спанам не прикрепляются (ревьюер приватности — чисто).
- **Правило проекта** (запрос владельца) в `CLAUDE.md` «Конвенции»: новый job/транспорт-обёртка/крон → span, атрибуты
  только из allowlist, без финансовых ПДн, сырой `$fetch` не оборачиваем. Карта — `docs/OBSERVABILITY.md`.
- **Багоохота** (3 агента по чистым ядрам) — реальных багов нет (подтверждает качество). PR #321, 5 ревьюеров, `pnpm check` зелёный (1582).

### 2026-07-18 — #109 мост `via-document` подключён + LIVE-VERIFIED (владелец дал креды) + фикс `deal-id`
- **Контекст:** владелец передал вебхук тест-портала (`b24-86sr2r`) + OAuth-креды (`bel.bitrix24.by`) →
  разблокирован live-verify гейтов. Оба смоука зелёные (`verify:109` 21/21 read, `sdk:crm:test` OAuth).
- **Мост `via-document` (`document-number`) — ПОДКЛЮЧЁН, гейт снят:** сгенерирован документ из шаблона #1 на
  seed-сделку → **живьём подтверждено**: конверт `{result:{documents:[…]}}`, обратный `filter:{number}` **работает**
  (не игнорируется), `entityTypeId`/`entityId` присутствуют. Wiring в `intentResolver`: `findDocumentEntities(value)` →
  чистый `routeDocumentRef` (2→deal / 31→invoice / == `configFields['smart-entity']`→smart-process; иначе пропуск) →
  `findCandidateById` **со скоупом по компании плательщика** (IDOR). Дошёл до 27/27 в `verify:109` (#8: мост→deal-кандидат
  + IDOR-контроль чужой компанией → null). `document-number` больше не `unsupported`.
- **Латентный баг `deal-id`-пути — вскрыт мостом и починен:** `findCandidateById('deal',2,…)` падал live с «An entity
  type can't be a parent/child type to itself» — `select` включал `parentId2` (для сделки `parentId2`=родитель типа 2=
  сама сделка = self-reference). `deal-id`-путь раньше **живьём не гонялся** (verify:109 не звал findCandidateById для
  сделок), поэтому баг не ловился. Фикс: `selectFields(entityTypeId)` не выбирает `parentId2` для сделки (для инвойса/СП —
  оставляет). Теперь deal-lookup подтверждён вживую.
- **Security-находка (live):** портал **игнорирует `select`** в `document.list` → ответ всегда несёт `*UrlMachine` с
  живым access-токеном; `findDocumentEntities` берёт только id-поля (утечки нет), сырой ответ не логируем. Комментарий
  модуля исправлен.
- **Скоуп:** `documentgenerator` добавлен в `B24_REQUIRED_SCOPES` (мост в hot-path; ⚠ ре-consent на установленных порталах).
- **Багоохота (3 адверсариальных агента на чистые ядра allocation/recognition/statement):** реальных багов НЕ найдено
  (кроме вскрытого live deal-`parentId2`, который не в этих модулях) — подтверждает качество чистого слоя.
- **Тесты:** `intentResolver` (мост: route/скоуп/unroutable/multi-ref/ошибка + прямые `routeDocumentRef`),
  `itemByIdLookup` (deal без `parentId2`, инвойс/СП с ним), `b24` (скоуп). `pnpm check` зелёный. Live: `verify:109` 27/27.
- **Каждый PR — 5 ревьюеров, замечания устранены.**
...
- **#62 результат импорта для сотрудников (слайс 1):** чистое ядро `app/utils/importStats.ts`
  ...
  (был в 3 местах).
- **#62 результат импорта для сотрудников (слайс 2):** компонент `app/components/ImportStatsChart.vue` —
  count-up плитки (операции/приходы/расходы) + ECharts бары приход/расход по дням + пончик доли, на чистом
  `importStats`. ECharts динамически импортится и tree-shaken (client-only, как `QueueMonitor`); свет/тёмная;
  `prefers-reduced-motion` глушит анимацию; приход/расход = emerald/rose (как в `OperationList`), всегда с
  подписями (↑/↓ + легенда) — вторичная кодировка для CVD. Встроен в `StatementUpload` над предпросмотром;
  визуально проверен (свет/тёмная).
- **#62 проводка на `/app`:** тот же `ImportStatsChart` встроен и на `/app` (in-portal, над «Последними
  операциями», заголовок «Сводка по операциям») — закрыт пункт «переиспользуемый для обоих путей»; убрана
  дублирующая строка итогов под списком; гейт ECharts за `hasCanvas2d` (headless-DOM без canvas → count-up
  без графиков, без unhandled-ошибок в тестах). Оставшиеся чек-боксы #62 (доля «сопоставлено с CRM») — за #23.
...
## Архив: снимки автономных прогонов (перенесено из project-map, 2026-07-09)

Ниже — исторический контекст двух остановленных сессий. Он описывал «следующий шаг», который с тех
пор сделан; хранится как журнальная запись, актуальное состояние — в `project-map.md`.
