...
| Стадия 4: компания + дело + чат | 🧪 ~75% | поиск компании **и запись дела** вживую (`bel.bitrix24.by`, `pnpm activity:test`); чат — ядра+тесты, не гонялось |
...
| Дедуп через origin-метку в B24 | 🧪 ~55% | **Носитель = настраиваемое дело, дедуп в B24 — СДЕЛАНО (#259, без флага):** `configurable.add` с маркером `originatorId`+`originId` + дедуп поиском `crm.activity.list`; стор `activity_dedup` и todo-путь **удалены**. Ядра+тесты; **live-смоук пройден** (`pnpm activity:test`, `bel.bitrix24.by` — write+дедуп round-trip; live-находка: `icon` обязателен). Осталось: Фаза A (факт разнесения → состояние цели), элемент СП, усиление ключа |
| Опрос банков (stage 5) | 🧪 ~38% | OAuth-ядра + **рефреш банк-токена** (A4) + **транспорт `bankFetch.ts`** (A5, Альфа GET) готовы; **подключён к воркеру (A9)** — реальный счёт с банк-токеном опрашивается вживую. Осталось: реестр счетов/токенов (A6), connect-эндпоинты (A7), rate-limiter (A8), Приор async (A5b), живой крон-таймер (A10) |
...
2. **Дедуп через origin-метку в B24** (#109/#259/PROCESSING §1) — ✅ **носитель = настраиваемое дело, дедуп в B24 (без флага, стор `activity_dedup` удалён)**; **live-смоук пройден** (`pnpm activity:test`, `bel.bitrix24.by`). Осталось Фаза A (факт разнесения → состояние цели) + элемент СП. Делает crm-sync масштабируемым.
