...
- **Пул оплат раз на операцию** ([#192](https://github.com/bx-shef/client-bank-alfa-by/issues/191)) —
...
4. **Батчинг `callBatch`** (до 50 команд) — стратегический рычаг: собрать поиск дубля/компании/запись в один-два
   вызова вместо N (см. пункт про батчи выше).
...
**Автомасштабирование** — сигнал очевиден: **длина очереди** (её и рисует `/queues`). По глубине
