...
|------|:---:|:---:|:---:|------------|
| Один контейнер (**дефолт**) | 1 | 1 | 1 | и API, и очереди, и крон — как раньше |
| HTTP/primary | **0** | 1 | 1 | отдаёт `/api/*` + крон-планировщик; джобы **не** обрабатывает |
| **worker** (масштабируем) | 1 | **0** | **0** | только тянет очереди; крон/миграцию не трогает |

В `docker-compose.prod.yml` роли уже разведены: `backend` = HTTP+крон (`QUEUE_WORKERS=0`), сервис
**`worker`** = обработка (`QUEUE_CRON=0`, `RUN_MIGRATION=0`). **Крон — ровно на одном** инстансе (иначе N
планировщиков наплодят дублей fetch-джобов); **миграцию** гоняет только backend (иначе реплики воркеров
racy на `CREATE TABLE`).
...
1. **Concurrency воркера** — `QUEUE_CONCURRENCY=N`: один воркер держит `N` джобов в параллель (дефолт `1`;
   применяется к fetch/parse/crm-sync, события — всегда 1, чтобы install/uninstall портала не переставились).
   Самый дешёвый рычаг. Узкое место — `crm-sync` (I/O к B24); **поднимать только вместе с per-portal
   лимитером/батчем**, иначе большой пакет упрётся в `QUERY_LIMIT`.
2. **Реплики воркер-контейнера** — `docker compose up -d --scale worker=N` (или `deploy.replicas`). Тот же
   `startWorkers()` в нескольких контейнерах; все на одном Redis тянут из **одной** очереди — **Redis
   отдаёт каждый джоб ровно одному воркеру**, поэтому добавил реплику → больше throughput, код не меняется.
