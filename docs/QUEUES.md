...
1. **Concurrency воркера** — `new Worker(name, fn, { concurrency: N })`: один воркер держит `N` джобов
   в параллель. Самый дешёвый рычаг (сейчас `N=1`). Узкое место — `crm-sync` (I/O к B24), его concurrency
   поднимаем первым.
2. **Реплики воркер-контейнера** — тот же `startWorkers()` в **отдельном контейнере**, несколько реплик.
   Все воркеры на одном Redis тянут из **одной** очереди: добавил реплику → больше throughput, код не
   меняется. Это горизонтальный scale-out (сейчас воркеры **in-process** в backend; вынос — следующий шаг).
