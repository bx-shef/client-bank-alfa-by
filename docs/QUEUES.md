...
- **Чистые обработчики с DI.** [`handlers.ts`](../server/queue/handlers.ts) — вся логика
  ...
  Альфы (A9, `fetchBankStatement`)**; реальный счёт без банк-токена → `[]` инертно; Приор — движок готов, но в автоопрос не включён. **Глобальный
...
> ⚠️ **Приор в автоопрос пока не включён** (`POLLABLE_PROVIDERS`): его задача стоит ~10 запросов, а
> лимитер A8 считает **задачи**, и `bank-fetch` идёт с `QUEUE_CONCURRENCY` (дефолт 1) — одна задача
> Приора держала бы слот минутами и блокировала Альфу. Нужен учёт по запросам + отдельная очередь/слот.
...
- **SDK-транспорт — единственный, по умолчанию** (`@bitrix24/b24jssdk`, `portalSdkResolver.ts`→`makePortalSdkCall`):
  ...
  UPDATE-only-эквивалент через tombstone-guarded `saveToken`. Рефреш идёт **мимо** advisory-lock (компромисс, принятый
...
пересобирается через `SDK_CLIENT_TTL_MS` 60с. persist рефреша — **UPDATE-only-эквивалент**
...
| `MANUAL_POLL_ENABLED` | Включает кнопку «Опросить сейчас» (`POST /api/poll-now`, #54). Default OFF — частота регулируется app-side, не порталом |
