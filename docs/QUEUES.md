...
- **X** — время (**скользящее окно**: обе границы сдвигаются каждый тик, график течёт справа налево),
  **Y** — backlog очереди; каждая очередь = линия с **горизонтальной заливкой** (слева плотнее, справа
  прозрачно — «хвост кометы»); на конце линии — текущее значение; справа — таблица
  `ждут / работа / готово / ошибки` (клик по строке скрывает/показывает линию); кнопка пауза + **выбор
  диапазона** (10 мин / 30 мин / 1 ч / 2 ч / 4 ч).
- Ряд строится **на клиенте**: `GET /api/ops/queues` отдаёт только текущий снапшот (без истории),
  поэтому каждый опрос добавляет точку и сдвигает окно (эффект бегущей ленты). **Память ограничена**:
  держим ≤ `maxPoints` (240) точек на очередь; шаг опроса выводится из диапазона (`range / maxPoints`,
  не чаще 2 с), поэтому 4-часовой вид не копит тысячи точек.
