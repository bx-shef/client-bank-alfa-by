...
- **At-least-once.** Доставка «хотя бы раз», поэтому `crm-sync` дедупит **внутри батча** по
  ...
  - **Целевая модель (#109/#259, [`PROCESSING.md`](PROCESSING.md) §1):** идемпотентность стремимся держать
    ...
    `allocation_fact` → состояние цели (Фаза A); стор дела снимается сменой носителя на настраиваемое дело
    или элемент СП (Фаза B, ⚠ OAuth-контекст — вебхуком не проверить). Чужие поля (originId/xmlId
    клиентских сделок/инвойсов) **не штампуем** — наш маркер на нашей записи. Пока код на сторах — ожидаемо.
...
- [issue #9](https://github.com/bx-shef/client-bank-alfa-by/issues/9) — персистентный стор дедупа дел (`activity_dedup`). **Верен by design, пока носитель — простое `todo`-дело** (#259: у него нет искомого маркера); настраиваемое дело (`crm.activity.configurable.add`, `originId`) или элемент СП (`xmlId`) снимут и его — [`PROCESSING.md`](PROCESSING.md) §1.
