...
**Осталось (до первого РЕАЛЬНОГО опроса портала — не демо; и обязательно до слайса записи разнесения):**
1. **`bind`-`RestCall`-once на джобу.** Сейчас `findCompany`/`resolveIntents`/`writeActivity`/`notifyChat`
   каждый зовёт `makePortalRestCall` (loadToken + ensureAccessToken) — до 4× на операцию. Резолвить токен
   один раз на `crm-sync`-джобу и прокидывать готовый `RestCall` в депсы.
2. **Глобальный троттл/лимитер** темпа REST — но лимит B24 **per-portal**, поэтому справедливее группировать
   по порталу (BullMQ `limiter` режет суммарный темп; см. выше про партиционирование). Ориентир — не упереться
   в `QUERY_LIMIT_EXCEEDED` при `QUEUE_CONCURRENCY>1`.
3. **Пагинация/сужение `findCompanyDealPayments`** — список сделок компании сейчас без пагинации; на компании
   с большим числом сделок один скан дорог.
4. **Батчинг `callBatch`** (до 50 команд) — стратегический рычаг: собрать поиск дубля/компании/запись в один-два
   вызова вместо N (см. пункт про батчи выше). При частичном падении батча в момент записи разнесения — опираться
   на идемпотентность ([#184](https://github.com/bx-shef/client-bank-alfa-by/issues/184)), не откатывать всё.
5. **Retry/backoff на `QUERY_LIMIT_EXCEEDED`** — троттл снижает частоту, но сам лимит всё равно случается: у
   `crm-sync`-джобы должна быть политика повтора (BullMQ `attempts` + exponential backoff + jitter), а не мгновенный
   фейл. Пер-портальная изоляция (пауза джобы упёршегося портала, не всей очереди — см. «партиционирование по
   порталу» выше) — часть этой же правки.

Почему не сделали сразу: пока пайплайн **log-only** (ничего не пишется) и реального опроса портала нет
(`fetchStatement` отдаёт только demo-items), максимум вреда — упавшая джоба + retry на `QUERY_LIMIT_EXCEEDED`.
Тюнинг лимитера требует **живого портала под нагрузкой**, поэтому дизайн зафиксирован здесь, а реализация —
отдельным инфра-PR перед реальным опросом/записью. Детали — [issue #191](https://github.com/bx-shef/client-bank-alfa-by/issues/191).
