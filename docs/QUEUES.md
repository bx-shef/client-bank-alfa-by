...
- **At-least-once.** Доставка «хотя бы раз», поэтому `crm-sync` дедупит **внутри батча** по
  ...
  - **Целевая модель (#109/#259, [`PROCESSING.md`](PROCESSING.md) §1):** идемпотентность стремимся держать
    в B24 (перед записью искать **маркер** сущности-носителя), но маркер есть **только** у `crm.item`
    (`originId`/`xmlId`), а у `todo`-дела — **нет** (#259 live-verified) → для дела стор `activity_dedup`
    **верен by design**. Миграция **частичная**: `allocation_fact` уходит в состояние цели / `xmlId`
    элемента смарт-процесса (Фаза A/B), стор дела остаётся до смены носителя на элемент СП. Чужие поля
    (originId/xmlId клиентской сделки/инвойса) **не штампуем**. Пока код на сторах — это ожидаемо.
...
- [issue #9](https://github.com/bx-shef/client-bank-alfa-by/issues/9) — персистентный стор дедупа дел (`activity_dedup`). **Остаётся by design** (#259: у `todo`-дела нет искомого B24-маркера); в B24 мигрирует только `allocation_fact` — [`PROCESSING.md`](PROCESSING.md) §1.
