...
- **Насыщение rate-limiter'а (A8).** Живой опрос Альфы капнут глобально (`QUEUE_FETCH_RATE_MAX`/
  ...
  от обычного бэклога. Если `bank-fetch waiting` хронически растёт при живом опросе — это упор в кап,
  а не «застрявший воркер»; поднять лимит только если Альфа поднимет свой. (Отдельный лог сатурации —
  follow-up, #78.)
...
- **Лог сатурации rate-limiter'а** `bank-fetch` (чтобы упор в кап был явным, а не «растущий бэклог»).
