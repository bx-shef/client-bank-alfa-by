...
**Снимать дамп** (на хосте, рядом с прод-compose):

```bash
docker compose -f docker-compose.prod.yml exec -T db \
  pg_dump -U app --format=custom app > backup-$(date +%F).dump
```
...
refresh банков — хранить его надо как секрет, не в публичном облаке без шифрования.
...
`GET /api/ready` отдаёт 200, а `portal_tokens` содержит ожидаемое число порталов.
