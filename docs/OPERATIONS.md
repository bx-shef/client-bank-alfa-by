...
**Включение.** Две переменные на сервисе `backend` (не на `worker` — проверка живёт на cron-инстансе,
иначе масштабированные реплики дадут N копий каждого сообщения):

```bash
TELEGRAM_ALERT_BOT_TOKEN=<от @BotFather, вид «цифры:строка»>
TELEGRAM_ALERT_CHAT_ID=<id чата: у группы отрицательный «-100…», либо @канал>
```

Бота надо **добавить в группу/канал**, иначе Telegram ответит 403. Fail-closed: не заданы обе ⇒
канал выключен, аварии остаются в логе и на `/queues`. Половинчатая настройка — тоже выключен, и
`envCheck` предупредит об этом на старте (это опаснее выключенного: оператор считает, что за
системой следят, а сообщения молча пропадают).

**О чём сообщает.** Проверка раз в 5 минут читает все очереди и выносит три вердикта:
...
| `stalled` | самая старая незавершённая задача пережила бюджет очереди | Жив ли `worker`-контейнер; не завис ли портал/банк. Бюджеты — по очередям, см. ниже |
| `failing` | ≥3 задач исчерпали все попытки за час | Смотреть `[queue-job-failed] … FINAL` в логах: это **наши** сбои — пер-портальные отказы (нет прав, `insufficient_scope`, `PAYMENT_REQUIRED`, портал разавторизован) из счёта исключены намеренно |
...
обход = `счета / лимит` минут, на масштабе — часы), `trigger-fire` часами ждёт в backoff, пока админ
портала зарегистрирует CODE (это чужое действие, не наша авария — правило простоя к ней не
применяется вовсе), `feedback-post` ретраит GitHub около часа. Быстрые пути (`b24-events`,
`file-parse`, `crm-sync`, `b24-deletions`) держат 20 минут. Таблица — `server/utils/queueAlert.ts`.

**Почему канал молчаливый.** Одно сообщение на **эпизод**, а не на замер: авария живёт дольше
5-минутного интервала, и канал, который пишет 12 раз в час, перестают читать — тогда он не сработает
в тот единственный раз, ради которого заведён. Повторное объявление той же поломки после её
исчезновения придавлено часом (иначе «мигающая» на пороге очередь дала бы 24 сообщения в час).
Восстановление тоже объявляется — иначе не отличить «починилось» от «мы замолчали».
