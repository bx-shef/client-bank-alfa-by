...
Дальше строки перешифровываются **сами**: пишем всегда новым ключом, а токены обновляются при
каждом рефреше (реактивно и суточным keep-alive у порталов near-expiry). Простаивающий портал
рефрешится не чаще раза в несколько суток, поэтому `B24_TOKEN_ENC_KEY_OLD` держать **не меньше
месяца**, и только потом убирать. Убрали раньше — токены тех порталов, что не успели обновиться,
осиротеют, и это опять переустановка.
