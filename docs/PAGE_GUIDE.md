...
Приложение живёт в **трёх контекстах**. Прежде чем делать страницу, определи, к какому она
относится — от этого зависит layout, тема, стиль и гейт.

⚠ Визуально контекста два (лендинг против b24ui), а вот layout'ов ТРИ: у страниц внутри портала и
у экранов оператора разметка совпадает, но авторитет разный (фрейм-токен против нашей сессионной
куки) и гейт разный. Раньше на них был один layout `clear`, и имя не различало их вовсе (#532).
...
| **In-portal** | UI внутри iframe Bitrix24 | `portal` | b24ui light/dark-auto | `/app`, `/import`, `/settings`, `/install` |
| **Экраны оператора** | служебные страницы в обычной вкладке | `operator` | b24ui light/dark-auto | `/login`, `/queues` |
...
- **Страница приложения** (`/app`, `/import`, `/settings`, `/install`) — тело в `<InPortalGate>` (#414): снаружи
...
- Layout `app/layouts/portal.vue` (in-portal) или `app/layouts/operator.vue` (служебные экраны
  ...
  ⚠ **`<B24App>` у нас в LAYOUT, а у первоисточника — в `app.vue`** (официальный пример
  `b24sdk-examples/js/03-nuxt-frame` держит его над `<NuxtLayout>`, а layout'ы оставляет чистым
  хромом). Отличие вынужденное: лендинг передаёт `B24App` собственные пропсы (`landing-shell dark`),
  и единый корневой пришлось бы параметризовать от layout'а. Второго `B24App` при этом не
  возникает — замерено на переходе `/` → `/login`: оболочка ровно одна. ⚠ Но если когда-нибудь
  включат `pageTransition`/`layoutTransition`, два layout'а окажутся смонтированы одновременно, и
  тосты с оверлеями (у них ГЛОБАЛЬНОЕ состояние) отрисуются дважды — тогда переносить придётся.
