...
- ⚠ **Редирект возвратом `navigateTo` из route-мидлвара на ПЕРВОЙ навигации не
  работает**, если страница пререндерена и открыта со строкой запроса (а in-portal
  страницы открываются именно так — портал добавляет `DOMAIN`/`APP_SID`/…): Nuxt
  гидратирует на голом пути, а на `app:suspense:resolve` восстанавливает исходный
  адрес в обход гардов, затирая наш редирект. Замерено на собранной статике (#555):
  мидлвар печатает верное решение, адрес не меняется. ⚠ В **SPA** (`ssr: false` —
  так собраны примеры Битрикса и соседний `ai-price-import`) этого нет вовсе, и
  проверено экспериментом: с `ssr: false` голый `return navigateTo` работает. Решения
  по мидлварам оттуда к нам **не переносятся**.
  Если страница обязана увести дальше (так живёт фрейм слайдера,
  `app/middleware/01.appSlider.global.ts`), навигируй из `onNuxtReady` под
  `nuxtApp.isHydrating`. Гвард `middleware: auth` этим не задет — он не редиректит
  на первой навигации, контент прячет `AuthGate`.
