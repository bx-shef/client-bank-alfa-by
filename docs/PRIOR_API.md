...
**Что НЕ доказано:** пройдёт ли BY-крипто хендшейк с `apibel.priorbank.by:9345` без клиентского
сертификата. Из dev-контейнера до банка нет сети — прокси честно ответил
`dial tcp 185.137.116.158:9345: i/o timeout`, то есть отказ **сетевой**, а не криптографический
(прямой запрос без прокси даёт `Connection reset by peer` на хендшейке — хост живой, но обычный TLS
не понимает). Проверяется одной командой с машины, у которой есть выход на банк:

```bash
xvfb-run -a ./avtunproxy -l dbg &
curl -x http://127.0.0.1:10224 -v https://apibel.priorbank.by:9345/
```
