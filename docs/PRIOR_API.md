...
1. **Инфраструктура** — BY-крипто TLS-шлюз (СКЗИ, СТБ 34.101.65),
   [#41](https://github.com/bx-shef/client-bank-alfa-by/issues/41). Покупается у вендора, кодом
   не решается.
2. **Код** — прод-метод аутентификации `private_key_jwt`,
   [#444](https://github.com/bx-shef/client-bank-alfa-by/issues/444). Нынешний
   `client_secret_basic` банк принимает **только в тестовой среде**. Проверяется на sandbox,
   СКЗИ для этого не нужен.
...
## 2. Живой Open Banking API по СПР (реализован; прод ждёт СКЗИ #41 + метод аутентификации #444)
...
- **`prior-by` (путь №2)** — нормализация **сделана**: `normalizePrior` в `app/utils/priorStatement.ts`
  ...
  **Осталось:** живой sandbox-прогон на кредах владельца, **прод-СКЗИ** (issue #41 — без BY-крипто
  TLS-шлюза прод-хост `:9345` недостижим; инфраструктура) и **прод-метод аутентификации**
  (issue #444 — `private_key_jwt` вместо sandbox-only `client_secret_basic`; код). Sandbox `:9344`
  работает обычным TLS и не требует ни того, ни другого.
