...
| Туннель до банка | `[PROXY] CONNECT apibel.priorbank.by:9345 -- 200 -- 73.5 ms` — соединение с `185.137.116.157:9345` **установлено** |
...
лицензией до 2030. **Остался ровно один шаг:** попросить АВЕСТ добавить `apibel.priorbank.by:9345`
(прод) и `api.priorbank.by:9344` (тест) в список. Вопрос предметный — для трёх других банков РБ это
уже сделано.
...
#### Совместимость крипто с сервером Приорбанка — ДОКАЗАНА (диагностический стенд)

Открытым оставался главный технический риск: договорятся ли реализация СТБ у АВЕСТ и сервер
Приорбанка. Проверено напрямую. Стенд (разобран сразу после): в `/etc/hosts` один из **тестовых
хостов самого АВЕСТ** из белого списка уведён на `127.0.0.1`, там `socat` слушает его порт и
пересылает TCP на `apibel.priorbank.by:9345`. Взята запись с нестандартным портом
(`cloud-ds-cd.qa.avest.by:7443`) — и порт свободен, и заодно проверяется случай «не 443».

Предварительно подтверждено, что банк **не привередлив к SNI**: `openssl s_client` с чужим
`-servername` получает тот же 90-байтный ServerHello (наш ClientHello при этом растёт ровно на
разницу длин имён), то есть подмена имени сама по себе соединение не рвёт.

Результат в журнале прокси:

```
[PROXY] getting roots for host cloud-ds-cd.qa.avest.by : [
    "CN=Корневой удостоверяющий центр службы технической поддержки,O=ЗАО \"АВЕСТ\"…",
    "CN=STEND BY Root CA NCES 2024,…", "CN=Корневой удостоверяющий центр ГосСУОК,…",
    "CN=BY Root CA NCES 2,O=Государственное предприятие \"НЦЭУ\",C=BY" ]
[ERR] [PROXY] HTTP 503 - x509: certificate is valid for apibel.priorbank.by,
                              not cloud-ds-cd.qa.avest.by
```
...
его и прочитать имя. Отказ произошёл **только** на сверке имени, которое мы сами и подменили.
Напомним, что обычный OpenSSL с этим сервером не согласовывает даже шифр — а крипто АВЕСТ дошло до
разбора сертификата. Значит **реализации совместимы**, и единственное недостающее звено —
запись в подписанном списке ресурсов.

⚠ Стенд — **только диагностика**. В прод он не годится: сертифицированное СКЗИ на подменённом имени
не закрывает требование СПР 6.02, ради которого всё и делается.

⚠ **Следствие для интеграции.** AvTunProxy — **прокси**, а не reverse-шлюз, поэтому подключается
НЕ подменой базового адреса (`PRIOR_OAUTH_API_BASE=http://crypto-gw:…`, как заложено в #455), а
прокси-диспетчером: URL остаётся публичным адресом банка, трафик направляется в прокси
(undici `ProxyAgent`). Это проще и снимает вопрос доверия к сертификату перехвата. Механизм #455
не пропадает — он нужен шлюзовым продуктам (AvAuthGate/AvTLSSrv), — но под AvTunProxy форма другая.
Проектировать до ответа АВЕСТ рано.
...
источником**, передаётся только сам носитель. И это отдельная причина не брать рабочий ключ
бухгалтера: им подписывают отчётность, всё подписанное юридически на нём, а экспорт в программный
контейнер снял бы аппаратную защиту.

⚠ **AvTunProxy — не прод-решение по умолчанию.** Это пользовательский продукт для браузера на
рабочем месте: требует виртуальный дисплей, ходит за случайностью на сервер вендора, сборке 3.5 года.
Как проверка гипотезы — годится; лицензирование и поддержка на сервере остаются вопросом к АВЕСТ
(#41). Вендор на прямой запрос рекомендовал AvTLSSrv с клиентскими настройками — но AvTLSSrv по
инструкции банка стоит **у самого банка**, см. цитату выше.
