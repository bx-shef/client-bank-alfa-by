...
готовы, A5b). Для **прода** второму пути дополнительно нужен BY-крипто TLS-шлюз (СКЗИ,
issue #41) — это инфраструктура, не код; sandbox `:9344` работает обычным TLS.
...
## 2. Живой Open Banking API по СПР (реализован; прод ждёт СКЗИ)
...
- **Прод:** `https://apibel.priorbank.by:9345` — **TLS на белорусских криптоалгоритмах**
  ...
  с банком (стр. 5 инструкции). ✅ **Договор с банком заключён** (владелец, до 2026-08) — этот
  шаг закрыт, остаются СКЗИ ([#41](https://github.com/bx-shef/client-bank-alfa-by/issues/41)) и
  прод-метод аутентификации ([#444](https://github.com/bx-shef/client-bank-alfa-by/issues/444)).
  Отдельного тестового хоста у прода нет — эндпоинт тот же, но в личном кабинете Магазина API
  можно создать **тестовые ключи** приложения и попробовать с ними (стр. 7, §1.3).
...
>
> ⚠ **Код пока на `client_secret_basic` во всех трёх местах** (регистрация `app/utils/priorOauth.ts`,
> обмен `code` `server/utils/bankConnectCallback.ts`, рефреш `server/utils/ensureBankToken.ts`);
> `client_assertion` не реализован — [#444](https://github.com/bx-shef/client-bank-alfa-by/issues/444).
...
- **Код к шлюзу готов:** адреса вынесены в env — `PRIOR_OAUTH_API_BASE`
  (`server/utils/priorFetch.ts`, `priorConnectStart.ts`) и `PRIOR_OAUTH_TOKEN_URL`; достаточно
  указать их на шлюз. ⚠ `PRIOR_OAUTH_AUDIENCE` трогать **нельзя** — это claim `aud` внутри
  подписанного `request`-JWT, который проверяет банк, а не сетевой адрес.
...
- **`prior-by` (путь №2)** — нормализация **сделана**: `normalizePrior` в `app/utils/priorStatement.ts`
  ...
  **Автоопрос — пока ВЫКЛЮЧЕН:** `prior-by` сознательно не в `POLLABLE_PROVIDERS` (`server/queue/cron.ts`).
  Две причины уровня планировщика (не корректности): (1) лимитер A8 считает ЗАДАЧИ и рассчитан на «1 задача ≈
  1 запрос» (верно для Альфы), а задача Приора — до 10 HTTP; (2) `bank-fetch` идёт с `QUEUE_CONCURRENCY`
  ...
  **Осталось:** живой sandbox-прогон на кредах владельца и **прод-СКЗИ** (issue #41) — без BY-крипто TLS-шлюза
  прод-хост `:9345` недостижим (инфраструктура, не код); sandbox `:9344` работает обычным TLS.
