...
**Вывод:** AvTunProxy применяет BY-крипто только к хостам из своего списка (`proxy.pac`, приезжает с
`config.avtunproxy.by`), а Приорбанка там нет. Иначе продукт был бы бессмысленным — обычный браузер
по СТБ не умеет, значит для поддерживаемых хостов прокси обязан терминировать TLS сам.

**Остался ровно один шаг** — попросить АВЕСТ добавить `apibel.priorbank.by:9345` (прод) и
`api.priorbank.by:9344` (тест) в список. Вопрос предметный: для трёх других банков РБ это уже
сделано. Ключевой носитель в этом сценарии не участвует нигде.
