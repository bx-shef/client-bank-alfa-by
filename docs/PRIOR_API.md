...
Тот же дистрибутив запущен **владельцем на прод-сервере**, у которого есть сеть до банка. Зависимости
(Debian 11): `xvfb libgtk-3-0 libglib2.0-0 libappindicator3-1 libpcsclite1 libusb-1.0-0`.
⚠ `libpcsclite1` обязателен, хотя смарт-карт у нас нет — без него бинарник не стартует вовсе
(`error while loading shared libraries`). ⚠ `libayatana-appindicator3-1` **не подходит**: SONAME
другой, нужен именно `libappindicator3-1` (или симлинк).

| Проверка | Результат |
|---|---|
| Старт | `avc self-test result: OK`, `[APP] binded OK` |
| Лицензия (админ-страница `/`) | **valid until 2030-01-01** — покупать, судя по всему, нечего |
| PAC | `200` |
...
| Туннель до банка | `[PROXY] CONNECT apibel.priorbank.by:9345 -- 200 -- 73.5 ms` — соединение с адрес банка **установлено** |
...
с `config.avtunproxy.by`), и это не гипотеза, а наблюдение.
...
банка → **получить его сертификат** (он передаётся уже внутри согласованного шифрования) → разобрать
