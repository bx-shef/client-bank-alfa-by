...
- **⛔ Открытый блокер (2026-08-12): `POST /accountConsents` → `403 BY.NBRB.Reauthenticate`**
  ...
  Вывод: отказ — решение банка о правах регистрации, и следующий шаг только один — запрос в банк с
  обоими `client_id`, временем и телом запроса. Пока блокер открыт, песочный прогон Приора
  (согласие → авторизация → выписка) пройти нельзя; код при этом готов и покрыт тестами.
