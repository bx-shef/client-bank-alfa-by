...
- **Подписки на API нужны КАЖДОЙ регистрации отдельно и различаются по симптому.** DCR создаёт
  ...
  |---|---|
  | `Open-banking-authorize v1.0` | `POST /oauth2/token` → 403 (токен Б не выдаётся) |
  | `Open-banking v1.0` | `POST /accountConsents` → 403 (токен есть, согласие нельзя) |

  То есть по номеру шага сразу видно, какой подписки не хватает.
