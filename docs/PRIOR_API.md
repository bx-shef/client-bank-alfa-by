...
- **Подписки на API нужны КАЖДОЙ регистрации отдельно и различаются по симптому.** DCR создаёт
  ...
  **не** на `:9443` (снаружи закрыт). Именно к нему относятся скоупы `apim:subscribe` /
  ...
  `GET /applications/{id}/oauth-keys` (там лежит `consumerKey` = наш `client_id`). Побочно это даёт
  **прямую проверку гипотезы про имя**: если приложения с нашим `consumerKey` в списке витрины нет
  вовсе, значит DCR создал клиента на уровне identity, а объекта в Магазине API не появилось — ровно
  то, чем грозит `client_name` вне `^[a-zA-Z0-9 ._-]*$`.
