...
- **Где стоит:** на **нашем сервере (backend)**, а не у конечного пользователя. Работает как
  локальный TLS-шлюз: наш код ходит в него обычным HTTP(S), а он поднимает BY-крипто TLS до
  `apibel.priorbank.by:9345`. Наш base URL Приора на проде указывает на этот шлюз.
- **Конечному пользователю ничего не нужно** — он лишь вводит логин/пароль интернет-банка на
  странице банка (обычный браузер/HTTPS); его пароль наше приложение не видит.
- **Sandbox работает без СКЗИ** — весь флоу разрабатываем и тестируем на `:9344` обычным TLS,
  СКЗИ подключаем последним шагом перед продом.
- **Docker:** СКЗИ — это **не библиотека в образ**, а отдельный крипто-шлюз (продукт АВЕСТ) +
  гос-сертификат. Разворачивается как **отдельный сервис** рядом с backend (сайдкар/на хосте —
  поддержку Linux/контейнера и лицензирование уточнять у АВЕСТ), backend его не «содержит», а
  ходит через него. Прод-схема (шлюз + сертификат + маршрут backend→шлюз→банк) — отдельный долг.
