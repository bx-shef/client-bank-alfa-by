...
- **⛔ Открытый блокер (2026-08-12): `POST /accountConsents` → `403 BY.NBRB.Reauthenticate`**
  ...
  **Причина — КАНАЛ, а не запрос.** Ответ банка (2026-08-12): «На стороне клиента должны быть
  установлены СКЗИ, обеспечивающие клиентскую часть TLS-соединения, согласно требованиям
  СТБ 34.101.65 … Для взаимодействия с нашим сервером авторизации для API `Open-banking-authorize`
  **вне зависимости от метода, который вызывается, или типа банковской операции** вызовы требуется
  осуществлять на `https://apibel.priorbank.by:9345`».

  То есть сервер авторизации у Приора **один и только на BY-крипто TLS**; песочный `:9344` выдаёт
  токен, но созданный там контекст клиента ресурсный бэкенд авторизацией не считает — отсюда
  «client re-authentication is required» при формально безупречном запросе. Единственная гипотеза,
  которую снаружи проверить было нельзя (мы измеряли всё, КРОМЕ канала), и она же оказалась верной.

  Проверено с прод-сервера (2026-08-12): `openssl s_client -connect apibel.priorbank.by:9345` →
  `CONNECTED`, прочитано 90 байт, затем `unknown cipher returned`, `Cipher: 0000`. То есть **TCP до
  боевого хоста проходит** (файрвола нет, порт открыт, `ServerHello` получен) и упор ровно и только
  в шифронабор — та самая картина, ради которой собран шлюз. Это же снимает вопрос «а достучимся
  ли вообще»: достучимся, как только на нашей стороне появится BTLS.

  ⚠ Практический вывод: **песочный прогон согласия без СКЗИ невозможен в принципе** — упирались не
  в наши параметры. Разблокирует его крипто-шлюз ([#460](https://github.com/bx-shef/client-bank-alfa-by/issues/460),
  собран и смёржен) — порядок включения в `deploy/crypto-gateway/README.md` §«Включение в проде»;
  адреса расщеплены заранее ([#455](https://github.com/bx-shef/client-bank-alfa-by/issues/455)):
  `PRIOR_OAUTH_API_BASE`/`PRIOR_OAUTH_TOKEN_URL` → на шлюз, `PRIOR_OAUTH_AUTHORIZE_BASE` → остаётся
  публичным адресом банка. Переезд на `:9345` — это боевая среда, значит нужна **новая регистрация**
  промышленными ключами и переподключение счетов.
