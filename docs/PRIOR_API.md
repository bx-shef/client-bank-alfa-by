...
- **⛔ Открытый блокер (2026-08-12): `POST /accountConsents` → `403 BY.NBRB.Reauthenticate`**
  ...
  **Решающий замер: дословное тело из официальной Postman-коллекции банка тоже даёт 403.**
  ...
  Итог: отказ — решение банка о правах КОНКРЕТНЫХ регистраций в песочнице, воспроизводимое на двух
  независимых `client_id` с разными методами аутентификации и на эталонном теле. Следующий шаг —
  только вопрос в банк; ставить эксперименты дальше не на чем.
