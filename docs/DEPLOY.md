...
>
> **Ещё один таргет — виртуальная машина Битрикс24 клиента** (приложение рядом с порталом,
> nginx машины терминирует TLS и проксирует в docker, обновления опросом git):
> [`DEPLOY_BITRIXVM.md`](DEPLOY_BITRIXVM.md).
...
Обёртки `Makefile`: полный список — **`make help`**, он же единственный, который не устаревает.
⚠ Перечислять цели здесь мы перестали намеренно: список успел разойтись с `Makefile` дважды, и
читатель узнавал о целях позже, чем они появлялись. Начинать всегда с **`make self-update`** —
`Makefile` кладётся на сервер один раз и сам не обновляется, поэтому новых целей там нет, пока их
не подтянуть; порядок и разбор ошибки `No rule to make target 'self-update'` —
[`OPERATIONS.md`](OPERATIONS.md).

Цели, зовущие скрипты (`doctor`, `queue-stats`, `poll-check`, `prior-probe`, `prior-switch`),
скачивают их с GitHub во временный файл — репозитория в этой папке нет, а сами скрипты в образ не
кладутся.
