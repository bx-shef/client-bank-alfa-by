...
> встанут молча; **`NUXT_PUBLIC_BUILD_DATE`** #425 — build-arg с датой коммита для `<lastmod>` в
> `sitemap.xml` (CI подставляет `git show -s --format=%cs`; пусто ⇒ элемент просто опускается);
> **`TELEGRAM_ALERT_*` на сервисе `backend`** #426 — их надо и добавить в `.env` на
