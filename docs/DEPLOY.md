...
2. На странице приложения (`/app`) каждого портала в блоке «Тестовая настройка (уровень приложения)»
   сохранить **разные** значения (напр. `AAA` на A, `BBB` на B). Значение уходит в `app.option`
   этого портала серверным REST по его токену.
3. Проверить доступ **со стороны сервера** (без браузера) для каждого `member_id`:
   ...
   **Изоляция ОК**, если каждый порт возвращает своё значение и никогда — чужое.
