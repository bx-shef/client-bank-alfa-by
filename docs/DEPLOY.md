...
`B24_APPLICATION_TOKEN` — **опционален**: токен приходит per-portal внутри `ONAPPINSTALL` и хранится
(последующие события сверяются с сохранённым), поэтому для мультитенанта оставляем пустым; задать
стоит лишь как guard-токен для серверных диагностик (`/api/queues`, `app-option-check`).
...
**Очереди (BullMQ) и демо-нагрузка.** Воркеры поднимаются в процессе backend на старте (если задан
`REDIS_URL`). `DEMO_LOAD_N>0` включает демонстрацию: крон каждые `CRON_INTERVAL_MIN` минут кладёт
...
подключения реального опроса выставить `DEMO_LOAD_N=0`. Полноценная телеметрия (Prometheus/Grafana
или bull-board) — отдельный этап (см. `docs/REFACTOR_PLAN.md`).
...
> `:?`→`:-` #53) синхронизировать файл на сервере руками, иначе `docker compose up` падёт на устаревшем.
