...
проходит весь путь и печатает каждый шаг: **OAuth (code → token) → `/accounts/` →
выписка по годам → refresh**. Конфиг берётся из `.env` (см. `.env.example`),
секреты в код/логи не попадают.

```bash
pnpm oauth:test                      # полный поток (откроет браузер для входа)
pnpm oauth:test --from-year 2020 --to-year 2025   # сузить диапазон лет
pnpm oauth:test --account <number>   # только один счёт
pnpm oauth:test --code <authCode>    # пропустить браузер (код уже есть)
pnpm oauth:test --refresh <token>    # только обновление токена
pnpm oauth:test --url-only           # только показать authorize-URL
```

Логирование маскировано: токены и номера счетов в консоли скрыты. Полный
**немаскированный** ответ пишется в `alfa-demo-output.json` (в `.gitignore` —
не коммитим: там реальные данные счёта). Флаг `--full` снимает маскировку и в консоли.
По умолчанию запрашивается `scope=accounts`, период `2000…2029`, `transactions=0`
(все операции), пауза `--delay-ms 700` между запросами (лимит 100/мин).
