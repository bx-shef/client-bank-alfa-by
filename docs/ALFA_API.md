...
>
> Замер на боевом портале: за одни сутки приходило **ровно 100 операций**, четыре дня подряд, при
> настоящих нулях в двух днях между. Капа на нашей стороне нет нигде по пути
> банк→разбор→`crm-sync` — значит сотня приходит от банка.
>
...
>
> Поэтому `fetchAlfaStatementPages` **обходит страницы**, пока очередная не перестанет приносить
> новое (дедуп — по `dedupKey`, тому же ключу, что у маркера дела). Цикл безопасен при **обоих**
> прочтениях, и это его смысл: если `0` правда «все», страница 1 вернёт то же самое, дедуп даст ноль
> нового и мы остановимся после одного лишнего запроса; если «страница» — заберём потерянное и
...
>
> ⚠ Цена: задача Альфы стоит теперь **два** запроса, и лимитер это учитывает (`QUEUES.md`).
>
...
> строке при непустых окнах — значит `0` правда означает «все».
>
