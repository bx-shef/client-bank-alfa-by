...
⚠ Здесь — вход **оператора** (нас) в служебную зону. Права **сотрудников клиента внутри его
портала** — другая ось и другой авторитет, разбор в [`PERMISSIONS.md`](PERMISSIONS.md) (#531).
...
(`/app`, `/import`, `/install`) этой авторизацией **не** закрыты — там своя модель (портал/OAuth).
...
- Пароль пустой ⇒ вход **выключен**. В деве это означает «зона открыта» (удобство), **в проде —
  ...
  **В проде пароль обязателен**. `SESSION_SECRET` в проде тоже **обязателен и независим от пароля**:
  ...
  открывается молча). На старте это видно дважды: не-секретное предупреждение канала `[auth]`
  (`authStartupWarning`, `server/plugins/authGuard.ts`) **и** ошибка канала `[env]` в проверке env
  (оба — через логгер, #529)
...
- **Silent-сессия внутри Bitrix24 (`/session/b24`)**: в оригинале портал валидируется
  ...
  фрейм уже даёт access-токен (`useFrameAuth`) — на его основе можно выдавать сессию;
