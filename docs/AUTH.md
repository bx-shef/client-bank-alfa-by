...
- **Реальная защита — на стороне API**: статический HTML пререндерится и публичен,
  поэтому клиентский `middleware` — это UX-редирект. Сейчас за `/queues` нет реальных
  данных (клиентский demo-генератор), а серверный `/api/queues` закрыт **отдельным**
  guard-токеном (`B24_APPLICATION_TOKEN`, не сессия `cba_sess`) + nginx `deny all`. Когда
  на служебных страницах появятся реальные данные, их обязан отдавать эндпоинт, проверяющий
  сессию `cba_sess`, — полагаться на клиентский `middleware` нельзя.
