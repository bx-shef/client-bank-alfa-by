...
- `server/utils/companyLookup.ts` — поиск компании по счёту (контрагент и «моя компания»); живёт
  вне этого файла исторически, но три находки о ФИЛЬТРАХ B24 замерены именно на нём (тестовый
  портал, 2026-08-21) и распространяются на все списковые методы CRM:
  - ⚠ **неизвестное поле фильтра игнорируется МОЛЧА** — список возвращает ВСЕ строки, ошибки нет
    (`filter: {RQ_TYPO_FIELD: …}` → все реквизиты портала). Разъехавшееся имя поля превращает
    «поиск по счёту» в «первая компания портала», поэтому `resolveCompanyIdsByAccount` сверяет
    каждую возвращённую строку с искомым счётом (`matchingEntityIds`, select запрашивает само
    поле);
  - ⚠ **`%` в ЗНАЧЕНИИ фильтра работает как подстрока** (`%30120A%` матчит живьём). Счёт
    контрагента приходит из выписки — в ручном пути из файла, — и до сверки строк такой ввод мог
    приземлить платёж в чужую компанию;
  ...
  Отдельно: **`RQ_IIK` на BY-портале НЕ записывается** — `crm.requisite.bankdetail.add` с ним
  отвечает 400 (поле в `.fields` есть, фильтр по нему работает и отдаёт пусто). Фолбэк
  `RQ_ACC_NUM→RQ_IIK` живьём безопасен (лишний запрос, не ошибка), но на BY-портале значений там
  не бывает — это поле казахского шаблона.
