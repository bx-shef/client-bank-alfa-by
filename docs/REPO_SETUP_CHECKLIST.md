...
> в этот же список.
>
> Job `visual` (визуальные регресс-тесты, #3) — тоже **не** required и намеренно: пиксельный тест
> на молодых эталонах чаще раздражает, чем помогает, а привычка мержить поверх красного значка
> заразна. Он информативный: краснеет — посмотрите diff-артефакт. Если практика покажет, что
> краснеет он по делу, добавьте `visual` сюда и в `needs` у джобы `deploy` (сейчас деплой ждёт
> только `ci`).
