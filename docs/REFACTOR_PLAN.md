...
8. **Приорбанк + ручная загрузка выписок** (два новых источника рядом, поверх абстракции
   ...
   свой шлюз, проверен вживую (#477), СКЗИ покупать не нужно, #41 **закрыт**. ⛔ **Осталось и
   заблокировано банком:** он отказывает на создании согласия (`403 BY.NBRB.Reauthenticate`),
   из-за чего стоит и sandbox, и прод — см. `PRIOR_API.md`, «Открытый блокер».
