...
| Фоновая обработка | **BullMQ + Redis** (не Nitro tasks) — под нагрузку и масштабирование: очереди `b24-events`, `bank-fetch`, `file-parse`, `crm-sync` (анализ + запись в B24). Дедуп ретраев по детерминированному `jobId`; Redis — на изолированной сети `queuenet`. **Фаза 1** (сервис+контракты) и **Фаза 2** (продюсеры, воркеры in-process, крон+демо-нагрузка, наблюдаемость `/api/queues`) — готовы; транспорты в обработчиках — заглушки до стадий 3–6; масштаб-аут (отдельный воркер-контейнер) и телеметрия в Grafana — далее. Техсправка (топология, поток, метрики) — [`QUEUES.md`](QUEUES.md) |
...
3. **Backend PoC: OAuth Альфы + выписка.** Чистое ядро (сборка `/authorize`, обмен/обновление
   ...
   **Очереди (BullMQ + Redis)** — **Фаза 1 + 2 готовы**: сервис `redis` (сеть `queuenet`),
   ...
   с DI — тесты), воркеры in-process + крон с **демо-нагрузкой** (`worker.ts`/`cron.ts`/
   `server/plugins/queue.ts`), 4-я очередь `crm-sync` (анализ→действие в B24), наблюдаемость
   `GET /api/queues` + `scripts/queue-stats.sh`. Транспорты в обработчиках — заглушки; реальная
   логика наполняется на стадиях 4–6 ниже. Масштаб-аут (отдельный воркер-контейнер) и телеметрия
   в Grafana — отдельные шаги.
4. **Поиск компании по корр-счёту + запись универсального дела** (перенос на todo-API).
   Воркер `crm-sync` → поиск компании → `crm.activity.todo.add`.
   **БЛОКЕР до включения реальной записи:** очереди BullMQ — at-least-once, поэтому дедупа
   ВНУТРИ батча (в памяти джоба, `handlers.ts`) недостаточно — при падении/ретрае воркера батч
...
9. **Приорбанк + ручная загрузка выписок** (два новых источника рядом, поверх абстракции
   ...
   **Осталось:** прод-СКЗИ — #41; серверный движок Приора (backend) на базе `priorOauth.ts` — далее.
   ...
   `app/utils/manualImport.ts`) — `***** ^Type=` (`normalizeClientBank`, проверен и на реальных
   `Type=4`: BYN для 13-значных счетов + фолбэк дедупа `Num|DocDate` без `DocID`) и **1С-обмен**
