...
2. **[~ этап 2, частично] B24 dual-mode + SDK.** Подключены `@bitrix24/b24jssdk` (+ `-nuxt`),
   ...
   **Критерий приёмки при добавлении `placement.bind`:** перенести из `currency-converter` guard
   «отказ биндить относительный handler-URL» (требует непустой `NUXT_PUBLIC_SITE_URL` в проде —
   убедиться, что repo-variable заведена), иначе портал сохранит битую относительную привязку.
3. **Backend PoC: OAuth Альфы + выписка.** Чистое ядро (сборка `/authorize`, обмен/обновление
   ...
   **Авторизация портала + события Б24** (`ONAPPINSTALL`/`ONAPPUNINSTALL`, `application_token`):
   ...
   Транспорт (эндпоинт вебхуков, хранилище токенов с шифрованием refresh, refresh-цикл) — на backend.
   Контракт и модель (по эталону `bx-synapse`) — [`B24_EVENTS.md`](B24_EVENTS.md). См. #12 (безопасность).
