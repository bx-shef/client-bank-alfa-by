...
8. **Приорбанк + ручная загрузка выписок** (два новых источника рядом, поверх абстракции
   ...
   async create+poll), connect-поток и автоопрос — тоже. **Осталось:** живой sandbox-прогон на
   кредах владельца и прод-СКЗИ — #41.
...
юнит-тестами. **Приор-прод** дополнительно требует СКЗИ (BY-крипто TLS) на `:9345` — issue #41,
**не закрыт**; это крипто-требование РБ, не сетевое ограничение. Кодовая часть под шлюз — **сделана**
(issue #455): `PRIOR_OAUTH_AUTHORIZE_BASE` отделяет адрес, который открывает браузер администратора,
от адреса, куда ходит backend; совмещение убивало подключение молча. ⚠ **Применимость под вопросом:**
это написано под шлюзовую модель, а найденный кандидат — диспетчерный прокси, которому подмена адреса,
вероятно, не нужна (см. `PRIOR_API.md`, «Следствие для интеграции»). Прод-метод аутентификации
