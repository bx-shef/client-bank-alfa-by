...
креды в git-ignored `.env.priorbank` (client_id/secret, зарегистрированный `redirect_uri`) и, для
Альфы, `ALFA_OAUTH_*` в `.env` сервера — их держит владелец, поэтому живой раунд гоняется на деплой-сервере
...
- **Доступы Альфы (sandbox) получены:** `client_id`, `client_secret` + свагеры. Секреты — только в
  env сервера, не в репозитории. ⚠ `redirect_uri` больше не нужен: authorize-поток Альфы убран
  (#488), подключение идёт **ключом API**, который владелец счёта генерирует у себя в кабинете.
