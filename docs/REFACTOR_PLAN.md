...
> **⚠ Актуальная карта состояния — [`project-map.md`](project-map.md)** (обновляется чаще этого плана),
> **целевая спека обработки платежа — [`PROCESSING.md`](PROCESSING.md)**, **эксплуатация — [`OPERATIONS.md`](OPERATIONS.md)**.
> Крупные вехи с момента последней ревизии этого файла: **контур распределения §9 (§109) закрыт кодом** —
> провижининг payment/dist-СП, транспорт леджера, UI-вкладка, пайплайн удаления, полный ретайр
> Postgres-`allocation_fact` (идемпотентность на маркере dist-СП); распознавание/резолв/проводка платежа в
> `crm-sync` доведены **до записи** (мутация за гейтом `autoDistribute`), часть путей **live-verified** на
> `bel.bitrix24.by` (регистрация+firing триггера, стадия инвойса apply/revert, IDOR-скоуп СП, мост
> `via-document`). **Не пройдено вживую:** сквозной прогон на боевой нагрузке (#501), провижининг+запись
> леджера на портале с платным тарифом СП, онлайн-опрос Альфы/Приора (за банк-кредами владельца). Разделы
> «этапы/осталось» ниже отражают более раннее состояние — сверяйтесь с `project-map.md`.
...
2. **[~ этап 2, частично] B24 dual-mode + SDK.** Подключены `@bitrix24/b24jssdk` (+ `-nuxt`),
   `useB24` (init/no-op вне фрейма), layout `portal`/`operator` (`<B24App>` через `AppShell`), `/install` (`init → installFinish`
...
4. **Поиск компании по корр-счёту + запись настраиваемого дела.**
   ...
   **Статус — стадия 4 в основном готова:** поиск компании (`companyLookup.ts`), read-before-write
   ...
   портале (#501); обработка `unmatched`-операций **сделана** (#91 закрыт). **Rate-limit REST на `crm-sync`** — встроенный
...
8. **Приорбанк + ручная загрузка выписок** (два новых источника рядом, поверх абстракции
   ...
   Серверный движок Приора на базе `priorOauth.ts` — **сделан** (`server/utils/priorFetch.ts`:
   async create+poll), connect-поток и автоопрос — тоже. BY-крипто канал к проду тоже есть —
   свой шлюз, проверен вживую (#477), СКЗИ покупать не нужно, #41 **закрыт**. ✅ **Блокер согласия
   снят 2026-08-14** — правка банка действовала только на клиентов, зарегистрированных после неё,
   поэтому повтор старого запроса её не проверял; новая DCR-регистрация проходит, и подключение
   пройдено насквозь боевым путём приложения (`PRIOR_API.md`, «Закрытый блокер»). Осталось
   владельческое: промышленные ключи и включение крипто-шлюза.
...
9. **Контур обратной связи (клиенты/сотрудники + ИИ).** Замкнуть петлю «пользователь → задача →
    ...
    - **MCP по выписке** — вынесен в [`IDEAS.md`](IDEAS.md) (не часть MVP): ИИ-доступ к уже внесённым данным
      (анализ через смарт-процесс) и отдельно — прямой клиент-банкинг (отдельное приложение).
...
юнит-тестами. **Приор-прод** говорит по BY-крипто TLS на `:9345` — это крипто-требование РБ, не
сетевое ограничение, и оно **закрыто своим шлюзом** (issue #41 закрыт; канал проверен живым
прогоном против боевого банка, #477 — покупать СКЗИ не нужно). Кодовая часть — **сделана**
(issue #455): `PRIOR_OAUTH_AUTHORIZE_BASE` отделяет адрес, который открывает браузер администратора,
от адреса, куда ходит backend; совмещение убивало подключение молча. ⚠ Модель именно **шлюзовая**;
прежняя оговорка про «диспетчерный прокси» снята — тот кандидат (AvTunProxy) отменён вендором. Прод-метод аутентификации
`private_key_jwt` (issue #444) — **сделан**, переключается `PRIOR_OAUTH_AUTH_METHOD`.
Альфа-прод не требует ни того, ни другого.
Сертификат ГосСУОК **не нужен** — он обязателен только для метода `tls_client_auth`, который мы не
используем (сверено с инструкцией Приорбанка, см. `PRIOR_API.md`).
...
- **Сделано (#16):** настройки живут в `app.option` портала и читаются/пишутся через backend
  (`/api/chat-settings`, `useChatSettings`). Прежняя демо-заглушка на `localStorage` и отдельная
  страница `/settings` — целевой экран слайдера портала: `/app` открывает её `openSliderAppPage`,
  вне портала — обычной навигацией. (Прежний самодельный `B24Slideover` внутри `/app` снят.)
