...
> Цели `make doctor` / `make queue-stats` из этой таблицы — **на сервере**, из
> `/home/bitrix/bank-import`. ⚠ Не «любая цель `make` — серверная»: в том же `Makefile` есть и
> дев-цели (`make dev`, `make build-local`), они работают в клоне репозитория — см.
> [`DEPLOY.md`](DEPLOY.md).
>
...
> ⚠ Берётся `main`, а не то, что развёрнуто: обычно совпадает, а когда важна точность — разовая
> команда `curl` с явным адресом коммита. **Параметром `make` это задать нельзя намеренно**
> (`override REF := main`): значение параметра раскрывается ДО шелла, поэтому
> `make -n doctor REF='$(shell …)'` выполняет код прямо в режиме «только показать», а
> `REF='../../чужой/репо/main'` уводит скачивание в чужой репозиторий (curl схлопывает `/../` сам,
> до отправки). Обе дыры проверены вживую на ревью PR #535.
>
> Расхождение «рантбук зовёт одно, `Makefile` умеет другое» стережёт `tests/serverCommands.test.ts`
> (несуществующая цель в доке, скачиваемый скрипт без файла в `scripts/`, возврат `bash scripts/…`),
> а разбор `./.env` — `tests/makefileEnvValue.test.ts` (прогоняет настоящий `make`).
