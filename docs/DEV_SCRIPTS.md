...
| `bash scripts/by-ca-bundle.sh` | собрать bundle корней ГосСУОК и проверить им сертификат банка (#459) | только чтение; **запускать из Беларуси** — `nces.by` отдаёт 503 чужому трафику |
...
- `scripts/by-ca-bundle.sh` — собирает **bundle корней ГосСУОК** и проверяет им сертификат
  `apibel.priorbank.by` (issue #459). Живой прогон (#457) закончился на
  `Verify return code: 21`: канал шифруется, а собеседник не подтверждён — в прод так нельзя.
  Сертификат банка выписан **«Республиканский удостоверяющий центр ГосСУОК»** (РУП «НЦЭУ»), и сервер
  шлёт **только свой лист**, без промежуточного, поэтому bundle обязан нести и промежуточный (РУЦ),
  и корень (КУЦ).
  Скрипт **не гадает по именам файлов**: берёт `issuer_hash` сертификата банка, ищет среди скачанных
  кандидатов совпадающий `subject_hash` и так поднимается по цепочке до самоподписанного корня —
  `ruc3.cer` не обязан быть текущим издателем, а ошибка вылезла бы только на живой проверке, далеко
  от причины. Печатает **сроки годности** каждого звена (это ранняя тревога, см. #462) и sha256
  bundle.
  ⚠ **Запускать из Беларуси:** `nces.by`/`pki.gov.by` отвечают **503** зарубежному трафику — именно
  поэтому шаг нельзя выполнить из dev-контейнера. Остальные BY-источники (`avest.by`,
  `portal.nalog.gov.by`, `cdn.belpost.by`) доступны, но там лежат **другие** УЦ (МНС, Белпочта,
  STEND-иерархия) — проверено, не подходят.
  ⚠ Это **не** клиентский сертификат: ГосСУОК-ключ, носитель и бухгалтер не участвуют. Здесь только
  публичные сертификаты УЦ, которыми проверяется **банк**.
