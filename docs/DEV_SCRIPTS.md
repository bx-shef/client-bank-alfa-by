...
| Команда | Что делает | Чем опасна |
|---|---|---|
| `pnpm oauth:test` | живой прогон OAuth/выписки Альфы (sandbox) | только чтение |
| `pnpm prior:test` | то же для Приорбанка (Open Banking СПР) | только чтение |
| `pnpm parse:statement <файл>` | разбор файла выписки в консоль | безопасно, без сети |
| `pnpm fuzz:allocation [seed] [N]` | фузз алгоритма разнесения на синтетике | безопасно, без сети |
| `pnpm verify:109` | живой READ-прогон разнесения на seed-данных портала | только чтение |
...
| `bash scripts/bee2evp-probe.sh` | проверка открытого BY-крипто стека (bee2evp) против прод-хоста Приорбанка (#457) | только чтение; собирает OpenSSL из исходников (долго), **нужна сеть до банка** |
...
- `scripts/bee2evp-probe.sh` — проверка **открытой** реализации белорусской криптографии
  ([bee2evp](https://github.com/bcrypto/bee2evp), патч BTLS к OpenSSL) против прод-хоста Приорбанка
  (issue #457). Отвечает на один вопрос: можно ли обойтись без проприетарного СКЗИ АВЕСТ, от которого
  сейчас зависит весь прод-путь Приора (#41) и который упирается в подписанный список ресурсов, нам
  недоступный. Основание проверять: AvTunProxy согласовал с `openapi.mtbank.by` шифронабор
  **65303 = `0xFF17`**, а в патче bee2evp этот же id — `BDHT-BIGN_WITH-BELT-CTR-MAC-HBELT`, то есть
  открытый стек реализует **ровно тот набор**, на котором работает банковская инфраструктура РБ.
  Скрипт сам делает **контрольный замер обычным OpenSSL** до сборки: без него успех ничего не
  доказывает (хост мог начать принимать обычный TLS), а провал не отличить от «нет сети».
  ⚠ Запускать **с машины, у которой есть сеть до банка** — из dev-контейнера порт 9345 недоступен.

  **✅ ПРОЙДЕНО ВЖИВУЮ (2026-08-12, прод-сервер Debian 11):**

  ```
  New, TLSv1.2, Cipher is DHT-BIGN-WITH-BELT-CTR-MAC-HBELT
  SSL handshake has read 1214 bytes and written 498 bytes
  Verify return code: 21 (unable to verify the first certificate)
  ```

  Открытый стек **согласовал СТБ-шифр с продом Приорбанка** (`0xFF17`) и получил полную цепочку
  сертификатов — 1214 прочитанных байт против 90 у обычного OpenSSL, который на том же хосте
  обрывается на `ServerHello`. Контрольный замер в том же прогоне подтвердил, что обычный TLS
  по-прежнему не согласуется, то есть результат не объясняется «хост стал принимать обычный TLS».

  ⚠ `Verify return code: 21` — это **не** ошибка крипто, а отсутствие корней в доверенных:
  сертификат банка выписан белорусским УЦ (ГосСУОК/НЦЭУ), их надо положить в `-CAfile`. **Это
  обязательный шаг, а не косметика:** пока проверка не включена, соединение шифруется, но
  собеседник не подтверждён. Клиентский сертификат при этом по-прежнему **не нужен** — банк
  подтвердил, что достаточно серверной аутентификации.

  **Флаг `--cafile <bundle>`** (#459) включает проверку сертификата сервера. Вердикт печатает
  **две независимые строки** — «шифр согласован» и «сертификат проверен»: слипание этих состояний и
  есть способ объявить готовым то, что шифруется, но не аутентифицировано. Раздел 5 вывода печатает
  цепочку, которую прислал сервер, и — из расширения AIA — **адрес, откуда качать издателя**: иначе
  вопрос «какой корень нам нужен» превращается в перебор `nces.by` / `avest.by` / `belpost` /
  `nalog.gov.by` вслепую.

  ⚠ Успех проверки **не равен разрешению использовать**: годится ли НЕсертифицированное средство под
  СПР 6.02 — вопрос юридический, задаётся банку письменно (см. #457).
