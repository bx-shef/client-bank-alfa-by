...
| `pnpm probe:sp-filter` | дев | фильтр `companyId` на СП без привязки к клиенту (#572) | **пишет** в портал: создаёт СП + элемент, удаляет в `finally` |
| `pnpm verify:registry` | дев | реестр платежей (#575): самолечение полей + запись + чтение обратно | **пишет** в портал, есть teardown |
| `pnpm verify:bindings` | дев | привязки дела (#579): запись + чтение `binding.list` обратно + повтор | **пишет** в портал (две компании, сделка, дело), есть teardown |
...
- `scripts/verify-registry-live.ts` (`pnpm verify:registry` / `--oauth` / `--keep`) — **живой прогон
  реестра платежей (#575)**: `provisionDistributionSp` (создание ИЛИ самолечение уже существующего СП) →
  `writePaymentRegistryViaRest` → `crm.item.get` с проверкой всех восьми полей реестра → повтор на
  идемпотентность → удаление элемента. ⚠ Шаг чтения обратно и есть смысл скрипта: `crm.item.add`
  **молча игнорирует неизвестный UF-ключ** (замерено 2026-08-22), поэтому опечатка в имени поля не
  падает — значение просто исчезает, и юнит-тест этого поймать не может. Прогнан вживую 2026-08-22:
  13/13, самолечение довезло 8 полей на СП, созданный до #575; повторный прогон — 0 добавленных.
  ⚠ Дата сверяется по КАЛЕНДАРНОЙ ЧАСТИ: поле типа `date` не возвращает присланную строку, портал
  нормализует её в свой часовой пояс (`2026-08-21` → `2026-08-21T03:00:00+03:00`). Скрипт поймал это
  сам — первая версия сверяла байт-в-байт и упала на верных данных.
  ⚠ Операция **синтетическая** и обязана такой остаться: файл лежит в ПУБЛИЧНОМ репозитории и пишет в
  портал, реальная строка выписки опубликовала бы счёт, УНП и назначение контрагента (`PRIVACY.md`).
  ⚠ Вебхуку `.env.b24test` не хватает права `userfieldconfig` — нужен `--oauth`. В этом режиме
  скрипт **перезаписывает** `.env.b24oauth` обновлённой парой токенов: Bitrix24 ротирует refresh на
  каждом обновлении, и без записи файл протухал бы после первого же прогона.
- `scripts/verify-bindings-live.ts` (`pnpm verify:bindings` / `--oauth` / `--keep`) — **живой прогон
  привязок дела (#579)**: провижининг СП → посев двух компаний и сделки → настоящие писатели
  (`writePaymentRegistryViaRest`, `writeTodoActivityViaRest`) → `planActivityBindings` →
  `bindActivityViaRest` → `crm.activity.binding.list` обратно → повтор → уборка. Прогнан вживую
  2026-08-22: 13/13.
  ⚠ Три факта, ради которых он и написан, из документации не выводятся:
  **повторная привязка той же пары — ОШИБКА** (`ACTIVITY_IS_ALREADY_BOUND`), поэтому транспорт
  читает список перед поштучным повтором; **привязка к несуществующей сущности отвечает
  `{result:true}`**, то есть успешный ответ ничего не доказывает и читать обратно обязательно; и
  **пара владельца висит на деле сама** — её привязывать не надо.
  ⚠ Сущность списания проверяется на ОДНОМ виде — сделке. Остальные (`invoice`/`smart-process`/
  `deal-payment`) держатся юнит-тестами: оба замеренных свойства метода от вида сущности не зависят,
  а сеять по элементу каждого вида значит писать в живой портал вчетверо больше ради того же ответа.
  ⚠ Как и у реестра, вебхуку `.env.b24test` не хватает права `userfieldconfig` — нужен `--oauth`.
  ⚠ Всё создаваемое **синтетическое**: файл в ПУБЛИЧНОМ репозитории и пишет в портал.
- `scripts/probe-sp-company-filter.ts` (`pnpm probe:sp-filter` / `--keep`) — **замер #572**: что
  делает `crm.item.list` с фильтром `companyId` на смарт-процессе БЕЗ привязки к клиенту
  (`isClientEnabled: false`). Вопрос был не праздный: комментарий в `intentResolver` предупреждал,
  что портал может ПРОИГНОРИРОВАТЬ незнакомый ключ фильтра и скоуп по компании «раскроется», а
  триггер-цели уже подключены к настоящей мутации.
  ⚠ **Результат (2026-08-23): FAIL-CLOSED** — фильтр принимается и отдаёт `items: []`, тогда как без
  фильтра тот же вызов возвращает элемент. То есть чужие строки не возвращаются, и предупреждение
  было неверным. ⚠ Зато вскрылось другое: на таком СП фильтр не совпадёт НИКОГДА, и админ, указавший
  его в «карте сопоставления», получит вечное «цель не найдена» без подсказки.
  ⚠ Скрипт **пишет** в портал (СП + элемент) и убирает за собой в `finally`, громко сообщая, чего
  убрать не смог: оставленный смарт-процесс в CRM клиента — именно тот мусор, которого этот
  репозиторий не оставляет. Базовый замер «без фильтра видно 1 элемент» обязателен — без него
  «фильтр сработал» неотличимо от «СП был пуст».
