...
- **Скрипты крипто-разведки переехали.** `bee2evp-probe.sh` (проба открытого BY-крипто стека против
  ...
  [bx-shef/bee2-tls-gateway](https://github.com/bx-shef/bee2-tls-gateway), вместе со скриптами. У нас
  остался только результат: замер и его следствия — [`PRIOR_API.md`](PRIOR_API.md), раздел про СКЗИ.
