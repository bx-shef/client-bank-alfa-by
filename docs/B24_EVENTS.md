...
- **CRM-события удаления** (`ONCRMDEALDELETE`/`ONCRMCOMPANYDELETE`/`ONCRMDYNAMICITEMDELETE`,
  ...
  смарт-процессов `ONCRMDYNAMICITEMDELETE` просто не фаерится). Пока консьюмер пайплайна
  удаления не подключён, эндпоинт их **200-подтверждает и игнорирует** (ветка `ignored`),
  так что B24 не ретраит и не отключает приложение; маршрутизация в `DeletionJob` — отдельный слайс.
