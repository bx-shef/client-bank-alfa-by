...
| Стор дедупа дел `{dedupKey→activityId}` (Postgres, скоуп по порталу, always-purge) | `server/utils/activityDedupStore.ts`, таблица `activity_dedup` (#9) | **готово** (проводка в crm-sync — стадия 4) |
