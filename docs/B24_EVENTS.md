...
| Миграция схемы (`portal_tokens`, `portal_tombstone`, `activity_dedup`, `allocation_fact`, `import_result`, `metrics_counter`) на старте | `server/plugins/migrate.ts` | **готово** |
...
Таблицы `portal_tokens`, `portal_tombstone`, `activity_dedup`, `allocation_fact`, `import_result` и
