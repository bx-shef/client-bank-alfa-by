...
Правила (как в `bx-synapse`, см. также issue #12):
...
| Слой | Где | Статус |
|---|---|---|
| Разбор события, вердикт токена, маршрутизация, SSRF-гуард, дедуп-крендов | `app/utils/b24Events.ts` (чистое, тесты) | **готово** |
| Типы события/кредов | `app/types/b24Events.ts` | **готово** |
| HTTP-эндпоинт `POST /api/b24/events`, хранилище токенов (БД, шифрование refresh), refresh-транспорт, регистрация хендлеров (`event.bind`) | backend-сервис | этапы 3–6 плана |
...
> (исходящие вебхуки на backend), и именно он даёт `application_token`. Регистрация
> обработчика события указывается в карточке версии приложения / `event.bind`.
...
- #12 — безопасность транспорта Альфы (секреты, шифрование refresh, timing-safe state).
