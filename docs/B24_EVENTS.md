...
3. **Решение → очередь → консьюмер (эндпоинт в БД не пишет).** `processB24Event`
   ...
   Роут **кладёт `action` пакетом в очередь `b24-events`** (`refresh_token` шифруется
   перед Redis — в открытом виде через брокер не ходит), а **консьюмер** (`handleEventJob`)
   — единственный писатель: регистрирует портал (`saveToken`, write-once `application_token`)
   или удаляет (`deleteToken`). Так приём быстрый, запись — асинхронная и ретраится BullMQ.

   **Очередь обязательна:** если пакет не удалось положить (Redis нет/упал), роут отвечает
   **503**, чтобы Б24 повторил вебхук — событие не теряется. (Синхронной записи в БД из роута
   больше нет — прод-`docker-compose` обязан поднимать Redis + `REDIS_URL`.)
