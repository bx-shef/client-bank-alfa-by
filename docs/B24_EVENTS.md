...
| Миграция схемы (`portal_tokens`, `activity_dedup`) на старте | `server/plugins/migrate.ts` | **готово** |
...
Таблицы `portal_tokens` и `activity_dedup` создаются автоматически на старте (`server/plugins/migrate.ts`).
