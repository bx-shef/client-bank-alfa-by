...
| Refresh-цикл access-токена, REST-вызовы к порталу, регистрация хендлеров (`event.bind`) | backend | этапы 4–6 плана (#35) |
| `installFinish` + диагностика в iframe | `app/pages/install.vue` | готово (этап 2) |

> Фрейм-страница `/install` завершает установку в iframe (`installFinish`).
> Серверные события (`ONAPPINSTALL`/`ONAPPUNINSTALL`) — **отдельный** механизм
> (исходящие вебхуки на backend `/api/b24/events`), и именно он даёт `application_token`.
> Регистрация обработчика события указывается в карточке версии приложения / `event.bind`.
...
- `B24_APPLICATION_TOKEN` — `application_token` зарегистрированного приложения, **обязателен в проде**.
...
3. **Регистрация обработчика события.** В карточке локального приложения на портале указать
   URL обработчика событий установки/удаления = адрес из п.1 (или разово вызвать `event.bind`
   на `ONAPPINSTALL`/`ONAPPUNINSTALL`). Точную процедуру фиксируем на тестовом портале (#35).
...
- **#35** — backend: эндпоинт вебхуков B24 + хранилище токенов портала (реализация этой карточки).
