...
- **Удаление приложения → полная очистка (политика).** Любой легитимный `ONAPPUNINSTALL`
  стирает всё, что связано с порталом (строка `portal_tokens` **и** строки дедупа дел
  `activity_dedup` по `member_id`); флаг `CLEAN` не смотрим — удалили приложение, значит не
...
| Стор дедупа дел `{dedupKey→activityId}` (Postgres, скоуп по порталу, always-purge) + read-before-write в `crm-sync` | `server/utils/activityDedupStore.ts`, таблица `activity_dedup` (#9), `handlers.ts` | **готово** (live-запись в CRM — стадия 4) |
| Миграция схемы (`portal_tokens`, `portal_tombstone`, `activity_dedup`, `allocation_fact`, `import_result`) на старте | `server/plugins/migrate.ts` | **готово** |
...
| REST-вызовы к порталу (дела/чат/поиск) | `server/utils/{portalRest,crmActivityWrite,chatNotifyWrite,companyLookup}.ts` | **готово** (стадии 4/6, ядра+тесты; живьём — далее) |
...
Таблицы `portal_tokens`, `portal_tombstone`, `activity_dedup`, `allocation_fact` и `import_result` создаются
автоматически на старте (`server/plugins/migrate.ts`).
