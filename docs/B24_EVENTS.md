...
- **Удаление приложения → полная очистка (политика).** Любой легитимный `ONAPPUNINSTALL`
  ...
  `allocation_fact`/`import_result`/`metrics_counter` по `member_id`; дедуп дел живёт в B24,
...
| Миграция схемы (`portal_tokens`, `portal_tombstone`, `allocation_fact`, `import_result`, `metrics_counter`) на старте | `server/plugins/migrate.ts` | **готово** |
...
Таблицы `portal_tokens`, `portal_tombstone`, `allocation_fact`, `import_result` и
