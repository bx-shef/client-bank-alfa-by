...
Владелец 2026-09-06 решил ровно три вещи: **коммиты — по-русски**,
**комментарии — по-русски**, **`releasing.md` — пропускаем**. Первые две меняют
строки таблицы, третья снимает ссылку в §4.1. Четвёртая правка (новый документ
→ русский) — вывод агента, а не решение владельца: она лишь приводит таблицу в
согласие с уже действующим жёстким правилом `CLAUDE.md` («документация — на
русском»), которое иначе противоречило бы ей в лоб. Замеры — на день адаптации,
...
- **Новый документ: English → русский** (вывод агента, не решение владельца).
  Исходная строка противоречила жёсткому правилу `CLAUDE.md` («пользовательский
  текст, README и документация — на русском»): эти документы читает владелец и
  клиент, и английский новый документ был бы недоступен адресату. Расхождение
  названо здесь, а не решено молча.
