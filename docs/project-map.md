...
| 5 | Опрос банков (cron, фильтры) | 📥 забор/разбор · 🏗️ движок | 🏗️ (A5 транспорт + A9 свап + A6 реестр + A10 таймер + A7 connect + A8 rate-limiter готовы; Приор A5b — движок+connect+автоопрос готовы; осталось банк-креды владельца) |
...
- **За владельцем (живая проверка):** B1/B2 sandbox-раунд (`pnpm oauth:test`/`prior:test` на деплое),
  B4 реальный redirect-consent, B5 e2e fetch→запись в Б24, B3 Приор-прод АВЕСТ-шлюз (issue #41;
  прод-метод аутентификации #444 — сделан), B6 —
...
- **Приорбанк прод — BY-crypto TLS**: **транспорт свой, вендор больше не нужен.** Открытая
  реализация bee2evp (Apache 2.0) согласует тот же шифронабор `0xFF17` и проверяет сертификат банка
  корнями ГосСУОК — **доказано живым прогоном против прода** (#457, #459). Модель — **шлюзовая**
  (прежняя оговорка про «диспетчерный прокси» снята: диспетчерным был кандидат АВЕСТ, от которого
  ушли). Образ собран — #460, вынесен в [bx-shef/bee2-tls-gateway](https://github.com/bx-shef/bee2-tls-gateway) (nginx + пропатченный OpenSSL, три гейта
  сборки, smoke-запуск в CI, публикация в GHCR); сервис в compose закомментирован, включается
  осознанно. Кодовая половина адресов — #455 (`PRIOR_OAUTH_AUTHORIZE_BASE`, `bankGatewayUrl.ts`).
  Ключ ГосСУОК и аппаратный носитель в схеме **не участвуют** — аутентификация банка односторонняя.
  **Осталось:** живой прод-прогон (#461), ротация корней (#462), ручное слежение за пинами (#470).
  ...
  Прод-метод аутентификации `private_key_jwt` (issue #444) — **сделан**; на sandbox не требуется
  ни то, ни другое.
