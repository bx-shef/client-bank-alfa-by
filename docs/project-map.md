...
Причина не в объёме кода: приложение собрано от края до края. Причина в двух внешних вещах —
отказ Приорбанка на создании согласия (ниже) и то, что путь через вендорское СКЗИ отпал, а свой
шлюз владелец решил **сертифицировать самостоятельно**. Оба срока задаются не нами.
...
- ✅ **Блокер согласия Приора снят (2026-08-14), песочница пройдена насквозь из портала.** Месячный
  ...
  Разбор — [`PRIOR_API.md`](PRIOR_API.md), «Закрытый блокер». Осталось владельческое: промышленные
  ключи и включение крипто-шлюза (порядок — [`OPERATIONS.md`](OPERATIONS.md)).
