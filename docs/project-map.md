...
- **Обработка платежей по спецификации** [`docs/PROCESSING.md`](PROCESSING.md) — **REST-слайс проводки**
  ...
  **Отсев FAIL-стадий смарт-процесса — СДЕЛАН:** `negativeStages` юнионит FAIL-стадии СП, когда настроен
  ...
  стадии не грузятся (поведение прежнее). Осталось **UI «карты сопоставления»** (матрицы/`configFields` пока только через `app.option` — ключи/
  форма блоба задокументированы в `PROCESSING.md §4`, но формы нет). **Мост `via-document` — ПОДКЛЮЧЁН + LIVE-VERIFIED**
