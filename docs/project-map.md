...
- **Приорбанк прод — СКЗИ (BY-crypto TLS)**: нужен крипто-шлюз (СКЗИ, issue #41) — **не закрыт**
  (продукт не выбран: вендор рекомендует AvTLSSrv, кандидат AvTunProxy; Linux/носитель/лицензия —
  вопросы к АВЕСТ). **Кодовая половина — сделана** (issue #455): адреса «backend→шлюз» и
  «браузер→банк» расщеплены (`PRIOR_OAUTH_AUTHORIZE_BASE`), правила схем/хостов —
  `app/utils/bankGatewayUrl.ts`, заготовка сервиса в `docker-compose.prod.yml`.
  ⚠ **Применимость под вопросом:** #455 написан под **шлюзовую** модель, а найденный кандидат
  (AvTunProxy) — **диспетчерный прокси**, для которого подмена адреса, вероятно, не нужна вовсе
  (интеграция через `ProxyAgent` при неизменном публичном URL банка). Проверки `envCheck` полезны
  независимо от модели, заготовка `crypto-gw` — под вопросом. Разбор — `PRIOR_API.md`, «СКЗИ».
  **Крипто-совместимость с сервером банка доказана** (#41): AvTunProxy проводит СТБ-рукопожатие и
  разбирает сертификат банка; не хватает записи в списке ресурсов вендора — заявка подана.
  Открытая альтернатива без вендора — bee2evp, проверяется в #457 (не блокер).
