...
| 3b| OAuth банков (Альфа/Приор) | 📥 | Альфа ✅ connect вживую · Приор 🧪 + ⛔ (согласие) · BY-крипто канал к проду ✅ |
...
| 5 | Опрос банков (cron, фильтры) | 📥 забор/разбор · 🏗️ движок | 🏗️ (A5 транспорт + A9 свап + A6 реестр + A10 таймер + A7 connect + A8 rate-limiter готовы; Приор A5b — движок+connect+автоопрос готовы. **Альфа: осталось подключить счёт и довести до записи в CRM — ни от кого не зависит. Приор: ⛔ банк отказывает на согласии**) |
...
- **Живые прогоны банков — на деплой-сервере с реальными кредами**: геоблок-ограничения нет,
  Альфа достижима целиком (sandbox и прод), Приор — упирается в блокер выше. Живой OAuth/выписку
  гоняет владелец на деплое (`.env.alfabankby`/`.env.priorbank`), т.к. креды у него; в dev-сессии —
  чистое ядро под юнит-тестами.

⚠ **Альфа блокером НЕ затронута.** Вся машинерия опроса собрана (A5/A6/A7/A8/A9/A10), connect-поток
подтверждён вживую (authorize → callback → счёт сохранён), креды у владельца есть. Не сделан
единственный шаг — подключить реальный счёт из портала и довести опрос до записи в CRM. Это ближайшая
работа и она ни от кого не зависит.
