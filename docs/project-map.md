...
`sale`-скоуп добавлен); **мост `via-document` — ПОДКЛЮЧЁН + LIVE-VERIFIED** (#109: `document-number` →
`findDocumentEntities` → `routeDocumentRef` → `findCandidateById` со скоупом по компании; `pnpm verify:109` #8;
`documentgenerator` в `B24_REQUIRED_SCOPES`); чат-уведомления стадии 6 вживую.
...
приложения — актуальный % и разбивка по блокам в `docs/WORKLOG.md` (канонический срез; hardening повышает
...
| 5 | Опрос банков (cron, фильтры) | 📥 забор/разбор · 🏗️ движок | 🏗️ (A5 транспорт + A9 свап + A6 реестр + A10 таймер + A7 connect + A8 rate-limiter готовы; Приор A5b — движок+connect готовы, автоопрос выключен; осталось банк-креды владельца) |
| 6 | Оповещения: дело + чат по правилам | 🏗️ | 🧪 ядро; настройки #16 ⚠ (спека: чат-бот, получатель ошибок) |
| 7 | Docker + деплой + CI | 🏗️ | ✅ |
| 8 | Приор + ручная загрузка файла | 📥 разбор · 🏗️ UI | 🔄 (разбор 🧪; UI+предпросмотр P4-слайс1 🧪; запись в CRM P4-слайс2 🧪 — `POST /api/import`→очередь, вживую не гонялось) |
| 9 | Обратная связь клиент/ИИ | 🏗️ | ⬜ (план #61/#108) |
...
(#90) + провижининг/запись СП-леджера — за живым порталом с платным тарифом СП. Из живого работает
...
`userfieldconfig` в `B24_REQUIRED_SCOPES` → ре-consent) + сквозной прогон всей цепочки (#90), включение Приора в автоопрос (движок+connect готовы), телеметрия в Grafana
...
- **Серверный движок опроса** (Альфа/Приор): транспорт Альфы живой (A9), реестр счетов (A6) + живой
  ...
  (`ALFA_OAUTH_*`) для живого прогона; Приор — движок+connect готовы, автоопрос выключен (A8 по запросам + слот).
...
Остаётся: банк-креды владельца для живого прогона, включение Приора в автоопрос —
...
4. ✅ **A6 — сделано** (PR #293): реестр счетов на опрос — `listAllBankAccounts` (все счета всех
   ...
   (группировка по порталу+провайдеру, дедуп, фильтр `POLLABLE_PROVIDERS`={alfa-by} (Приор пока вне автоопроса), отсев демо) кормит
...
6. ✅ **A5 — сделано** (PR #289): HTTP-транспорт `server/utils/bankFetch.ts` (ядра за DI):
   ...
   тихий `[]`). **Приор** (async create+poll) — движок `priorFetch.ts` готов (резолв `accountId`, 429≠пусто), в автоопрос не включён. **Подключён к
   воркеру — A9 (ниже).**
...
8. ✅ **A8 — сделано** (PR #298): **глобальный** rate-limiter `Q_FETCH` — BullMQ worker `limiter`
   ...
   поэтому до включения Приора в автоопрос нужен учёт по запросам + отдельная занятость слота.
...
- **Приорбанк прод — СКЗИ (BY-crypto TLS)**: нужен крипто-шлюз (СКЗИ, issue #41) + прод-метод
  аутентификации `private_key_jwt` (issue #444); на sandbox не требуется ни то, ни другое.
  Сертификат ГосСУОК **не нужен** (он только для `tls_client_auth`); Договор об использовании
  Платёжного API с банком — **заключён**.
