...
- ⛔ **Приорбанк, песочница: банк отказывает на создании согласия** (`403 BY.NBRB.Reauthenticate`,
  переписка идёт, 2026-08-13). Без согласия не выдаётся ни `x-accountConsentId`, ни
  `openbanking_intent_id`, а без них `GET /accounts` не работает — то есть **весь онлайн-путь Приора
  стоит**, и sandbox, и прод. Со стороны приложения исчерпано всё, что проверяется снаружи: тело,
  заголовки, scope, метод аутентификации, подписки, тип ключей, имя приложения, наличие приложения в
  Магазине API, владелец. Доказательная база и таблица исключённого — [`PRIOR_API.md`](PRIOR_API.md),
  «Открытый блокер». ⚠ Это **не** транспорт: BY-крипто канал к проду проверен живым прогоном (ниже).
