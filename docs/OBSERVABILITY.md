...
- **Ручные спаны** (`@opentelemetry/api`, no-op когда SDK не зарегистрирован) — **покрывают весь конвейер**:
  ...
  - `withSpan('http.<route>', …)` / `withFrameRouteSpan(...)` — **все фрейм-токен HTTP-роуты** (порт #220/#221):
    ...
    `import/{status,metrics,metrics-reset}`, `bank/{connect,accounts,disconnect,set-account}` (#404/#407), `setup-status` (#409/#405) и `distribution/{ledger,provision,recompute}` (у последних
...
## Алертинг — это не телеметрия

Собственных алертов у OpenTelemetry-контура нет: коллектор опциональный, дашборды нужно открыть
глазами. Единственный канал, который **стучится сам** — Telegram-оповещения о здоровье очередей
(#426). Он работает **независимо от телеметрии**, читает BullMQ напрямую и спанами не покрыт (кроме
корневого `cron.queue-health`). Порядок включения и границы покрытия — [`OPERATIONS.md`](OPERATIONS.md)
