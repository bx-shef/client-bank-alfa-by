...
| `crm.item.update` | classic | `crm` | `server/utils/allocationMutationWrite.ts` | да | актуален | **Мутация разнесения — стадия инвойса** (§2, #109): переводит смарт-счёт (`entityTypeId=31`) в оплаченную стадию (`fields:{stageId}`) при цели `invoice` и заданной настройке `invoicePaidStageId`. Ответ `{result:{item}}`. За гейтом `autoDistribute`; тот же idempotent-порядок. Live READ+dry-run — `pnpm mutate:test --invoice <id> --stage <s>`. |
