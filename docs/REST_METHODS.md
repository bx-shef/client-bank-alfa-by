...
| `crm.activity.delete` | classic | `crm` | `server/utils/todoActivityWrite.ts` | нет | актуален | Сносит дело, которому не удалось поставить маркер: немаркированное дело хуже отсутствующего — дедуп не найдёт его никогда, и повтор положит рядом второе. |
...
| `im.dialog.get` | im | `im` | `server/utils/chatSearch.ts` | **нет** | актуален | Название сохранённого чата по `DIALOG_ID` для пикера настроек (иначе в форме виден сырой `chat123`). Зовётся только когда названия нет ни в кэше настроек, ни в списке недавних. |
