...
| `crm.item.list` | classic | `crm` | `invoiceLookup.ts`, `companyLookup.ts`, `itemByIdLookup.ts` | да | актуален | Поиск смарт-счёта (`entityTypeId=31`) по номеру+компании (#109); фильтр «моей» компании (`entityTypeId=4`, `isMyCompany='Y'`, Этап C); поиск любой цели **по id+компании** (IDOR-скоуп, `itemByIdLookup`). Поля подтверждены на живом портале. |
