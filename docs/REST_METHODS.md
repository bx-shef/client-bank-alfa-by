...
| Метод | Поколение | Scope | Где будет | Батч | Статус / замена | Назначение |
|-------|-----------|-------|-----------|------|-----------------|------------|
| `im.search.chat.list` | im | `im` | backend-прокси поиска чатов (#16 PR-C) | TBD¹ | актуален | Поиск чата по названию/участникам (`FIND`≥3, `LIMIT`≤50, `OFFSET`). |
| `im.recent.list` | im | `im` | backend-прокси (дефолтный список) | да | актуален | Последние диалоги пользователя (пагинация) — список «по умолчанию» в пикере. |

> ¹ Батчируемость `im.search.chat.list` пока **не подтверждена**: в оф. доке нет явного
> per-method-флага (`ERROR_BATCH_METHOD_NOT_ALLOWED` — общий текст в списке ошибок у многих методов,
> не признак именно этого). Проверить на живом портале при внедрении PR-C.
>
> **Триггер переноса planned→live:** когда метод внедрён — переносим строку в серверную таблицу
> выше, ставим реальный файл-владелец и **фактическую** батчируемость, и переносим сюда же тонкость
> идентичности (ниже), если она подтвердилась.

> **Тонкость идентичности (`im.*`).** `im.search.chat.list`/`im.recent.list` возвращают чаты,
> доступные **текущему пользователю** токена. Уведомления шлём серверным токеном (личность
> установщика) → и **искать чаты нужно тем же токеном**, иначе можно выбрать чат, куда отправитель
> не входит, и `im.message.add` упадёт. Поэтому пикер ходит в backend-прокси, а не во фрейм.
