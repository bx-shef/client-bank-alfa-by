...
| `crm.item.list` | classic | `crm` | `server/utils/invoiceLookup.ts`, `companyLookup.ts` | да | актуален | Поиск смарт-счёта (`entityTypeId=31`) по номеру+компании (#109); а также фильтр «моей» компании (`entityTypeId=4`, `isMyCompany='Y'`) среди найденных по счёту — Этап C. Поля подтверждены на живом портале. |
