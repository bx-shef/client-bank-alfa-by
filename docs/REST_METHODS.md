...
| `crm.documentgenerator.document.list` | classic | `crm` | `server/utils/documentLookup.ts` | да | актуален | Мост-документ (#109): `document-number` → привязанная сущность `{entityTypeId, entityId}` (фильтр `number`, ответ `result.documents[]`, поля `number`/`entityTypeId`/`entityId`). Scope `crm` (метод под `crm.documentgenerator.*`). ⚠ поля из офдоки, **вживую не подтверждено** (в seed 0 документов). Ref недоверенный → вызывающий рескоупит по компании через `itemByIdLookup`. |
