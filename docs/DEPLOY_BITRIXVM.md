...
```bash
deploy/bitrixvm/offline-snapshot.sh   # в cron раз в неделю
```

Работающие контейнеры переживают исчезновение источника образов, а вот пересоздание
контейнера — нет. Скрипт обновления поэтому **никогда** не удаляет образы (в основном
таргете `make prod-redeploy` делает `docker image prune -f` — здесь такого шага нет).
