...
- **Backend (Nitro, `server/`):** серверная часть в том же приложении (как `bx-synapse`).
  ...
  - **REST-фундамент разнесения оплат (#109, первый слайс; чистое ядро + стор, DI, тесты):**
    ...
    - `server/utils/stageLoader.ts` — чистый **loader «отрицательных» стадий** (DI над `RestCall`, тесты):
      ...
      `invoiceStageEntityId(catId)` (`SMART_INVOICE_STAGE_<catId>`); `makeIsNegativeStage(set)` строит предикат,
      который принимает `findInvoicesByNumber` (раньше инъектировался «снаружи»); `loadInvoiceNegativeStage`
      — loader+предикат одним вызовом. **Подтверждено вживую**: «Не оплачен» `DT31_11:D` = `SEMANTICS='F'`.
    Осталось: lookup остальных целей (сделка/оплата/смарт-процесс/мост-документ), поиск моей компании,
    проводка в `crm-sync` (там же связать `stageLoader`→`invoiceLookup`), хранение матриц/карты в настройках.
...
- **Скрипты разведки (dev, не часть SSG):**
  ...
  - **Тестовый портал Б24 и скоупы вебхука** (для ручной проверки #109; сам портал сменный —
    ...
    - **`crm`** — есть; хватает для всего текущего seed (компании/реквизиты/счета/сделки/смарт-процессы/
      товары) и путей #109 (поиск компании, инвойс, оплата сделки — создание/проведение).
    - **`sale`** — **нужен только для полной очистки**: отмена/удаление **оплаченной** оплаты сделки
      (`sale.payment.update PAID=N`); без него `--purge` пропускает сделку с оплаченной оплатой (чистится
      в UI). Для прод-#109, скорее всего, **не нужен** (разнесение — запись в нашей БД `allocation_fact`).
      Владелец добавит по необходимости — до этого ограничение известно и обойдено.
    ...
    (`crm`, `im`, `user_brief`, `placement`); `sale` там пока нет — открытый вопрос в `PROCESSING.md §8`.
