...
- `app/pages/install.vue` — обработчик установки B24 (layout `clear`): `init` → `event.bind`
  ...
  **Проверка серверной части (#413):** `ONAPPINSTALL` идёт **мимо iframe** — исходящим вебхуком
  ...
  backend недоступен. Чужой маршрут взят намеренно — он уже отвечает 409 «portal not installed»
  ровно в искомом случае, отдельный эндпоинт не нужен. Проверка с 3 попытками и паузой: доставка
  события асинхронна и идёт параллельно с `installFinish`, мгновенный приговор давал бы ложную
  тревогу на здоровой установке. Не удалось определить ⇒ `unknown` ⇒ молчим. Билдер батча
