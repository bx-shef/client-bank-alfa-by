...
- **UI-контур распределения (#109 §9.3 #4, admin-only, за feature-gate `DISTRIBUTION_PROVISION_ENABLED`
  ...
  неограниченному держателю достаётся более короткое из двух ожиданий.
  ...
  (`ensureBankToken(token, deps, { lockWait })`, без умолчания — фоновому вызывающему ждать долго
  правильно), а исчерпание ожидания на сверке читается как состояние — тем же текстом, что у
  ...
  замерено мутацией), а всплеск nginx на обоих маршрутах держит ниже пула соединений С УЧЁТОМ
  стоимости запроса: сверка спрашивает банки параллельно, то есть берёт по соединению на банк. По смыслу очереди тоже не
...
- **Backend (Nitro, `server/`):** серверная часть в том же приложении (как `bx-synapse`).
  ...
  - **«Моя компания» и сверка счетов — предусловие и экран (#493/#494).** Первый боевой прогон дал
    ...
    - `server/utils/bankAccountList.ts` — сторона БАНКА: `extractAlfaAccounts` (`GET /accounts/` →
      ...
      чинить то, что не сломано. Отдельная задача (#538 по соседству).
