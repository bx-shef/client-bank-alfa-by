...
- `server/utils/serverLogger.ts` — **серверный** канальный логгер поверх jssdk (#529): `useServerLogger(channel)`,
  ...
  ⚠ Формат свой, а не готовый `LineFormatter`, по измеренной причине: дефолт SDK добавляет к каждой
  строке `{} {} <дата>` — ~26 лишних байт и дубль метки времени, которую docker ставит сам, а объём
  логов у нас измерен (#498). ⚠ Маркер печатает КАНАЛ, поэтому из `buildOpLogLine`/`runSummaryLine`
