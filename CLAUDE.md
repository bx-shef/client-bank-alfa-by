...
> ⚠ Из issue остаётся **объём логов**: `[op]` безусловна и на масштабе «10 порталов × 100 оплат/мин»
> сжимает историю до ~3 часов — не блокирует сегодня, ударит на масштабе).
...
- **Backend (Nitro, `server/`):** серверная часть в том же приложении (как `bx-synapse`).
  ...
  - **Очереди (BullMQ + Redis)** — шина под нагрузку и масштабирование. Полное описание (поток,
    ...
    **Диагностические строки лога (#498):** `[fetch]` — на каждый опрос (провайдер/счёт/окно/
    ...
    без цели/сколько опущено построчно), `[op]` — на обработанную операцию (направление/валюта/
    **счёт контрагента**/исход/сколько номеров распознано).
    ⚠ **Безусловны первые две, а не `[op]`.** Замерено: строка `[op]` весит 221 байт с обвязкой
    docker json-file, и на масштабе «10 порталов × 100 оплат/мин» ОДНА она сжимает историю логов до
    четырёх часов при ротации 10 МБ × 5 (⚠ замерена САМА СТРОКА, четыре часа — арифметика над ней;
    оценка «с распознаванием останется ~1,5 часа» — экстраполяция при множителе ×2,5, не замер). Поэтому
    безусловная гарантия перенесена с «каждой операции» на «каждый прогон»: `[crm-sync]` печатается
    ВСЕГДА (раньше её не было вовсе — сводка считалась, уезжала в БД и метрики, но в лог не
    попадала), а `[op]` по умолчанию печатается только для операций, которые **ни во что не
    приземлились**. ⚠ Это не потеря: у приземлившегося платежа авторитетная запись — само ДЕЛО в
    Б24 с маркером, оно точнее строки лога и не истекает через четыре часа. Критерий
    самонастраивающийся: на здоровом портале лог тихий, на сломанном («117 обработано, 0 создано»)
    полный — ровно тогда, когда нужен. Режим — `STATEMENT_OP_LOG` (`notable` по умолчанию | `all`
    для калибровки | `off` аварийный клапан, ⚠ глушит и диагностику). Сколько строк опущено,
    говорит сам итог прогона — молчаливое сокращение читалось бы как «больше ничего и не было».
