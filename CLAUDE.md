...
- **Доменное ядро (чистое, переносимо в backend, покрыто тестами):**
  ...
  - `app/utils/importStatsChart.ts` — **чистое построение опций столбчатого графика** (#419): расходы
    ...
    подписи оси вынесены НАРУЖУ (`inside` убран) + отступ `BAR_GRID_LEFT` под них, подписи и подсказка
    форматируются по модулю (иначе двойной минус). ⚠ Знак — **только отображение**: в `importStats`
