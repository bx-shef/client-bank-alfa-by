...
- `app/components/DayField.vue` / `DayRangeField.vue` — выбор дня и периода штатной парой
  ...
  в документации b24ui), #588. Наружу — строка
...
- **Backend (Nitro, `server/`):** серверная часть в том же приложении (как `bx-synapse`).
  ...
  - `server/api/poll-now.post.ts` (+ чистое ядро `server/utils/pollNow.ts`, DI, тесты; #54/#302) —
    ...
    **Забор за выбранный день (#588):** тот же маршрут принимает необязательный `day` (`ГГГГ-ММ-ДД`)
  ...
  - **Очереди (BullMQ + Redis)** — шина под нагрузку и масштабирование. Полное описание (поток,
    ...
    **Диагностические строки лога (#498):** `[fetch]` — на каждый опрос (провайдер/счёт/окно/
    ...
    условные приписки **«⚠ N без записи в реестр»** (#575) и **«⚠ N без привязок»** (#579) — обе про ОТКАЗ, а не про штатную
