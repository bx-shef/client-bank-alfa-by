...
- ⚠ **Но у самой проверки типов есть слепое пятно, и починка #542 в него УПАЛА.** Транспорт
  ...
  **маршрутом через типизированный хелпер** — он объявляет `dealId: number`, и невидимое место
  становится видимым. ⚠ Цена промаха несимметрична: это сверка ПОСЛЕ мутации живого портала, и
  ложное «оплата не помечена» толкает оператора повторить `--apply` по уже проведённому платежу.
...
- ⚠ **Комментарии скриптов — АНГЛИЙСКИЕ**, и это не вкусовщина: `mutate-payment-live`,
  `fuzz-allocation`, `trigger-register-test`, `lib/*` — англоязычные модули (32–36 строк
  комментариев против 3–5 русских, которые #542 туда и занёс). Смешение языков ВНУТРИ модуля —
  тот самый дефект чтения из §Конвенции. ⚠ Русские слова в «ёлочках» остаются: это ЦИТАТЫ значений
  портала (`Оплачен`, `Организация`, «Сделка Опт»), а не проза. Кластер `tests/**`, наоборот,
  русский (145 файлов против 112) — тест-гарды пишем по-русски.
