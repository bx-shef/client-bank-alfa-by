...
- **Доменное ядро (чистое, переносимо в backend, покрыто тестами):**
  ...
  - `app/utils/purposeMatch.ts` — **чистое распознавание идентификатора из назначения платежа по МАТРИЦАМ**
    ...
    не найдена». Решено ИМЕНОВАННЫМ коротким списком (`PREFIX_NOISE`: слово «счёт» в падежах +
    `N`/`NO`), а не послаблением границы для любой буквы: послабление вернуло бы ложные номера из
    ...
    проверяется граница — без неё `PN1234` стал бы номером. `foldHomoglyphs`
