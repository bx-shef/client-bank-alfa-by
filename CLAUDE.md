...
- `app/components/DayField.vue` — выбор ОДНОГО дня на штатном `B24Calendar` (#588). Наружу — строка
  ...
  очистки. ⚠ Родного `type="date"` было мало по РАЗНЫМ причинам: у забора нужна граница «не в
  ...
  подстановка сегодняшнего числа поменяла бы смысл настройки за человека. Границы периода очистки
  связаны между собой (`:max="to"` / `:min="from"`), поэтому перевёрнутый период не выбрать вовсе.
