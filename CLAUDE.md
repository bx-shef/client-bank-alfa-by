...
- `app/pages/app.vue` — in-portal просмотр выписки на **b24ui** (по образцу B24-списка «Последние
  ...
  **Демо-набор операций — только за `?preview=1`** (скриншоты и визуальные тесты; в портале список
  ...
  показались бы никогда — список в портале пуст. **Баннер «не
...
использует (`--ui-color-design-filled-success-bg: var(--ui-color-accent-main-success)`). На белом
фоне карточки они дают **2.07:1** и **3.12:1** при пороге 4.5:1 — то есть «переход с сырой палитры
Tailwind на семантические токены» выглядит правильным по конвенции и молча ломает читаемость.
Ровно это и произошло со СУММОЙ в списке операций — самым важным элементом строки.

Рабочая пара для текста — **токен на тему**: `text-(--ui-color-green-90)
dark:text-(--ui-color-accent-main-success)` (4.77 / 5.27) и `text-(--ui-color-red-80)
dark:text-(--ui-color-accent-main-alert)` (6.07 / 4.14). Для иконки-носителя смысла порог 3:1, и
штатный `tinted-*-content` на фоне `tinted-*-bg` его тоже не проходит в светлой теме (2.44:1).

⚠ Проверять — **числом**, а не глазом: обе темы, оба фона (карточка и плитка). Скриншот этого не
ловит, а конвенция «только семантические токены» сама по себе не гарантирует контраст.
