...
- **Backend (Nitro, `server/`):** серверная часть в том же приложении (как `bx-synapse`).
  ...
  - `server/utils/bankConnectStart.ts` + `server/api/bank/connect.post.ts` (стадия 5; A7b-1) — **старт
    ...
    **ОДНО ПОДКЛЮЧЕНИЕ — НЕСКОЛЬКО СЧЕТОВ (#23-#25)** — колонка `grant_id`, `POST /api/bank/add-account`
    ...
    ⚠ **Само добавление лок НЕ берёт**, и это классифицировано, а не забыто (`bankRefreshLock.ts`):
    исходная строка только ЧИТАЕТСЯ, пишется новая, за которую никто не борется. ⚠ Новый счёт **не
