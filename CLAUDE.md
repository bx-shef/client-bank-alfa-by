...
- `app/components/SettingsForm.vue` — форма настроек чата (#16 PR-C): два пикера чатов на
  **`AsyncSearchSelect`** (чат уведомлений `chat.dialogId` + **чат ошибок** `errorChat.dialogId`,
  ...
  блок «Исключения» `B24Textarea` (наш счёт / **счёт контрагента** (#562: налоговая, комиссии банка,
  эквайринг — плательщики, которых в CRM не будет никогда, а их дела затапливали «мою компанию»;
  поле ОТДЕЛЬНОЕ от списка наших счетов — смешение значило бы, что номер, вписанный «выключить
  плательщика», однажды совпадёт с нашим счётом и молча выключит весь его поток) / подстрока
