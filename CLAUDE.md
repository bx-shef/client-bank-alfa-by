...
- **UI-контур распределения (#109 §9.3 #4, admin-only, за feature-gate `DISTRIBUTION_PROVISION_ENABLED`
  ...
  правильно), а исчерпание ожидания на сверке показывает «банк опрашивается прямо сейчас», а не
  сырое `canceling statement due to lock timeout` с именами таблиц. Инвариант структурный:
  `tests/bankRefreshLock.test.ts` требует `lockWait` от КАЖДОГО вызова `ensureBankToken` в
  `server/api/bank/**` — поведенческий тест такого не ловит, маршрут-то работает. По смыслу очереди тоже не
