...
> **⚠ Срез после первого боевого прогона (2026-08-15).** Транспорт добыт целиком: **оба банка**
> подключены на промышленном контуре из портала, за окно опроса прошли **208 реальных операций**
> (банк → очередь → разбор → `crm-sync`). **И ничего не записалось** — `117 обработано, 117 не
