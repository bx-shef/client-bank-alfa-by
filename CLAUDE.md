...
реестр»** (#575) и **«⚠ N без привязок»** (#579) — все три про ОТКАЗ, а не про штатную
  ...
  - **Разнесение оплат: серверные модули** (#109) — реестр вынесен в
    ...
    - **привязки дела** (#579) — чистый `app/utils/activityBindings.ts` (что вязать: элемент реестра,
      сущность списания, обе компании; владелец исключается) + `server/utils/activityBindingsWrite.ts`
