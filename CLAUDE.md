...
> опознано, 0 создано`: в CRM нет компании «моя» с реквизитами подключённого счёта, поэтому §4/§2
> даже не запускаются. Дальше мешает не код, а ненастроенный портал и четыре дефекта —
...
- **Backend (Nitro, `server/`):** серверная часть в том же приложении (как `bx-synapse`).
  ...
  - **Очереди (BullMQ + Redis)** — шина под нагрузку и масштабирование. Полное описание (поток,
    ...
    **Диагностические строки лога (#498 контекст):** `[fetch]` — на каждый опрос (провайдер/счёт/окно/
    ...
    исход/сколько номеров распознано). Обе **безусловны**; `[recognize]`/`[resolve]`/`[allocate]` — только
    когда распознавание что-то нашло. ⚠ Заведены потому, что операция, не нашедшая компанию, раньше не
