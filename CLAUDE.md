...
- ⚠ **Тесты типизируются ТРЕТЬИМ проходом** (`tsconfig.tests.json`, #527). До него `pnpm typecheck`
  ...
  **поведение**, а тип описывает **контракт с production-кодом**, и расходятся они молча. Замер при
  ...
  альтернативы — полсотни `!` по ассертам и приученная к ним рука, которая пишет то же в production. ⚠ **Скрипты тоже в этом проходе** (#542) — и у них риск ВЫШЕ, чем у тестов:
  `mutate-payment-live`, `todo-activity-test`, `trigger-register-test` с `--apply` мутируют CRM
  живого клиента, а запускаются через `node --experimental-strip-types` — это ВЫРЕЗАНИЕ типов, без
  проверки. Замер дал пять ошибок, из них две — в самом мутирующем скрипте (`findDealPayments`
  принимает `dealId: string`, а получал число; работало только потому, что внутри стоит
  `String(dealId)`). ⚠ `tests/nuxt/**` исключён потому, что покрыт
  **app-проходом**, `tests/visual/**` и `playwright.config.ts` — Playwright со своим окружением.
  ⚠ Последнее — не отговорка, а замер: единственная ошибка там (`reducedMotion` якобы не существует
  в `UseOptions`) — **артефакт разрешения типов**, а не дефект кода: поле объявлено в
  `PlaywrightTestOptions` установленной версии 1.62.1, проверено прямым чтением `test.d.ts`. Под
  app-проходом тот же файл вдобавок теряет `process` (нет node-типов). То есть форсировать его
  сюда значило бы либо повесить заведомо ложное подавление, либо держать проход вечно красным —
  Playwright нужен свой конфиг, и это отдельная задача. Охват стережёт
