...
- **Скрипты разведки (dev, не часть SSG):**
  ...
  - `scripts/seed-test-b24.mjs` (`pnpm seed:b24` / `--list` / `--purge`) — **идемпотентный посев тестовых
    ...
    процессы (с направлениями `entityTypeId=1032` / без `1030`), смарт-счета (оплачен `DT31_11:P` /
    открытый `:N` / не оплачен `:D`=SEMANTICS=F, исключается `invoiceLookup`), сделки в разных воронках
    (с привязанным оплаченным счётом / без оплаты), товары, компании-клиенты (с реквизитами и без → путь
    ...
    **Порядок purge важен**: реквизиты (каскад банк-деталей) удаляются ПОКА компания жива — иначе Б24
    осиротит их без прав на удаление, и «зомби»-банк-деталь навсегда испортит поиск по счёту. Подтверждено
    на живом портале: `companyLookup` (счёт→компания) и поля смарт-счёта работают. Dev-only, не часть SSG.
