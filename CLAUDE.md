...
- **Вторичные экраны открываются НАСТОЯЩИМ слайдером портала** (`slider.openSliderAppPage({ place })`,
  ...
  **наш** адрес и передаёт `place` в `PLACEMENT_OPTIONS`; глобальный мидлвар
  ...
  `ai-price-import` тот же код работает потому, что там страница рендерится НА ЗАПРОС. ⚠ Ось
  различия — пререндер против рендера на запрос, а НЕ «nginx против Nitro»: Nitro есть и у нас, он
  отдаёт `/api/*`, просто HTML страниц он не рендерит никогда. Взводит поведение метка
  `prerenderedAt` в САМОМ payload (`.output/public/app/_payload.json`), поэтому отдавай ту же
  пререндеренную страницу хоть наш Nitro — было бы то же самое;
