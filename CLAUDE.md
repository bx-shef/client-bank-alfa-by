...
- **UI-контур распределения (#109 §9.3 #4, admin-only, за feature-gate `DISTRIBUTION_PROVISION_ENABLED`
  ...
  ⚠ Ожидание лока здесь **короткое** (`SINGLE_FLIGHT_LOCK_WAIT`, 1 с) и это НЕ копия решения из
  #509: там держатель работает доли секунды и подождать осмысленно, здесь он делает десятки
  REST-вызовов и работает десятки секунд — ждать почти наверняка не дождаться, заняв соединение из
  пула (пул — 10), из которого берут readiness-проба и все остальные порталы. По смыслу очереди
  тоже не нужно: если операция идёт, второму делать нечего.
