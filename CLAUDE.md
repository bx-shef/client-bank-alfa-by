...
- **Backend (Nitro, `server/`):** серверная часть в том же приложении (как `bx-synapse`).
  ...
  - **Умирающие подключения — видим МЫ, а не клиент (#497 §3).** Два выхода из одних данных, и это
    ...
    - **Пульс ВОРКЕРОВ (#466 §1)** — `app/utils/workerHeartbeat.ts` (чистое правило) + ключи
      ...
      ⚠ Заведён потому, что ВСЕ правила здоровья выведены из НАЛИЧИЯ застрявшей работы:
    ...
    - **Канал оповещений говорит о себе (#466 §3)** — `alertChannelState` + `presentAlertChannel`,
      ...
      «сигнализация выключена» (нет env) или «включена, но не доходит» (отозванный бот, неверный
      `chat_id` — сегодня это только `console.error`). Три состояния разведены явно; «данных нет»
      читается как «выключено», а не как «исправно».
