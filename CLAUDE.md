...
- `app/components/LandingDemo.vue` + чистое ядро `app/utils/demoExtract.ts` (карта — [`docs/DEMO_LANDING.md`](docs/DEMO_LANDING.md))
  ...
  **по матрицам** номера счетов/заказов через реальный `recognizeByMatrices`). **Онлайн-подключение**
  ...
  ⛔ **Онлайн-путь Приора при этом СТОИТ, и не из-за транспорта:** банк отказывает на создании
  согласия (`403 BY.NBRB.Reauthenticate`) и в песочнице, и, следовательно, в проде — без согласия нет
  ни `x-accountConsentId`, ни `openbanking_intent_id`, а без них `/accounts` не отвечает. Переписка с
  банком идёт; всё, что проверяется со стороны приложения, исчерпано — доказательная база в
  [`docs/PRIOR_API.md`](docs/PRIOR_API.md), «Открытый блокер»).
...
- **Backend (Nitro, `server/`):** серверная часть в том же приложении (как `bx-synapse`).
  ...
  - `server/utils/bankConnectStart.ts` + `server/api/bank/connect.post.ts` (стадия 5; A7b-1) — **старт
    ...
    **Страницы возврата из банка закрываются сами** (`bankConnectCallback.ts`, 5 с, `AUTO_CLOSE_SEC`):
    ...
    («можно закрыть эту вкладку») — счётчик рисует сам скрипт. ⚠ **Успешных страниц ДВЕ**: подключение
