...
- `app/components/SettingsForm.vue` — форма настроек чата (#16 PR-C): два пикера чатов на
  **`AsyncSearchSelect`** (чат уведомлений `chat.dialogId` + **чат ошибок** `errorChat.dialogId`,
  ...
  ⚠ **ПРОГРАММНО НЕ ИСКЛЮЧАЕТСЯ НИЧЕГО** (решение владельца, 2026-08-23): никаких зашитых перечней
  плательщиков, эвристик по имени контрагента и «умных» умолчаний — в CRM не попадает ровно то, что
  админ вписал в эти три поля. Отсюда **отклонён** обсуждавшийся вариант «автоматически заводить
  бюджет/банки/эквайринг в отдельную компанию»: он выглядел удобством, а означал бы, что приложение
  само решает, чей платёж куда положить. Собрать их в одну компанию «Прочие расходы» админ может и
  **без нашего кода** — завести компанию и вписать их счета в её реквизиты, поиск по `RQ_ACC_NUM`
  найдёт её как любую другую; цена — отдельный реквизит на плательщика. Выбор за админом, и это
  сказано ему в справке (`no-client`) и на самом экране «Исключений», а не только здесь: пустые поля
  иначе читаются как «очевидное уже отфильтровано»)
