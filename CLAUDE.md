...
- **UI-контур распределения (#109 §9.3 #4, admin-only, за feature-gate `DISTRIBUTION_PROVISION_ENABLED`
  ...
  освобождает только `expires_at`. Падение посреди операции запирает **один** портал до конца срока
  (`PROVISION_LEASE_SEC` / `RECOMPUTE_LEASE_SEC` — второй втрое длиннее, у него потолок работы
  минуты, а не секунды); прежний отказ выедал общий пул, то есть накрывал весь сервис.
  ...
  в удержании соединения, а не в числе операций, и с арендой он исчез целиком. Общий семафор,
  наоборот, сериализовал бы НЕЗАВИСИМЫЕ порталы — админ портала Б получал бы «занято» из-за
  портала А.
