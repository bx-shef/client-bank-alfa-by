...
- **Вторичные экраны открываются НАСТОЯЩИМ слайдером портала** (`slider.openSliderAppPage({ place })`,
  ...
  **`ssr: false`** — SPA, где восстанавливать нечего. ⚠ Ось различия — ПРЕРЕНДЕР против SPA, а не
  «nginx против Nitro»: Nitro есть и у нас, он отдаёт `/api/*`. Проверено экспериментом: с
  `ssr: false` `_payload.json` не появляется вовсе, и голый `return navigateTo` уводит фрейм штатно.
