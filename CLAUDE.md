...
- **UI-контур распределения (#109 §9.3 #4, admin-only, за feature-gate `DISTRIBUTION_PROVISION_ENABLED`
  ...
  `RENAME_LOCK_WAIT` (#509), а не другое. Прежняя формулировка «там держатель работает доли
  ...
  неограниченному держателю достаётся более короткое из двух ожиданий. По смыслу очереди тоже не
  нужно: если операция идёт, второму делать нечего.
...
- **Backend (Nitro, `server/`):** серверная часть в том же приложении (как `bx-synapse`).
  ...
  - `server/utils/bankConnectStart.ts` + `server/api/bank/connect.post.ts` (стадия 5; A7b-1) — **старт
    ...
    **Временный ключ подключения (#407):** порядок «сначала банк, потом счёт» родился здесь, а поле
    ...
    новому он не пересёкся бы с обновляющей стороной ни разу. ⚠ Ждать можно 10 с (`lock_timeout`),
    а держит POST с потолком 15 с, поэтому «не дождался» — ШТАТНЫЙ исход: **503 «занято, повторите»**,
