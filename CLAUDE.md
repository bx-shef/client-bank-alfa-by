...
- **Backend (Nitro, `server/`):** серверная часть в том же приложении (как `bx-synapse`).
  ...
  - `server/utils/bankConnectStart.ts` + `server/api/bank/connect.post.ts` (стадия 5; A7b-1) — **старт
    ...
    **Прод-метод аутентификации — СДЕЛАН** (#444): `private_key_jwt` вместо sandbox-only
    ...
    места в обход. **Осталось:** живой прогон (`pnpm prior:test --auth-method private_key_jwt`) —
    упирается в общий блокер согласия, а не в транспорт: BY-крипто канал к проду проверен вживую
    (#477), покупать СКЗИ не нужно, issue #41 закрыт. Переключение реального деплоя тянет смену
    `client_id` → переподключение счетов (порядок — `docs/PRIOR_API.md`).
    ...
    **Канал к боевому банку проверен живым прогоном** (2026-08-13: `upstream=401` от
    ...
    прогон повторять. Живая интеграция целиком (боевая регистрация в банке) — #461.
