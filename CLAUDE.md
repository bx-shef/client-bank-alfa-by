...
- **Доменное ядро (чистое, переносимо в backend, покрыто тестами):**
  ...
  - `app/utils/clientBankStatement.ts` — нормализация разобранной текстовой выписки в `StatementItem`
    ...
    → числовой ISO `CurrCode`/`I3`/`I1` (`643`=RUB, `933`=BYN — единственный маркер на валютных
    `Type=3`/`Type=5` «за день», #169) → BYN-дефолт для BY-счёта. Для инвалютной операции сумма берётся
