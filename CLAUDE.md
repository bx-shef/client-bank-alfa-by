...
> ✅ **Ночь прожита пять раз** — дела созданы 15, 16, 17, 18 и 21 августа, в **03:00**, то есть ровно
> в окно, где подключение прежде и умирало. Почти шесть суток при сроке refresh ~10 ч возможны только
> при работающем продлении, поэтому
> [#488](https://github.com/bx-shef/client-bank-alfa-by/issues/488)/[#489](https://github.com/bx-shef/client-bank-alfa-by/issues/489)
> **закрыты**. ⚠ Прежняя формулировка «подключение умерло за ~16 часов» относилась к состоянию ДО
> #489, когда refresh обновлял только сам опрос по дороге; сейчас `scheduleBankKeepAlive` зовётся ДО
> `queueEnabled()`, то есть продление идёт независимо от опроса и от Redis.
