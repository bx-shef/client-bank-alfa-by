...
> ничего не делает). План на тестовую эксплуатацию — [`docs/project-map.md`](docs/project-map.md).
...
- **Backend (Nitro, `server/`):** серверная часть в том же приложении (как `bx-synapse`).
  ...
  - `server/utils/bankConnectStart.ts` + `server/api/bank/connect.post.ts` (стадия 5; A7b-1) — **старт
    ...
    **Канал к боевому банку проверен живым прогоном** (2026-08-13: `upstream=401` от
    ...
    прогон повторять — и запускать `bash scripts/prod-doctor.sh`, он ищет строку
  ...
  - **Keep-alive БАНКОВСКИХ токенов (#488/#489)** — `server/utils/bankTokenKeepAlive.ts` + чистое ядро
    ...
    ⇔ интерфейс говорит истекло» закреплён тестом. Честный срок — `expirationDate` согласия (#503).
  ...
  - **Очереди (BullMQ + Redis)** — шина под нагрузку и масштабирование. Полное описание (поток,
    ...
    **Диагностические строки лога (#498 контекст):** `[fetch]` — на каждый опрос (провайдер/счёт/окно/
    сколько операций) и `[op]` — на каждую обработанную операцию (направление/валюта/**счёт контрагента**/
    исход/сколько номеров распознано). Обе **безусловны**. Остальные три — нет, и условия у них РАЗНЫЕ:
...
**Регресс-защита в CI (#3):** джоба `visual` снимает 8 экранов (26 снимков: у лендинга тема одна — он форсирует тёмную)
...
- **Базовый канал «сотрудник» (реализовано):** виджет `app/components/FeedbackWidget.vue` (+
  ...
  **ПЯТЬ точек (#499)**, и они расставлены по тому, где проблема СЛУЧАЕТСЯ, а не где есть место:
  ...
  самой постановке задачи); на `/app` под полосой статуса.
