...
- **Backend (Nitro, `server/`):** серверная часть в том же приложении (как `bx-synapse`).
  ...
  - **Обзор состояния подключений для ОПЕРАТОРА (#497 §3)** — чистое ядро
    `app/utils/bankHealthOverview.ts` (`summarizeBankHealth`, `HEALTH_ORDER`/`HEALTH_TITLE`) +
    `server/api/ops/bank-health.get.ts` (гейт — та же сессионная кука, что у `/api/ops/queues`) +
    карточка «Подключения банков» на `/queues`. Админ портала видит СВОИ подключения (бейдж в
    настройках, #488); мы не видели ничего, и умирающее подключение узнавалось по факту
    ...
    ⚠ **Отдаются ТОЛЬКО счётчики** — ни номеров счетов, ни `member_id`, ни доменов; токены не
    расшифровываются (`listAllBankAccountInfo` их и не выбирает). Оператору нужно «что-то ломается
    и у скольких», а не чужие реквизиты — та же граница, что в телеметрии (`PRIVACY.md`).
    ...
    ⚠ **Сбой БД ⇒ 503 и текст ошибки на экране**, а не пустая сводка: «ноль подключений» читалось бы
    как «всё спокойно» ровно тогда, когда спокойно точно не всё. Тот же принцип, что у
    `queueHealthRead` (недоступная очередь = `unreadable`, а не нули).
