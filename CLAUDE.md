...
- **Backend (Nitro, `server/`):** серверная часть в том же приложении (как `bx-synapse`).
  ...
  - **Оповещения оператору (Telegram, #426)** — единственный канал, который **стучится сам** (лог и
    ...
    пол-часа на переобъявление против мерцания; «объявлено» = реально доставлено, иначе один 429
    ...
    назначений/контрагентов в канале нет по построению. Env — только на cron-инстансе (иначе N копий
    от реплик воркера); порядок работы — [`OPERATIONS.md`](docs/OPERATIONS.md) «Оповещения оператору».
